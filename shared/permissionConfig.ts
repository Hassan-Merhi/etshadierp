 ...
 *
 * Key naming conventions (separate from existing FEATURE_KEYS which use plain names):
 ...
 *
 * Semantics (handled by permissionHelpers.ts):
 ...
 *
 * UI consequence:
 *   Owner/Manager/POS: switch ON = restriction is active (stored as enabled=false)
 *   Normal User:       switch ON = access is granted  (stored as enabled=true)
 */

export type PermissionType = "page" | "tab" | "action" | "sensitive" | "export" | "pos";
...
const ACTION_PERMISSIONS: PermissionEntry[] = [
  { key: "act_create_voucher",   label: "Create Vouchers",           group: "Accounting Actions",  type: "action",
  ...
  { key: "act_manage_users",     label: "Manage Users & Roles",      group: "Admin Actions",       type: "action",
...
const SENSITIVE_PERMISSIONS: PermissionEntry[] = [
  { key: "fld_cost_price",       label: "Cost Price / Avg Rate",    group: "Financial Fields",  type: "sensitive",
...
const EXPORT_PERMISSIONS: PermissionEntry[] = [
  ...
  { key: "exp_backup_download",      label: "Download Backup / Data Export",group: "Export & Print", type: "export",
...
const POS_PERMISSIONS: PermissionEntry[] = [
  { key: "pos_perm_override_price",  label: "Override Item Price",     group: "POS Capabilities", type: "pos",
...
export const PERMISSION_BY_KEY: Record<string, PermissionEntry> =
  Object.fromEntries(PERMISSION_CATALOG.map((e) => [e.key, e]));

/** All distinct type values */
export const PERMISSION_TYPES = ["page", "tab", "action", "sensitive", "export", "pos"] as const;
