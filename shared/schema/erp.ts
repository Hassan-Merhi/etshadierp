...
export const aiImportJobs = pgTable("ai_import_jobs", {
  id: serial("id").primaryKey(),
  companyId: integer("company_id").notNull(),
  userId: varchar("user_id").notNull(),
...
export const insertAiImportJobSchema = createInsertSchema(aiImportJobs).omit({ id: true, createdAt: true, updatedAt: true });
...
export const aiImportRows = pgTable("ai_import_rows", {
  id: serial("id").primaryKey(),
...
export const insertSupplierProformaLineSchema = createInsertSchema(supplierProformaLines).omit({ id: true }).extend({
...
export const insertSupplierContainerLoadedItemSchema = createInsertSchema(supplierContainerLoadedItems).omit({ id: true }).extend({
...
export const insertStoredFileSchema = createInsertSchema(storedFiles).omit({ id: true, uploadedAt: true }).extend({
  companyId: z.number().min(1),
...
export const insertSpreadsheetSchema = createInsertSchema(spreadsheets).omit({ id: true, updatedAt: true }).extend({
  companyId: z.number().min(1),
...
export const insertLiveSpreadsheetSchema = createInsertSchema(liveSpreadsheets).omit({ id: true, createdAt: true, updatedAt: true }).extend({
  companyId: z.number().min(1),
...
export const insertErpWorkerDocSchema = createInsertSchema(erpWorkerDocs).omit({ id: true, uploadedAt: true }).extend({
  companyId: z.number().min(1),
...
export const insertLocationPriceGroupSchema = createInsertSchema(locationPriceGroups).omit({ id: true, createdAt: true });
...
export const insertStatusBuilderSheetSchema = createInsertSchema(statusBuilderSheets).omit({ id: true, updatedAt: true });
