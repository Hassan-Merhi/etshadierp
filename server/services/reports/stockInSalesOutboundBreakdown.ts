...
async function loadSalesOut(filters: StockInSalesReportFilters): Promise<AggregateRow[]> {
  ...
  const key = periodKey(activityDate, filters.grouping);
  ...
  addItemFilters(
    ...
    filters.search ? [ilike(vouchers.voucherNumber, `%${filters.search}%`), ilike(vouchers.locationName, `%${filters.search}%`)] : []
  );
...
export async function getStockInSalesOutboundBreakdown(filters: StockInSalesReportFilters): Promise<{
  summary: StockInSalesOutboundMetrics;
  rows: Map<string, StockInSalesOutboundMetrics>;
}> {
...
export function applyOutboundBreakdown(
  ...
  return {
    ...result,
    summary: enrich(result.summary, breakdown.summary),
    rows: result.rows.map((row) => enrich(row, breakdown.rows.get(row.periodKey) ?? publicMetrics(emptyBreakdown()))),
  };
}
