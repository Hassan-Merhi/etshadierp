...
export async function applyExactHistoricalCostReplayV5(
  ...
  try {
    ...
    for (const rawStockId of publicScope.rawStockIdsToUpdate) {
      ...
      if (containerId == null || canonicalRate == null || !approvedContainerIds.has(containerId)) {
        throw scopeViolation(`raw-stock ${rawStockId} has no approved container correction`);
      }
    ...
    if (onCommit) await onCommit(executor, result, { before, after });
    await client.query("COMMIT");
  } catch (error) {
