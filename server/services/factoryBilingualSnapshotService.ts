...
async function targetExists(item: FactoryBilingualSnapshotTarget, executor: typeof db = db): Promise<boolean> {
  ...
  try {
    ...
    const columnExists = Boolean((columnResult.rows[0] as { column_exists?: boolean } | undefined)?.column_exists);
    ...
    for (const relation of relations) {
      ...
      `);
      const tableExists = Boolean((tableResult.rows[0] as { table_exists?: boolean } | undefined)?.table_exists);
