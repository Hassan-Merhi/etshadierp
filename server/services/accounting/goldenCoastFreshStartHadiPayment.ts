...
export const GOLDEN_COAST_FRESH_START_HADI_PAYMENT_SOURCE_TYPE =
  "golden-coast-fresh-start-hadi-payment";
const MONEY_SCALE = 2;
...
function parseDate(value: unknown): string {
  ...
  if (
    ...
    throw new GoldenCoastFreshStartHadiPaymentError(
      "paymentDate must be a real calendar date",
    );
  }
...
function parseCashAccount(
  value: unknown,
): GoldenCoastFreshStartHadiCashAccount {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new GoldenCoastFreshStartHadiPaymentError(
      "hadiCashAccount is required",
    );
  }
  const raw = value as Record<string, unknown>;
  if (raw.kind !== "ledger" && raw.kind !== "bank") {
    throw new GoldenCoastFreshStartHadiPaymentError(
      'hadiCashAccount.kind must be "ledger" or "bank"',
    );
  }
...
export function parseGoldenCoastFreshStartHadiPayment(input: {
  ...
  if (
    ...
    throw new GoldenCoastFreshStartHadiPaymentError(
      "A payment request body is required",
    );
  }
  ...
  if (
    ...
    throw new GoldenCoastFreshStartHadiPaymentError(
      "clientRequestId must be 1-64 supported characters",
    );
  }
  const reference =
    typeof raw.reference === "string" && raw.reference.trim()
      ? raw.reference.trim().slice(0, 200)
      : null;
  return {
    companyId,
    hadiCompanyId,
...
export function planGoldenCoastFreshStartHadiPayment(input: {
  ...
  const payable = nonNegativeMoney(
    input.gcSalesCashPayableUsd,
    "gcSalesCashPayableUsd",
  );
  const outstanding = nonNegativeMoney(
    input.outstandingHadiSalesCashUsd,
    "outstandingHadiSalesCashUsd",
  );
  const hadiAsset = nonNegativeMoney(
    input.hadiIntercompanyAssetUsd,
    "hadiIntercompanyAssetUsd",
  );
...
export function goldenCoastFreshStartHadiPaymentDigest(input: {
  ...
  return createHash("sha256")
    .update(
      JSON.stringify({ payment: input.payment, accounts: input.accounts }),
    )
    .digest("hex")
    .slice(0, 32);
}
...
export function buildGoldenCoastFreshStartHadiPaymentPostings(input: {
  ...
  const gcDescription = releaseDebtEnglish(
    `HADI paid Fresh Start for Golden Coast${suffix}`,
  );
  const hadiDescription = releaseDebtEnglish(
    `Fresh Start payment on behalf of Golden Coast${suffix}`,
  );
  ...
  const tag = (
    ...
    source: {
      ...
      idempotencyKey: goldenCoastFreshStartHadiPaymentIdempotencyKey(
        plan.companyId,
