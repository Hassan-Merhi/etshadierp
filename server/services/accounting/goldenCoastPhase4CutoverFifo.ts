...
function positiveId(value: number, label: string): number {
  if (!Number.isInteger(value) || value <= 0) throw new GoldenCoastPhase4CutoverError(`${label} must be a positive integer`);
  return value;
}
...
export function buildGoldenCoastCutoverFifoPlan(input: {
  ...
  if (opening.isNegative()) throw new GoldenCoastPhase4CutoverError("stockInHandOpeningUsd cannot be negative");
  ...
  for (const row of input.inventory) {
    ...
    if (!articleCode) throw new GoldenCoastPhase4CutoverError(`inventory ${inventoryId} is missing an article code`);
    if (seenInventoryIds.has(inventoryId)) throw new GoldenCoastPhase4CutoverError(`duplicate inventory row ${inventoryId}`);
    ...
    movements.push({
      ...
      articleCode,
      description: `Golden Coast ${GOLDEN_COAST_CUTOVER_DATE} opening FIFO snapshot from ERP inventory #${inventoryId}${row.description ? ` — ${row.description}` : ""}`,
      stockItemId,
      locationId,
  ...
  if (!difference.isZero()) {
    throw new GoldenCoastPhase4CutoverError(
      `ERP inventory snapshot value ${roundedSnapshot.toFixed(2)} does not reconcile to Phase 3 Stock in Hand opening ${roundedOpening.toFixed(2)} (difference ${difference.toFixed(2)})`
    );
  }
