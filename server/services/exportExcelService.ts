...
async function qStream(sql: string): Promise<any[]> {
  try {
    ...
    console.warn(`[ExportStream] Query warning: ${err.message}`);
    return [];
  }
}
...
export async function streamCompanyWorkbookDirect(
  ...
  // ── Finance ────────────────────────────────────────────────────────────────
  await fetch1("Ledger Accounts",       `SELECT * FROM ledger_accounts WHERE company_id = ${cid} AND deleted_at IS NULL ORDER BY code`);
  await fetch1("Bank Accounts",         `SELECT * FROM bank_accounts WHERE company_id = ${cid} AND deleted_at IS NULL ORDER BY name`);
  await fetch1("Fixed Assets",          `SELECT * FROM fixed_assets WHERE company_id = ${cid} ORDER BY name`);
  await fetch1("Exchange Rates",        `SELECT * FROM exchange_rates WHERE company_id = ${cid} ${df("effective_date")} ORDER BY effective_date DESC, id`);
  ...
  // ── Vouchers ───────────────────────────────────────────────────────────────
  await fetch1("Vouchers",       `SELECT * FROM vouchers WHERE company_id = ${cid} ${df("voucher_date")} ORDER BY voucher_date, id`);
  await fetch1("Voucher Entries",`SELECT ve.* FROM voucher_entries ve INNER JOIN vouchers v ON v.id = ve.voucher_id WHERE v.company_id = ${cid} ${df("v.voucher_date")} ORDER BY ve.id`);
  ...
  await fetch1("Customer Order Bales",   `SELECT cob.* FROM customer_order_bales cob INNER JOIN customer_orders co ON co.id = cob.order_id WHERE co.company_id = ${cid} ORDER BY cob.id`);
  await fetch1("Customer Order Charges", `SELECT coc.* FROM customer_order_charges coc INNER JOIN customer_orders co ON co.id = coc.order_id WHERE co.company_id = ${cid} ORDER BY coc.id`);
  await fetch1("Customer Proformas",     `SELECT * FROM customer_proformas WHERE company_id = ${cid} ORDER BY id DESC`);
  await fetch1("Customer Proforma Lines",`SELECT cpl.* FROM customer_proforma_lines cpl INNER JOIN customer_proformas cp ON cp.id = cpl.proforma_id WHERE cp.company_id = ${cid} ORDER BY cpl.id`);
  await fetch1("Credit Note Items",      `SELECT cni.* FROM credit_note_items cni INNER JOIN vouchers v ON v.id = cni.voucher_id WHERE v.company_id = ${cid} ORDER BY cni.id`);
  ...
  await fetch1("Factory Worker Categories", `SELECT * FROM factory_worker_categories WHERE company_id = ${cid} ORDER BY id`);
  await fetch1("Factory Worker Documents",  `SELECT * FROM factory_worker_documents WHERE company_id = ${cid} ORDER BY id`);
  await fetch1("Factory Worker Advances",   `SELECT * FROM factory_worker_advances WHERE company_id = ${cid} ORDER BY id`);
  await fetch1("Factory Advance Repayments",`SELECT * FROM factory_advance_repayments WHERE company_id = ${cid} ORDER BY id`);
  await fetch1("Factory Payrolls",          `SELECT * FROM factory_payrolls WHERE company_id = ${cid} ${df("period_start")} ORDER BY period_start, id`);
  await fetch1("Factory Attendance",        `SELECT * FROM factory_attendance WHERE company_id = ${cid} ${df("attendance_date")} ORDER BY attendance_date, id`);
  ...
  await fetch1("Factory Daybook",        `SELECT * FROM factory_daybook_entries WHERE company_id = ${cid} ${df("tx_date")} ORDER BY tx_date, id`);
  await fetch1("Daybook Entry Edits",    `SELECT fde.* FROM factory_daybook_entry_edits fde INNER JOIN factory_daybook_entries fdb ON fdb.id = fde.daybook_entry_id WHERE fdb.company_id = ${cid} ORDER BY fde.id`);
  await fetch1("Factory KPI Snapshots",  `SELECT * FROM factory_daily_kpi_snapshots WHERE company_id = ${cid} ${df("date")} ORDER BY date DESC`);
  await fetch1("Factory Daily Usages",   `SELECT * FROM factory_daily_usages WHERE company_id = ${cid} ${df("used_date")} ORDER BY used_date DESC`);
  await fetch1("Factory Alerts",         `SELECT * FROM factory_alerts WHERE company_id = ${cid} ORDER BY id DESC`);
  await fetch1("Factory Duty Audit Log", `SELECT * FROM factory_duty_audit_log WHERE company_id = ${cid} ORDER BY id DESC LIMIT 50000`);
  ...
  await fetch1("Factory Bales",               `SELECT * FROM factory_bales WHERE company_id = ${cid} ${df("pressed_at::date")} ORDER BY pressed_at DESC, id`);
  ...
  await fetch1("Factory Ctnr Commissions",    `SELECT * FROM factory_container_commissions WHERE company_id = ${cid} ORDER BY id`);
  await fetch1("Factory Ctnr Other Charges",  `SELECT * FROM factory_container_other_charges WHERE company_id = ${cid} ORDER BY id`);
  await fetch1("Factory Ctnr PL Snapshots",   `SELECT * FROM factory_container_profit_snapshots WHERE company_id = ${cid} ORDER BY id DESC`);
  await fetch1("Offload Additional Charges",  `SELECT * FROM factory_offload_additional_charges WHERE company_id = ${cid} ORDER BY id`);

  // ── Factory Suppliers ──────────────────────────────────────────────────────
  await fetch1("Factory Suppliers",           `SELECT * FROM factory_suppliers WHERE company_id = ${cid} ORDER BY name`);
  await fetch1("Factory Supplier Payments",   `SELECT * FROM factory_supplier_payments WHERE company_id = ${cid} ${df("date")} ORDER BY date DESC, id`);
  await fetch1("Factory Supplier FX Xfers",   `SELECT * FROM factory_supplier_fx_transfers WHERE company_id = ${cid} ORDER BY id DESC`);
  await fetch1("Factory Supplier Scores",     `SELECT * FROM factory_supplier_score_snapshots WHERE company_id = ${cid} ORDER BY id DESC`);

  // ── Factory FX & POS ──────────────────────────────────────────────────────
  await fetch1("Factory FX Rates",       `SELECT * FROM factory_fx_rates WHERE company_id = ${cid} ORDER BY id DESC`);
  await fetch1("Factory FX Allocations", `SELECT * FROM factory_fx_allocations WHERE company_id = ${cid} ORDER BY id DESC`);
  await fetch1("Factory POS Sales",      `SELECT * FROM factory_pos_sales WHERE company_id = ${cid} ${df("tx_date")} ORDER BY tx_date DESC, id`);
  await fetch1("Factory POS Sale Items", `SELECT fps.* FROM factory_pos_sale_items fps INNER JOIN factory_pos_sales fp ON fp.id = fps.sale_id WHERE fp.company_id = ${cid} ORDER BY fps.id`);

  // ── Bales (Sorting) ────────────────────────────────────────────────────────
  await fetch1("Bales (Sorting)",        `SELECT * FROM bales WHERE company_id = ${cid} ${df("date_pressed")} ORDER BY date_pressed DESC, id`);
  await fetch1("Bale Products",          `SELECT * FROM bale_products WHERE company_id = ${cid} ORDER BY name`);
  await fetch1("Bale Product Categories",`SELECT * FROM bale_product_categories WHERE company_id = ${cid} ORDER BY name`);
  await fetch1("Bale Sequences",         `SELECT * FROM bale_sequences WHERE company_id = ${cid} ORDER BY id`);
  await fetch1("Bale Label Prints",      `SELECT * FROM bale_label_prints WHERE company_id = ${cid} ORDER BY id DESC LIMIT 100000`);
  await fetch1("Bale Transfers",         `SELECT * FROM bale_transfers WHERE company_id = ${cid} ORDER BY id DESC`);
  await fetch1("Bale Transfer Items",    `SELECT bti.* FROM bale_transfer_items bti INNER JOIN bale_transfers bt ON bt.id = bti.transfer_id WHERE bt.company_id = ${cid} ORDER BY bti.id`);
  await fetch1("Bale Recode Sessions",   `SELECT * FROM bale_recode_sessions WHERE company_id = ${cid} ORDER BY id DESC`);
  await fetch1("Bale Recode Items",      `SELECT bri.* FROM bale_recode_items bri INNER JOIN bale_recode_sessions brs ON brs.id = bri.session_id WHERE brs.company_id = ${cid} ORDER BY bri.id`);
  ...
  await fetch1("Stock Item Location Prices",`SELECT silp.* FROM stock_item_location_prices silp INNER JOIN stock_items si ON si.id = silp.stock_item_id WHERE si.company_id = ${cid} ORDER BY silp.id`);
  await fetch1("Inventory by Location",    `SELECT i.*, si.code AS item_code, si.name AS item_name, l.name AS location_name FROM inventory i INNER JOIN stock_items si ON si.id = i.stock_item_id INNER JOIN locations l ON l.id = i.location_id WHERE si.company_id = ${cid} ORDER BY l.name, si.code`);
  await fetch1("Inventory Negative Layers",`SELECT * FROM inventory_negative_layers WHERE company_id = ${cid} ORDER BY id`);
  await fetch1("Stock Group Archives",     `SELECT * FROM stock_group_location_archives WHERE company_id = ${cid} AND deleted_at IS NULL ORDER BY id`);
  await fetch1("Stock Group Archive Items",`SELECT sglai.* FROM stock_group_location_archive_items sglai INNER JOIN stock_group_location_archives sgla ON sgla.id = sglai.archive_id WHERE sgla.company_id = ${cid} ORDER BY sglai.id`);
  await fetch1("Stock Transfers",          `SELECT stv.*, v.voucher_number, v.voucher_date, v.description AS transfer_notes, l1.name AS from_location_name, l2.name AS to_location_name FROM stock_transfer_vouchers stv INNER JOIN vouchers v ON v.id = stv.voucher_id LEFT JOIN locations l1 ON l1.id = stv.source_location_id LEFT JOIN locations l2 ON l2.id = stv.destination_location_id WHERE v.company_id = ${cid} ${df("v.voucher_date")} ORDER BY v.voucher_date, stv.id`);
  ...
  // ── Containers ─────────────────────────────────────────────────────────────
  await fetch1("Containers Detail", `
  ...
  await fetch1("Containers",               `SELECT * FROM containers WHERE company_id = ${cid} ${df("import_date")} ORDER BY import_date DESC, id`);
  await fetch1("Container Charges",         `SELECT cc.* FROM container_charges cc INNER JOIN containers c ON c.id = cc.container_id WHERE c.company_id = ${cid} ORDER BY cc.id`);
  await fetch1("Container Offloads",        `SELECT co.* FROM container_offloads co INNER JOIN containers c ON c.id = co.container_id WHERE c.company_id = ${cid} ORDER BY co.id`);
  await fetch1("Offload Items",             `SELECT coi.* FROM container_offload_items coi INNER JOIN container_offloads co ON co.id = coi.offload_id INNER JOIN containers c ON c.id = co.container_id WHERE c.company_id = ${cid} ORDER BY coi.id`);
  await fetch1("Container Freight",         `SELECT * FROM container_freight WHERE company_id = ${cid} ORDER BY id DESC`);
  await fetch1("Container Freight Payments",`SELECT * FROM container_freight_payments WHERE company_id = ${cid} ORDER BY id DESC`);
