...
function hasProductText(record: Record<string, unknown>): boolean {
  return ["productName", "productNameEn", "productNameAr", "productNameFr", "baleName", "baleNameEn", "baleNameAr", "baleNameFr", "product_name", "product_name_ar", "product_name_fr", "bale_name", "bale_name_ar", "bale_name_fr"].some((key) => key in record);
}
function isCatalogProduct(record: Record<string, unknown>): boolean {
  return Boolean(objectArticleCode(record)) && ("name" in record || "nameAr" in record || "nameFr" in record || "description" in record || "descriptionAr" in record || "descriptionFr" in record);
}
function isCategoryRecord(record: Record<string, unknown>): boolean {
  return !objectProductId(record) && !objectArticleCode(record) && "id" in record && "name" in record && ("nameAr" in record || "nameFr" in record || "nameEn" in record);
}
...
function localizeRecord(record: Record<string, unknown>, catalog: CatalogRecord | null, language: FactoryCatalogLanguage, options: ResolveOptions): void {
  ...
  if (englishSnapshot || arabicSnapshot || frenchSnapshot) {
    displayName = resolveFactorySnapshotProductName({
      language,
      snapshot: { articleCode, productNameEn: englishSnapshot, productNameAr: arabicSnapshot, productNameFr: frenchSnapshot },
      catalog: catalog ? { articleCode: catalog.articleCode, name: catalog.name, nameAr: catalog.nameAr, nameFr: catalog.nameFr } : null,
    });
  } else {
    const resolved = resolveFactoryProductLanguage({
      ...
      categoryNameAr: firstText(record, ["categoryNameAr", "categoryAr", "category_name_ar"]) ?? catalog?.categoryNameAr,
      categoryNameFr: firstText(record, ["categoryNameFr", "categoryFr", "category_name_fr"]) ?? catalog?.categoryNameFr,
    }, language);
