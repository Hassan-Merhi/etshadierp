...
function objectArticleCode(value: Record<string, unknown>): string {
  return normalizeFactoryArticleCode(value.articleCode ?? value.article_code ?? value.productArticleCode ?? value.product_article_code);
}
function hasProductText(record: Record<string, unknown>): boolean {
  return ["productName", "productNameEn", "productNameAr", "baleName", "baleNameEn", "baleNameAr", "product_name", "product_name_ar", "bale_name", "bale_name_ar"].some((key) => key in record);
}
function isCatalogProduct(record: Record<string, unknown>): boolean {
  return Boolean(objectArticleCode(record)) && ("name" in record || "nameAr" in record || "description" in record || "descriptionAr" in record);
}
function isCategoryRecord(record: Record<string, unknown>): boolean {
  return !objectProductId(record) && !objectArticleCode(record) && "id" in record && "name" in record && ("nameAr" in record || "nameEn" in record);
}
function isResolvableRecord(record: Record<string, unknown>): boolean {
  return (Boolean(objectProductId(record) || objectArticleCode(record)) && (hasProductText(record) || isCatalogProduct(record))) || isCategoryRecord(record);
}
...
function firstText(record: Record<string, unknown>, keys: string[]): string | null {
  for (const key of keys) { const result = clean(record[key]); if (result) return result; }
  return null;
}
...
function localizeRecord(record: Record<string, unknown>, catalog: CatalogRecord | null, language: FactoryCatalogLanguage, options: ResolveOptions): void {
  ...
  if (englishSnapshot || arabicSnapshot) {
    displayName = resolveFactorySnapshotProductName({ language, snapshot: { articleCode, productNameEn: englishSnapshot, productNameAr: arabicSnapshot }, catalog: catalog ? { articleCode: catalog.articleCode, name: catalog.name, nameAr: catalog.nameAr } : null });
  } else {
    const resolved = resolveFactoryProductLanguage({ articleCode, name: clean(record.name) ?? catalog?.name, nameAr: clean(record.nameAr) ?? catalog?.nameAr, description: clean(record.description) ?? catalog?.description, descriptionAr: clean(record.descriptionAr) ?? catalog?.descriptionAr, categoryName: firstText(record, ["categoryName", "category", "category_name"]) ?? catalog?.categoryName, categoryNameAr: firstText(record, ["categoryNameAr", "categoryAr", "category_name_ar"]) ?? catalog?.categoryNameAr }, language);
    displayName = resolved.name;
    record.displayDescription = resolved.description;
    record.displayCategoryName = resolved.categoryName;
  }
  const displayCategory = resolveFactoryCategoryName({ name: firstText(record, ["categoryName", "category", "category_name"]) ?? catalog?.categoryName, nameAr: firstText(record, ["categoryNameAr", "categoryAr", "category_name_ar"]) ?? catalog?.categoryNameAr }, language, articleCode);
  record.language = language;
  record.direction = language === "ar" ? "rtl" : "ltr";
  if (articleCode) record.normalizedArticleCode = articleCode;
  if (displayName) { record.displayName = displayName; record.displayProductName = displayName; }
  if (displayCategory) { record.displayCategory = displayCategory; record.displayCategoryName = displayCategory; }
  ...
  if (isCatalogProduct(record)) { if (!record.nameEn) record.nameEn = clean(record.name) ?? catalog?.name ?? null; record.name = displayName; }
  if (displayCategory) ["category", "categoryName", "category_name"].forEach((key) => setIfPresent(record, key, displayCategory));
}
export async function resolveFactoryBilingualSurfacePayload(companyId: number, payload: unknown, language: FactoryCatalogLanguage, options: ResolveOptions): Promise<unknown> {
  const productIds = new Set<number>();
  const articleCodes = new Set<string>();
  visit(payload, (record) => { const productId = objectProductId(record); const articleCode = objectArticleCode(record); if (productId) productIds.add(productId); if (articleCode) articleCodes.add(articleCode); });
  const catalog = await loadCatalog(companyId, [...productIds], [...articleCodes]);
  visit(payload, (record) => {
    const productId = objectProductId(record); const articleCode = objectArticleCode(record);
    const product = (productId ? catalog.byId.get(productId) : null) ?? (articleCode ? catalog.byArticleCode.get(articleCode) : null) ?? null;
