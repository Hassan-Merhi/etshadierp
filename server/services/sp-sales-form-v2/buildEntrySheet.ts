...
export async function buildEntrySheet(
  ...
  for (const gb of groupBounds) {
    ...
    gb.items.forEach((item, gIdx) => {
      ...
      for (let d = 0; d < dayCount; d++) {
        ...
        prC.value = {
          formula: `IF(OR(${qL}${r}="",${pL}${r}=""),0,${pL}${r}-${colLetter(COL_COSTBAG)}${r})`,
          result: profitVal ?? 0,
        } as ExcelJS.CellValue;
        prC.numFmt = MONEY_FMT;
        prC.font = normSm;
        prC.alignment = right;
        prC.border = thin;
      ...
      cqC.value = {
        formula: `IF(${openQtyL}${r}="",0,${openQtyL}${r})-SUM(${qtyCellRefs.join(",")})`,
        result: Math.round(item.closeQty),
      } as ExcelJS.CellValue;
      cqC.numFmt = QTY_FMT;
      cqC.font = normSm;
      cqC.alignment = right;
      cqC.border = thin;
      cqC.fill = fill(CLOSE_GRN);
      cqC.protection = { locked: true };

      // Closing Value — formula =CloseQtyCell * $F{r}  ← $F = Cost/Bag (col 6)
      const cvC = ws.getCell(r, closeQtyCol + 1);
      cvC.value = {
        formula: `${colLetter(closeQtyCol)}${r}*${colLetter(COL_COSTBAG)}${r}`,
        result: r2(item.closeValue) || 0,
      } as ExcelJS.CellValue;
      cvC.numFmt = MONEY_FMT;
      cvC.font = normSm;
      cvC.alignment = right;
      cvC.border = thin;
      cvC.fill = fill(CLOSE_GRN);
      cvC.protection = { locked: true };
      ...
      avgC.numFmt = QTY_FMT;
      avgC.font = normSm;
      avgC.alignment = right;
      avgC.border = thin;
      avgC.fill = fill(TOTALS_ORG);
      avgC.protection = { locked: true };
    });
    ...
    stLabel.value = gb.groupName;
    stLabel.fill = fill(YELLOW_GRP);
    ...
    ws.getCell(stRow, COL_OPENQTY).numFmt = QTY_FMT;
    ws.getCell(stRow, COL_OPENQTY).alignment = right;
    ...
    for (let d = 0; d < dayCount; d++) {
      ...
      ws.getCell(stRow, b).value = {
        formula: `SUM(${qL}${gb.firstRow}:${qL}${gb.lastRow})`,
        result: qtyTot,
      } as ExcelJS.CellValue;
      ws.getCell(stRow, b).numFmt = QTY_FMT;
      ws.getCell(stRow, b).alignment = right;
      ...
      ws.getCell(stRow, b + 1).numFmt = MONEY_FMT;
      ws.getCell(stRow, b + 1).alignment = right;
      ...
      ws.getCell(stRow, b + 2).numFmt = MONEY_FMT;
      ws.getCell(stRow, b + 2).alignment = right;
    }

    // Closing Qty/Value sums (close qty can be negative — no clamping)
    const cqL = colLetter(closeQtyCol),
      cvL = colLetter(closeQtyCol + 1);
    ...
    ws.getCell(stRow, closeQtyCol).numFmt = QTY_FMT;
    ws.getCell(stRow, closeQtyCol).alignment = right;
    ...
    ws.getCell(stRow, closeQtyCol + 1).numFmt = MONEY_FMT;
    ws.getCell(stRow, closeQtyCol + 1).alignment = right;
  }
  ...
  if (items.length > 0) {
    ...
    ws.getCell(totalRowNum, COL_OPENQTY).numFmt = QTY_FMT;
    ws.getCell(totalRowNum, COL_OPENQTY).font = { ...boldSm, color: { argb: WHITE } };
    ...
    for (let d = 0; d < dayCount; d++) {
      ...
      ws.getCell(totalRowNum, b).value = { formula: `SUM(${stNumRefs(b)})`, result: qtyTot } as ExcelJS.CellValue;
      ws.getCell(totalRowNum, b).numFmt = QTY_FMT;
      ws.getCell(totalRowNum, b).font = { ...boldSm, color: { argb: WHITE } };

      ws.getCell(totalRowNum, b + 1).value = {
        formula: `SUM(${stNumRefs(b + 1)})`,
        result: salTot || 0,
      } as ExcelJS.CellValue;
      ws.getCell(totalRowNum, b + 1).numFmt = MONEY_FMT;
      ws.getCell(totalRowNum, b + 1).font = { ...boldSm, color: { argb: WHITE } };

      ws.getCell(totalRowNum, b + 2).value = {
        formula: `SUM(${stNumRefs(b + 2)})`,
        result: profTot || 0,
      } as ExcelJS.CellValue;
      ws.getCell(totalRowNum, b + 2).numFmt = MONEY_FMT;
      ws.getCell(totalRowNum, b + 2).font = { ...boldSm, color: { argb: WHITE } };
    }

    // Closing totals (can be negative — no clamping)
    ws.getCell(totalRowNum, closeQtyCol).value = {
      formula: `SUM(${stNumRefs(closeQtyCol)})`,
      result: Math.round(items.reduce((s, i) => s + i.closeQty, 0)),
    } as ExcelJS.CellValue;
    ws.getCell(totalRowNum, closeQtyCol).numFmt = QTY_FMT;
    ws.getCell(totalRowNum, closeQtyCol).font = { ...boldSm, color: { argb: WHITE } };

    ws.getCell(totalRowNum, closeQtyCol + 1).value = {
      formula: `SUM(${stNumRefs(closeQtyCol + 1)})`,
      result: r2(items.reduce((s, i) => s + i.closeValue, 0)),
    } as ExcelJS.CellValue;
    ws.getCell(totalRowNum, closeQtyCol + 1).numFmt = MONEY_FMT;
    ws.getCell(totalRowNum, closeQtyCol + 1).font = { ...boldSm, color: { argb: WHITE } };
  }
  ...
  tpLbl.value = "Total Payments";
  tpLbl.font = { ...boldSm, color: { argb: WHITE } };
  tpLbl.fill = fill(DARK_BLUE);
  tpLbl.border = thin;
  tpLbl.alignment = leftAl;
  ...
  for (let d = 0; d < dayCount; d++) {
    ...
    cashC.value = { formula: `SUM(${cL}${payFirst}:${cL}${payLast})`, result: 0 } as ExcelJS.CellValue;
    cashC.numFmt = MONEY_FMT;
    cashC.fill = fill(DARK_BLUE);
    cashC.border = thin;
    cashC.alignment = right;
    cashC.font = { ...boldSm, color: { argb: WHITE } };
    cashC.protection = { locked: true };
    const bankC = ws.getCell(totalPayRow, b + 1);
    bankC.value = { formula: `SUM(${bL}${payFirst}:${bL}${payLast})`, result: 0 } as ExcelJS.CellValue;
    bankC.numFmt = MONEY_FMT;
    bankC.fill = fill(DARK_BLUE);
    bankC.border = thin;
    bankC.alignment = right;
    bankC.font = { ...boldSm, color: { argb: WHITE } };
    bankC.protection = { locked: true };
  }
  ...
  balLbl.value = "Balance Cash";
  balLbl.font = { ...boldSm, color: { argb: WHITE } };
  balLbl.fill = fill(GREEN_HDR);
  balLbl.border = thin;
  balLbl.alignment = leftAl;
