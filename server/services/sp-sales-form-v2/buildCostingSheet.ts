...
export function buildCostingSheet(wb: ExcelJS.Workbook, items: ItemRow[]): void {
  ...
  items.forEach((item, i) => {
    const r = i + 2;
    const openQty = r2(item.openQty),
      openVal = r2(item.openValue);
    ...
    avgCell.value =
      openQty > 0 ? ({ formula: `IF(D${r}=0,0,E${r}/D${r})`, result: r4(item.openRate) } as ExcelJS.CellValue) : 0;
    avgCell.numFmt = NUM4;
    // Closing Value: =G{r}*F{r}
    const clvCell = ws.getCell(r, 8);
    clvCell.value = { formula: `G${r}*F${r}`, result: r2(item.closeValue) } as ExcelJS.CellValue;
    clvCell.numFmt = NUM;
    [4, 5, 6, 7, 8].forEach((c) => {
      ws.getCell(r, c).numFmt = c === 6 ? NUM4 : NUM;
