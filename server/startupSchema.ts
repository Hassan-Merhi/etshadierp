 ...
 *
 * The idempotent `CREATE TABLE / ALTER TABLE / DO $$ … $$` statements run once at
 * server boot to bring a database up to the runtime-authoritative schema. Extracted
 * verbatim from the `migrations` array in server/index.ts so the bootstrap file stays
 * focused on wiring; the runMigrations() executor still lives in index.ts and iterates
 * this array unchanged. Statement order and contents are preserved exactly — this is a
 * pure move.
 */
import {
