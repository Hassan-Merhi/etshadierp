...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Get items at a location for editing cost prices
  app.get("/api/inventory-location-items", requireAuth, async (req, res) => {
    try {
      const companyId = req.session.currentCompanyId;
      ...
      const items = await db
        .select({
          ...
          name: stockItems.name,
          quantity: inventory.quantity,
          currentRate: inventory.averageRate,
        })
        .from(inventory)
      ...
      console.error('Inventory location items error:', error);
      res.status(500).json({ message: error.message });
    }
  });

  // Update cost price for item at location
  app.patch("/api/inventory/:locationId/item/:itemId/cost-price", requireAuth, async (req, res) => {
    try {
      const companyId = req.session.currentCompanyId;
      const locationId = parseInt(req.params.locationId);
      ...
      if (!companyId || !locationId || !itemId || !costPrice) {
        return res.status(400).json({ message: "Missing required fields" });
      }

      const newRate = parseFloat(costPrice);
      if (isNaN(newRate)) {
      ...
      const quantity = parseFloat(inv[0].quantity || "0");
      const newTotalValue = quantity * newRate;
      ...
      res.json({ message: "Cost price updated", newRate, newTotalValue });
    } catch (error: any) {
