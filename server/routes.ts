...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Payroll - Bulk Employee Payment
  app.post(
    "/api/payroll/bulk-pay-employees",
    requireAuth,
    async (req, res) => {
      try {
        if (!req.session.currentCompanyId) {
        ...
        // Create single voucher for all payments
        const voucherNumber = `EMP-BULK-${Date.now()}`;
        const [voucher] = await db
          .insert(vouchers)
          .values({
            companyId: req.session.currentCompanyId,
            ...
            voucherType: "Payment",
            voucherDate: date,
            description: notes || `Bulk salary payment for ${payments.length} employees`,
            totalAmount: totalAmount.toFixed(2),
            optional: false,
          })
          .returning();
        ...
        await db.insert(voucherEntries).values({
          ...
          debitAmount: totalAmount.toFixed(2),
          creditAmount: "0",
          narration: `Bulk salary payment - ${payments.length} employees - ${voucherNumber}`,
        });
