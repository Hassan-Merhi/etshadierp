...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/stats/net-profit", requireAuth, async (req, res) => {
    try {
      ...
      // EXPLICIT FORMULA as requested:
      // ASSETS = Inventory + Inventory OTW + Customers + Cash in Hand + Banks + All Other Assets
      ...
      // Get the net position adjustment from company settings
      const companySettingsData = await db
        .select({ netPositionAdjustment: companySettings.netPositionAdjustment })
        .from(companySettings)
        .where(eq(companySettings.companyId, companyId))
        .limit(1);
      const netPositionAdjustment = parseFloat(companySettingsData[0]?.netPositionAdjustment || "0");

      // Track totals for explicit categories
      let inventoryTotal = 0;
      let inventoryOtwTotal = 0;
      ...
      // Track for P&L (kept for backward compatibility)
      let expensesTotal = 0;
      let incomeTotal = 0;
      ...
      // Process ledger accounts for EXPLICIT NET POSITION categories
      // Categories: Cash in Hand, Banks, Loans, Other Assets
      ...
      for (const acc of companyAccounts) {
        ...
        // Income accounts (for P&L, not Net Position)
        ...
        // Expense accounts (for P&L, not Net Position)
        if (expenseTypes.includes(accountType)) {
          if (netBalance > 0) {
            expensesTotal += netBalance;
            categoryTotals[`exp_${accountType}`] = (categoryTotals[`exp_${accountType}`] || 0) + netBalance;
            expensesAccounts.push({ name: acc.name, code: acc.code || "", value: netBalance, category: accountType });
          }
          continue;
        }
        
        // Skip Profit/Equity accounts (not part of Net Position)
        if (["Profit", "Equity", "Income"].includes(accountType)) continue;
        
        // LOANS - check by account type or name pattern
        if (accountType === "Loan" || nameLower.includes("loan")) {
          if (netBalance < 0) {
            // Negative balance = we owe (liability)
            loansTotal += Math.abs(netBalance);
            categoryTotals["liability_Loans"] = (categoryTotals["liability_Loans"] || 0) + Math.abs(netBalance);
            onUsAccounts.push({ name: acc.name, code: acc.code || "", value: Math.abs(netBalance), category: "Loans" });
          } else {
            // Positive balance = someone owes us (rare for loans, but possible)
            otherAssetsTotal += netBalance;
            categoryTotals["asset_Loan Receivable"] = (categoryTotals["asset_Loan Receivable"] || 0) + netBalance;
            forUsAccounts.push({ name: acc.name, code: acc.code || "", value: netBalance, category: "Loan Receivable" });
          }
          continue;
        }
        
        // CASH IN HAND - check by code pattern, account type or name pattern
        // Find parent account if exists
        const parentAcc = acc.parentId ? companyAccounts.find(p => p.id === acc.parentId) : null;
        const isUnderCashInHand = parentAcc?.code === "CASH_IN_HAND";
        if (isUnderCashInHand || accountType === "Cash" || 
            codeLower.includes("cash") || nameLower.includes("cash in hand") || nameLower.includes("petty cash")) {
          if (netBalance > 0) {
            cashInHandTotal += netBalance;
            categoryTotals["asset_Cash In Hand"] = (categoryTotals["asset_Cash In Hand"] || 0) + netBalance;
            forUsAccounts.push({ name: acc.name, code: acc.code || "", value: netBalance, category: "Cash In Hand" });
          }
          continue;
        }
        
        // BANKS - check by account type or name pattern
        if (accountType === "Bank" || nameLower.includes("bank")) {
          if (netBalance > 0) {
            banksTotal += netBalance;
            categoryTotals["asset_Banks"] = (categoryTotals["asset_Banks"] || 0) + netBalance;
            forUsAccounts.push({ name: acc.name, code: acc.code || "", value: netBalance, category: "Banks" });
          } else {
            // Overdraft (negative bank balance) - treat as liability
            loansTotal += Math.abs(netBalance);
            categoryTotals["liability_Bank Overdraft"] = (categoryTotals["liability_Bank Overdraft"] || 0) + Math.abs(netBalance);
            onUsAccounts.push({ name: acc.name, code: acc.code || "", value: Math.abs(netBalance), category: "Bank Overdraft" });
          }
          continue;
        }
        
        // ALL OTHER ASSETS - any remaining asset-type accounts with positive balance
        if (["Asset", "Current Asset", "Fixed Asset"].includes(accountType) && netBalance > 0) {
          otherAssetsTotal += netBalance;
          categoryTotals["asset_Other Assets"] = (categoryTotals["asset_Other Assets"] || 0) + netBalance;
          forUsAccounts.push({ name: acc.name, code: acc.code || "", value: netBalance, category: "Other Assets" });
        }
      }
      ...
      if (activeLocationIds.length > 0) {
        ...
        for (const inv of inventoryData) {
          inventoryTotal += parseFloat(inv.quantity || "0") * parseFloat(inv.averageRate || "0");
        }
      }
      if (inventoryTotal > 0) {
        categoryTotals["asset_Inventory"] = inventoryTotal;
        forUsAccounts.push({ name: "Stock In Hand (Inventory)", code: "COMPUTED", value: inventoryTotal, category: "Inventory" });
      }

      // ============ INVENTORY OTW (Stock On The Way) ============
      const otwContainers = await db
        .select({
      ...
      inventoryOtwTotal = otwContainers.reduce((sum, container) => {
        return sum + parseFloat(container.grandTotal || "0");
      }, 0);
      
      if (inventoryOtwTotal > 0) {
        categoryTotals["asset_Inventory OTW"] = inventoryOtwTotal;
        forUsAccounts.push({ name: "Stock On The Way (OTW)", code: "COMPUTED", value: inventoryOtwTotal, category: "Inventory OTW" });
      }

      // ============ WORKERS (Employee Liabilities) ============
      ...
      for (const emp of companyEmployees) {
        const balance = parseFloat(emp.currentBalance || "0");
        if (balance > 0) {
          workersTotal += balance;
        }
      }
      if (workersTotal > 0) {
        categoryTotals["liability_Workers"] = workersTotal;
        onUsAccounts.push({ name: "Workers/Employees Payable", code: "COMPUTED", value: workersTotal, category: "Workers" });
      }

      // ============ CUSTOMERS (Receivables) ============
      // Customers use ledger accounts for their balances
      // Find customer ledger accounts and calculate from voucher entries
      ...
      for (const cust of companyCustomers) {
        // Calculate from opening balance and voucher entries using their linked ledger account
        let custBalance = 0;
        if (cust.ledgerAccountId) {
          const custAcc = companyAccounts.find(a => a.id === cust.ledgerAccountId);
          if (custAcc) {
            custBalance = getAccountNetBalance(custAcc);
          }
        } else {
          // Fallback: use opening balance if no ledger account
          const opening = parseFloat(cust.openingBalance || "0");
          const openingSide = cust.openingBalanceSide === "Dr" ? 1 : -1;
          custBalance = opening * openingSide;
        }
        if (custBalance > 0) {
          customersTotal += custBalance;
        }
      }
      if (customersTotal > 0) {
        categoryTotals["asset_Customers"] = customersTotal;
        forUsAccounts.push({ name: "Customer Receivables", code: "COMPUTED", value: customersTotal, category: "Customers" });
      }
      ...
      inventoryOtwTotal = round2(inventoryOtwTotal);
      customersTotal = round2(customersTotal);
      cashInHandTotal = round2(cashInHandTotal);
      banksTotal = round2(banksTotal);
      otherAssetsTotal = round2(otherAssetsTotal);
      workersTotal = round2(workersTotal);
      loansTotal = round2(loansTotal);
      ...
      const forUsTotal = totalAssets;
      const onUsTotal = totalLiabilities;
      
      // Debug logging for Net Position calculation
      console.log("Net Position Debug (Explicit Formula):", {
        companyId,
        inventoryTotal,
        inventoryOtwTotal,
        customersTotal,
