...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Create location with cash account
  app.post("/api/locations/with-cash-account", requireAuth, async (req, res) => {
    try {
      if (!req.session.currentCompanyId) {
        return res.status(400).json({ message: "No company selected" });
      }
      ...
      // Step 1: Find or create CASH ledger account
      const allLedgers = await storage.getAllLedgerAccounts(req.session.currentCompanyId);
      let cashLedger = allLedgers.find(l => l.code === "CASH");
      
      if (!cashLedger) {
        // Create CASH ledger account as Asset type (standard for cash)
        cashLedger = await storage.createLedgerAccount({
          companyId: req.session.currentCompanyId,
          code: "CASH",
          name: "Cash",
          accountType: "Asset",
      ...
      // Step 2: Create bank account for this location
      const cashAccountCode = `${parsed.code}-CASH`;
      const existingBankAccount = await storage.getBankAccountByCode(cashAccountCode);
      if (existingBankAccount) {
        return res.status(400).json({ message: `Cash account ${cashAccountCode} already exists` });
      }

      const bankAccount = await storage.createBankAccount({
        companyId: req.session.currentCompanyId,
        code: cashAccountCode,
        name: `${parsed.name} - Cash`,
        bankName: "Cash",
        accountNumber: "N/A",
        linkedLedgerId: cashLedger.id,
        openingBalance: "0",
        active: true,
      });

      // Step 3: Create location linked to the bank account
      const location = await storage.createLocation({
        ...parsed,
        linkedCashAccountId: bankAccount.id,
      });
