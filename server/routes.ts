...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    async (req, res) => {
      try {
        ...
        for (const company of allCompanies) {
          ...
          // NOTE: Mixed vouchers are EXCLUDED because their net effect is already in stockOnFloorValue
  ...
  app.get("/api/stats/import-cycle-balance", requireAuth, async (req, res) => {
    try {
      ...
      //       - These movements are internal transfers that should net to 0 in the import cycle
      //       - We track them for reporting purposes but don't include in the balance calculation
