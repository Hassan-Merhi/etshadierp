...
export async function adjustInventory(
  ...
  if (existing) {
    ...
    if (deltaQty > 0) {
      ...
      if (newQty > 0) {
        ...
        await createNegativeLayer(
          tx, companyId, locationId, stockItemId,
          shortageQty, provisionalRate,
          sourceVoucherType, sourceVoucherId,
        );
      }
    } else {
      // deltaQty === 0: no-op
    ...
    if (newQty > 0 && newTotalValue < 0)    { newTotalValue = 0; newRate = 0; }
...
export async function reverseInventoryByExactValue(
  ...
  if (newQty > 0) {
    ...
    if (newQty < 0 && companyId) {
      const shortageQty     = Math.abs(newQty);
      const provisionalRate = currentRate > 0
        ? currentRate
        : Math.abs(qtyToReverse > 0 ? valueToReverse / qtyToReverse : 0);
      await createNegativeLayer(
        tx, companyId, locationId, stockItemId,
        shortageQty, provisionalRate,
        sourceVoucherType, sourceVoucherId,
