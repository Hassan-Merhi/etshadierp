...
function modeForPath(path: string): string {
  ...
  if (path.startsWith("/api/pos/") || path.startsWith("/api/factory/pos/")) return "POS";
  return "ERP";
}
...
export function getPerformanceDashboardSnapshot() {
  ...
  const routeMap = new Map<string, { method: string; route: string; mode: string; count: number; errors: number; totalMs: number; maxMs: number; bytes: number; dbMs: number }>();

  for (const sample of samples) {
    const key = `${sample.method} ${sample.routeTemplate}`;
    const row = routeMap.get(key) || { method: sample.method, route: sample.routeTemplate, mode: sample.mode, count: 0, errors: 0, totalMs: 0, maxMs: 0, bytes: 0, dbMs: 0 };
    row.count += 1;
  ...
  const routes = [...routeMap.values()].map((row) => ({
    ...row,
...
export function handlePerformanceDashboard(req: Request, res: Response): boolean {
  ...
  const rows = snapshot.slowestRoutes.map((row) => `<tr><td>${escapeHtml(row.mode)}</td><td>${escapeHtml(row.method)}</td><td><code>${escapeHtml(row.route)}</code></td><td>${row.count}</td><td>${row.averageMs}</td><td>${row.maxMs}</td><td>${row.averageDbMs}</td><td>${Math.round(row.averageBytes / 1024)} KB</td><td>${row.errors}</td></tr>`).join("");
