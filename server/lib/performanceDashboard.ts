...
function finiteConfig(name: string, fallback: number, minimum: number): number { const parsed = Number(process.env[name]); return Number.isFinite(parsed) ? Math.max(minimum, parsed) : fallback; }
...
export function getPerformanceDashboardSnapshot() {
  prune();
  const memory = process.memoryUsage(), completed = samples.length, durations = samples.map((s) => s.durationMs), errors = samples.filter((s) => s.status >= 500).length;
  ...
  for (const sample of samples) {
    ...
    row.count++; row.errors += sample.status >= 500 ? 1 : 0; row.totalMs += sample.durationMs; row.maxMs = Math.max(row.maxMs, sample.durationMs); row.bytes += sample.responseBytes; row.dbMs += sample.dbDurationMs; row.durations.push(sample.durationMs); routeMap.set(key, row);
  }
  const routes = [...routeMap.values()].map(({ durations: values, ...row }) => ({ ...row, p95Ms: percentile(values, .95), averageMs: Math.round(row.totalMs / Math.max(1, row.count)), averageBytes: Math.round(row.bytes / Math.max(1, row.count)), averageDbMs: Math.round(row.dbMs / Math.max(1, row.count)) }));
  const byMode = ["ERP", "Factory", "Supplier Partner", "Properties", "POS"].map((mode) => { const rows = samples.filter((s) => s.mode === mode); return { mode, requests: rows.length, errors: rows.filter((s) => s.status >= 500).length, p95Ms: percentile(rows.map((s) => s.durationMs), .95) }; });
...
export function handlePerformanceDashboard(req: Request, res: Response): boolean {
  ...
  if (req.path.startsWith("/api/health/incidents")) {
    ...
    else res.status(200).type("html").send(`<!doctype html><html><head><meta charset="utf-8"><meta http-equiv="refresh" content="30"><title>ERP Incidents</title></head><body><h1>Operational Incidents</h1><p><a href="/api/health/performance">Performance</a> · <a href="/api/health/incidents.json">JSON</a></p><pre>${escapeHtml(JSON.stringify(snapshot, null, 2))}</pre></body></html>`);
    return true;
  }
  if (req.path.endsWith(".json")) { res.status(200).json(performance); return true; }
  res.status(200).type("html").send(`<!doctype html><html><head><meta charset="utf-8"><meta http-equiv="refresh" content="30"><title>ERP Performance</title></head><body><h1>Production Performance</h1><p>Bounded ${performance.windowMinutes}-minute window · maximum ${performance.sampleLimit} samples · <a href="/api/health/performance.json">JSON</a> · <a href="/api/health/incidents">Incidents</a></p><pre>${escapeHtml(JSON.stringify(performance, null, 2))}</pre></body></html>`);
  return true;
}
