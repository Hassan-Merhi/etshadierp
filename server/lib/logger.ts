...
function sanitiseValue(value: unknown, depth = 0, seen = new WeakSet<object>()): unknown {
  ...
  if (typeof value === "object") {
    ...
    for (const [key, entry] of Object.entries(value as Record<string, unknown>)) output[key] = SENSITIVE_KEY_PATTERN.test(key) ? "[REDACTED]" : sanitiseValue(entry, depth + 1, seen);
    return output;
  }
  return String(value);
}
