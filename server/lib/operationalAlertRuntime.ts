...
function evaluate(): void {
  ...
  try { evaluateOperationalAlerts(getPerformanceDashboardSnapshot()); }
  catch (error) { logger.warn("Operational alert evaluation failed", { module: "observability-alerts", action: "evaluation_failed", error }); }
}
...
export function handleOperationalIncidents(req: Request, res: Response): boolean {
  ...
  if (req.path.endsWith(".json")) { res.status(200).json(snapshot); return true; }
