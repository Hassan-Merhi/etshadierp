...
// ── AI plan generation ────────────────────────────────────────────────────────
const TOOL_LIST = TOOL_REGISTRY.map(t =>
  `  - ${t.name} (${t.type}${t.requiresApproval ? ", needs approval" : ""}): ${t.description}`
).join("\n");
  ...
  app.post("/api/ai-agent/tasks/:id/run", requireAuth, requireNonPOS, requireRole("Admin", "Owner", "Manager"), async (req, res) => {
    try {
      ...
      if (!Array.isArray(plan.steps) || plan.steps.length === 0) {
        await db.update(aiAgentTasks).set({ status: "completed", updatedAt: new Date() }).where(eq(aiAgentTasks.id, taskId));
        return res.json({ status: "completed", plan });
      }

      // Mark task as running
      await db.update(aiAgentTasks).set({ status: "running", updatedAt: new Date() }).where(eq(aiAgentTasks.id, taskId));
      ...
      await db.update(aiAgentTasks).set({
        ...
        errorMessage: failedStep
          ? (plan.steps.find(s => s.status === "failed")?.error ?? "A step failed")
          : null,
  ...
  app.post("/api/ai-agent/approvals/:id/approve", requireAuth, requireNonPOS, requireRole("Admin", "Owner", "Manager"), async (req, res) => {
    try {
      ...
      const [approval] = await db
        ...
        .from(aiAgentApprovals)
        .where(and(
          eq(aiAgentApprovals.id, approvalId),
          eq(aiAgentApprovals.companyId, companyId),
        ));

      if (!approval) return res.status(404).json({ message: "Approval not found" });
      if (approval.status !== "pending") return res.status(409).json({ message: `Approval is already ${approval.status}` });

      // Mark approved
      await db.update(aiAgentApprovals).set({
        status:     "approved",
        approvedBy: userId,
        approvedAt: new Date(),
      ...
      const finalStatus  = hasFailed ? "failed"
                        : hasWaiting ? "waiting_for_approval"
                        : allDone    ? "completed"
                        : "running";
