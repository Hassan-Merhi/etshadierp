...
export function registerSpMigrationSalesRoutes(app: Express) {
  ...
  app.post("/api/sp/migration/gc-sales-readonly", requireAuth, requireRole("Developer"), async (req: any, res: any) => {
  ...
  app.post("/api/sp/migration/gc-profit-opening", requireAuth, requireRole("Developer"), async (req: any, res: any) => {
    try {
      const {
        targetCompanyId,
        cutoffDate,
