...
export function registerPayrollBulkAdjustmentRoutes(app: Express) {
  ...
  app.post("/api/payroll/bulk-bonus-employees", requireAuth, requireNonPOS, async (req, res) => {
    try {
      ...
      const validBonuses = bonuses.filter((b: unknown) => {
      ...
      const totalAmount = validBonuses.reduce((sum: number, b: unknown) => sum + parseFloat(b.amount), 0);
      ...
      for (const [grp, grpTotal] of bonusByGroup) {
        ...
        let bonusAccount = freshAccounts.find((a: unknown) => a.code === bonusCode);
        if (!bonusAccount) {
  ...
  app.post("/api/payroll/bulk-withdraw-employees", requireAuth, requireNonPOS, async (req, res) => {
    try {
      ...
      // Filter out empty/zero amounts and validate
      const validWithdrawals = withdrawals.filter((w: unknown) => {
      ...
      // Calculate total amount
      const totalAmount = validWithdrawals.reduce((sum: number, w: unknown) => sum + parseFloat(w.amount), 0);
      ...
      if (paymentAccountType === "bank") {
        ...
        paymentAccount = allAccounts.find((a: unknown) => a.id === parseInt(paymentAccountId));
      }
      ...
      if (paymentAccountType === "bank") {
        ...
        paymentLedgerAccount = allAccounts.find((a: unknown) => a.bankAccountId === paymentAccountId_num);
        ...
        paymentLedgerAccount = allAccounts.find((a: unknown) => a.id === paymentAccountId_num);
        if (!paymentLedgerAccount) {
