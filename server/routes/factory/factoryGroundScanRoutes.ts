...
export function registerFactoryGroundScanRoutes(app: Express) {
    ...
    try {
      ...
      const result = await pool.query(
        `INSERT INTO factory_ground_scan_items
         ...
         RETURNING *`,
        [companyId, locVal, ref, articleCode || null, productName || null,
         weightKg || null, status || null, isInLoadingOrder === true, userId],
      );
  ...
  app.post("/api/factory/ground-scan-items/bulk", requireAuth, async (req: any, res: any) => {
    try {
      ...
      for (const item of items) {
        const ref = String(item.refCode || item.reference_number || "").trim().toUpperCase();
  ...
  app.delete("/api/factory/ground-scan-items/:id", requireAuth, async (req: any, res: any) => {
    try {
      ...
      await pool.query(
        `DELETE FROM factory_ground_scan_items WHERE id=$1 AND company_id=$2`,
        [id, companyId],
      );
  ...
  app.delete("/api/factory/ground-scan-items", requireAuth, async (req: any, res: any) => {
    try {
      ...
      if (locVal !== null) {
        await pool.query(
          `DELETE FROM factory_ground_scan_items WHERE company_id=$1 AND location_id=$2`,
          [companyId, locVal],
        );
      } else {
        await pool.query(
          `DELETE FROM factory_ground_scan_items WHERE company_id=$1 AND location_id IS NULL`,
          [companyId],
        );
      }
      return res.json({ success: true });
