...
export function registerV5LocationSummaryRoutes(app: Express) {
  ...
  app.get("/api/factory/v5/location-summary", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const reservedMap = new Map<string, number>(resultRows(reservedRaw).map((r) => [r.articleCode, Number(r.total)]));
      ...
      const loadingMap = new Map<string, number>(resultRows(loadingRaw).map((r) => [r.articleCode, Number(r.count)]));
