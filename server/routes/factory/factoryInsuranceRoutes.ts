...
import { assertRequestCompanyMatchesSession } from "../../services/security/companyIsolationPolicy";
...
function resolveRequestCompanyId(req: any): number {
  const sessionCompanyId = getFactoryCompanyId(req);
  const supplied = req.body?.companyId ?? req.query?.companyId;
  if (supplied != null && supplied !== "") {
    const requestedCompanyId = Number(supplied);
    assertRequestCompanyMatchesSession(
      {
        userId: req.session.userId,
        role: req.session.currentRole || "",
        companyId: Number(sessionCompanyId),
      },
      requestedCompanyId
    );
  }
  ...
  return Number(sessionCompanyId);
}

function handleCompanyIsolationError(res: any, error: any): boolean {
  if (error?.name !== "CompanyIsolationError") return false;
...
export function registerFactoryInsuranceRoutes(app: Express) {
  app.get("/api/insurance/members", requireAuth, async (req: any, res: any) => {
    try {
      ...
      const rows = await db.select().from(insuranceMembers).where(and(...conditions)).orderBy(insuranceMembers.name);
      res.json(rows);
    } catch (err: any) {
      if (handleCompanyIsolationError(res, err)) return;
  ...
  app.get("/api/insurance/members/:id/entries", requireAuth, async (req: any, res: any) => {
    try {
      ...
      const [member] = await db.select().from(insuranceMembers).where(and(eq(insuranceMembers.id, id), eq(insuranceMembers.companyId, companyId)));
    ...
    try {
      ...
      const inserted = await pool.query(
        `INSERT INTO insurance_members
         ...
         RETURNING *`,
        [companyId, data.name, data.nationality ?? null, data.positionWorking ?? null, data.insuranceNumber ?? null,
          data.startDate, data.amount, data.dob ?? null, data.notes ?? null, data.active ?? true, ledger.id]
      );
  ...
  app.patch("/api/insurance/members/:id", requireAuth, async (req: any, res: any) => {
    try {
      ...
      const companyId = resolveRequestCompanyId(req);
      const [existing] = await db.select().from(insuranceMembers).where(and(eq(insuranceMembers.id, id), eq(insuranceMembers.companyId, companyId)));
      ...
      if (data.name && data.name !== existing.name && existing.ledgerAccountId) {
        await db.update(ledgerAccounts).set({ name: `Insurance - ${data.name}` }).where(and(eq(ledgerAccounts.id, existing.ledgerAccountId), eq(ledgerAccounts.companyId, companyId)));
      }
      const [updated] = await db.update(insuranceMembers).set(data).where(and(eq(insuranceMembers.id, id), eq(insuranceMembers.companyId, companyId))).returning();
      res.json(updated);
    } catch (err: any) {
      if (handleCompanyIsolationError(res, err)) return;
  ...
  app.patch("/api/insurance/members/:id/toggle", requireAuth, async (req: any, res: any) => {
    try {
      ...
      const [existing] = await db.select().from(insuranceMembers).where(and(eq(insuranceMembers.id, id), eq(insuranceMembers.companyId, companyId)));
      if (!existing) return res.status(404).json({ message: "Member not found" });
      const [updated] = await db.update(insuranceMembers).set({ active: !existing.active }).where(and(eq(insuranceMembers.id, id), eq(insuranceMembers.companyId, companyId))).returning();
      res.json(updated);
    } catch (err: any) {
      if (handleCompanyIsolationError(res, err)) return;
  ...
  app.post("/api/insurance/generate", requireAuth, async (req: any, res: any) => {
    try {
      const schema = z.object({ month: z.number().min(1).max(12), year: z.number().min(2000).max(2100) });
      ...
      const members = await db.select().from(insuranceMembers).where(and(eq(insuranceMembers.companyId, companyId), eq(insuranceMembers.active, true)));
      ...
      for (const member of eligibleMembers) {
        ...
        if (!ledgerId) {
          ...
          ledgerId = ledger.id;
          await db.update(insuranceMembers).set({ ledgerAccountId: ledgerId }).where(and(eq(insuranceMembers.id, member.id), eq(insuranceMembers.companyId, companyId)));
        }
      ...
      const result = await db.transaction(async (tx) => {
        const [newVoucher] = await tx.insert(vouchers).values({
          companyId, voucherNumber, voucherType: "Journal", description: narration,
          voucherDate: periodStart, totalAmount: totalAmount.toFixed(2), sourceModule: "ERP",
        }).returning({ id: vouchers.id, voucherNumber: vouchers.voucherNumber });
        await tx.insert(voucherEntries).values([
          { voucherId: newVoucher.id, ledgerAccountId: expenseAccount.id, debitAmount: "0", creditAmount: totalAmount.toFixed(2), narration },
          ...memberLedgers.map((member) => ({ voucherId: newVoucher.id, ledgerAccountId: member.ledgerId, debitAmount: member.amount.toFixed(2), creditAmount: "0", narration })),
        ]);
        return { voucherId: newVoucher.id, voucherNumber: newVoucher.voucherNumber };
      });
      res.json({ voucherId: result.voucherId, voucherNumber: result.voucherNumber, totalAmount, membersCount: eligibleMembers.length, period: monthLabel });
    } catch (err: any) {
      if (handleCompanyIsolationError(res, err)) return;
