...
export function registerFactoryEmployeeBulkPayrollRoutes(app: Express) {
  ...
  app.post("/api/factory/employees/bulk-payroll", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const totalSalary = validDeposits.reduce((sum: number, d: unknown) => sum + (parseFloat(d.amount) || 0), 0);
      const totalDeduction = validDeposits.reduce((sum: number, d: unknown) => sum + (parseFloat(d.deduction) || 0), 0);
