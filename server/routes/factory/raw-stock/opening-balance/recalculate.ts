...
export function registerRawStockRecalculateUsedRoutes(app: Express) {
  ...
  app.post("/api/factory/raw-stock/recalculate-used", requireAuth, async (req: import("express").Request, res: import("express").Response) => {
    try {
  ...
  app.post("/api/factory/raw-stock/recalculate-bale-costs", requireAuth, async (req: import("express").Request, res: import("express").Response) => {
    try {
      if (!checkFactoryAdmin(req, res)) return;
      const companyId = req.session.factoryCompanyId || req.session.currentCompanyId;
      if (!companyId) return res.status(400).json({ message: "No company selected" });
