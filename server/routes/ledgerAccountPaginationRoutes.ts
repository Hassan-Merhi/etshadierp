...
export function registerLedgerAccountPaginationRoutes(app: Express): void {
  app.get("/api/ledger-accounts", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const conditions: any[] = [eq(ledgerAccounts.companyId, companyId), isNull(ledgerAccounts.deletedAt)];
      if (req.query.includeHidden !== "true") conditions.push(eq(ledgerAccounts.isHidden, false));
      ...
      const structured = wantsStructuredPagination(req);
      applyPaginationHeaders(res, total, page, limit, totalPages, !structured);

      if (!structured) return res.json(data);
      return res.json({
        items: data,
        total,
        page,
