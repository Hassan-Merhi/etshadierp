...
async function findOrCreateLedgerAccount(
  ...
  const [created] = await db.insert(ledgerAccounts).values({
    companyId, code: `${code}-${Date.now()}`, name, accountType: accountType as any,
    subType: subType ?? null, active: true,
  }).returning();
  return created.id;
}
...
export interface ApplySnapshot {
  ...
  orphanedVouchersDeleted: { id: number; voucherNumber: string; companyId: number; totalAmount: string; voucherType: string; voucherDate: string; description: string | null; entries: { ledgerAccountId: number | null; debitAmount: string; creditAmount: string; narration: string | null }[] }[];
  transfersDeleted: { id: number; transferType: string; fromCompanyId: number; toCompanyId: number; transferDate: string; amount: string; fromLedgerAccountId: number; toLedgerAccountId: number; fromVoucherId: number | null; toVoucherId: number | null; description: string | null; sourcePaymentId: number | null }[];
  depositSnapshots: { contractId: number; oldFlag: boolean; newFlag: boolean; oldPostedAmount: number; newPostedAmount: number }[];
}
  ...
  app.get(
    ...
    async (req: any, res: any) => {
      try {
        ...
        const allLedger = await db.select().from(propertyMonthlyLedger)
        ...
        const allUnits = await db.select({ id: propertyUnits.id, locationGroup: propertyUnits.locationGroup, unitNumber: propertyUnits.unitNumber, unitType: propertyUnits.unitType })
        ...
        const result: ScanResult = {
          ...
          totalDiscrepancies: ledgerDrifts.length + voucherEntryMissing.length + orphanedTransfers.length + depositFlagMismatches.length,
        };
        res.json(result);
      } catch (err: any) {
      ...
      try {
        ...
        for (const row of allLedger) {
          ...
          if (Math.abs(computed - stored) > 0.005) {
            ...
            await db.update(propertyMonthlyLedger).set({ paidAmount: computed.toFixed(2) }).where(eq(propertyMonthlyLedger.id, row.id));
          }
        }
        ...
        for (const row of (paymentsRows as any).rows ?? paymentsRows) {
          ...
          if (cnt === 0 && cashAccId) {
            ...
            if (isShop) {
              incomeOrExpenseId = await findOrCreateLedgerAccount(companyId, "Rent Expense - Shops", "Indirect Expense", "SHOP-RENT-EXP");
            } else {
              // Pick income account name matching the module
              const incomeAccName = module === "ERP" ? "Rent Income - ERP" : module === "FACTORY" ? "Rent Income - Factory" : "Rental Income";
              incomeOrExpenseId = await findOrCreateLedgerAccount(companyId, incomeAccName, "Income", "RENT-INC", "Indirect Income");
            }
            ...
            if (isShop) {
              ...
              inserted = await db.insert(voucherEntries).values([
                { voucherId: vid, ledgerAccountId: incomeOrExpenseId!, debitAmount: amtStr, creditAmount: "0", narration },
              ...
              inserted = await db.insert(voucherEntries).values([
                ...
                { voucherId: vid, ledgerAccountId: incomeOrExpenseId!,   debitAmount: "0",    creditAmount: amtStr, narration },
              ]).returning({ id: voucherEntries.id });
            }
    ...
    async (req: any, res: any) => {
      try {
        ...
        for (const s of snapshot.ledgerSnapshots ?? []) {
          await db.update(propertyMonthlyLedger).set({ paidAmount: s.oldPaid.toFixed(2) }).where(eq(propertyMonthlyLedger.id, s.id));
        }
