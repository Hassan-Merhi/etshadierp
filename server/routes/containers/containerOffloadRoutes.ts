...
export function registerContainerOffloadRoutes(app: Express) {
  ...
  app.get("/api/containers/:id/offload-charges", requireAuth, requireNonPOS, async (req: any, res: any) => {
    try {
      ...
      const allCharges = [
        { label: "Duties",           amount: parseFloat(offloadRow.duties         || "0"), accountName: dutiesAccount?.accountName  ?? null },
        { label: "Transport Fees",   amount: parseFloat(offloadRow.transportFees  || "0"), accountName: transAccount?.accountName   ?? null },
        { label: "Office Charges",   amount: parseFloat(offloadRow.officeCharges  || "0"), accountName: officeAccount?.accountName  ?? null },
