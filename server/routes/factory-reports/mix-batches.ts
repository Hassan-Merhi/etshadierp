...
export function registerFactoryMixBatchesByDateRoutes(app: Express, requireAuth: unknown, _db: unknown) {
  // ── Mix batches by date ───────────────────────────────────────────────────
  app.get("/api/factory/mix-batches-by-date", requireAuth, async (req: import("express").Request, res: import("express").Response) => {
    try {
      ...
      const enriched = batches.map((b) => {
        ...
        const totalCost = batchSources.reduce((sum: number, s: unknown) => sum + (parseFloat(s.total_cost) || 0), 0);
