...
async function validateStockItemRows(
  ...
  const [existingCodes, groups] = await Promise.all([
...
async function validateVoucherRows(
  ...
  return rows.map(row => {
    ...
    const debitAcct  = debitRef  ? (acctByCode.get(debitRef.toLowerCase())  ?? acctByName.get(debitRef.toLowerCase()))  : null;
    const creditAcct = creditRef ? (acctByCode.get(creditRef.toLowerCase()) ?? acctByName.get(creditRef.toLowerCase())) : null;
