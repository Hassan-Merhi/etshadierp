...
export function registerFactoryCashflowRoutes(app: Express, requireAuth: unknown, db: unknown) {
  app.get("/api/factory/cashflow", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const totalMonthlyPayroll = activeWorkers.reduce((s: number, w: unknown) => {
      ...
      const expectedIncome = pendingOrders.reduce((s: number, o: unknown) => s + parseFloat(o.grandTotal || "0"), 0);
