...
export function registerFactoryMixOptimizeRoutes(app: Express, requireAuth: unknown, db: unknown) {
  app.post("/api/factory/mix/optimize", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const mixBatchIds = Array.from(
        new Set(balesForProduct.map((b: unknown) => b.mixBatchId).filter(Boolean))
      ) as number[];
      ...
      if (mixBatchIds.length > 0) {
        ...
        const mixPerformance = mixes.map((mix: unknown) => {
          ...
          const mixBales = balesForProduct.filter((b: unknown) => b.mixBatchId === mix.id);
          ...
          const sourceRatios = mixSources.map((src: unknown) => ({
        ...
        mixPerformance.sort((a: unknown, b: unknown) => a.wastePct - b.wastePct);
        const top3 = mixPerformance.slice(0, 3);

        suggestions = top3.map((perf: unknown) => {
          const avgMaterialCost = perf.sourceRatios.reduce((s: number, r: unknown) => s + r.costPerKg * r.kgRatio, 0);
          ...
          return {
            sources: perf.sourceRatios.map((r: unknown) => ({
      ...
      if (suggestions.length === 0 && availableMaterials && availableMaterials.length > 0) {
        ...
        const avgCost =
          availableMaterials.reduce((s: number, m: unknown) => s + parseFloat(m.costPerKg || "0"), 0) /
        ...
        suggestions = [
          {
            sources: availableMaterials.map((m: unknown) => ({
