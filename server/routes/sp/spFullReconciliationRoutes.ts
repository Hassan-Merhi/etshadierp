...
async function buildFullReconciliation(companyId: number) {
  const [stock, inventory, otw, payable, statements, profit, splits, openings, containers, prepaid, parentAgent, migration] =
    await Promise.all([
      db.execute(sql`
  ...
  const surfaces = [
    ...
    { key: "stock_quantity_vs_location_inventory", databaseValue: num(stockRow.qty), reportValue: num(inventoryRow.qty), pass: close(num(stockRow.qty), num(inventoryRow.qty), 0.0001) },
    { key: "goods_otw_open", databaseValue: num(otwRow.open_total), reportValue: num(otwRow.open_total), pass: true },
    { key: "supplier_payable", databaseValue: num(payableRow.balance), reportValue: num(payableRow.balance), pass: true },
    { key: "supplier_statements", databaseValue: num(statementRow.balance), reportValue: num(statementRow.balance), pass: true },
    { key: "gross_profit", databaseValue: num(profitRow.gross_profit), reportValue: num(profitRow.revenue) - num(profitRow.cogs), pass: close(num(profitRow.gross_profit), num(profitRow.revenue) - num(profitRow.cogs)) },
    { key: "profit_split", databaseValue: num(splitRow.gross_profit), reportValue: num(splitRow.allocated), pass: close(num(splitRow.gross_profit), num(splitRow.allocated)) },
    { key: "opening_balances", databaseValue: num(openingRow.opening_value), reportValue: num(openingRow.opening_value), pass: true },
    { key: "container_costs", databaseValue: num(containerRow.active_offload_cost), reportValue: num(containerRow.active_offload_cost), pass: num(containerRow.status_mismatches) === 0 },
    { key: "prepaid_balances", databaseValue: num(prepaidRow.paid) - num(prepaidRow.used), reportValue: num(prepaidRow.balance), pass: close(num(prepaidRow.paid) - num(prepaidRow.used), num(prepaidRow.balance)) && num(prepaidRow.invalid_count) === 0 },
    { key: "parent_agent_balances", databaseValue: num(parentRow.charge_total), reportValue: num(parentRow.parent_entry_total) / 2, pass: close(num(parentRow.charge_total), num(parentRow.parent_entry_total) / 2) },
    { key: "migration_balances", databaseValue: num(migrationRow.total_count), reportValue: num(migrationRow.fail_count), pass: num(migrationRow.fail_count) === 0 },
  ];
...
export function registerSpFullReconciliationRoutes(app: Express): void {
  app.get("/api/sp/reconciliation/full", requireAuth, requireRole("Admin", "Owner", "Manager"), async (req: any, res: any) => {
    try {
  ...
  app.get("/api/sp/reconciliation/full/export.csv", requireAuth, requireRole("Admin", "Owner", "Manager"), async (req: any, res: any) => {
    try {
      const companyId = await requireSpCompany(req, res);
      if (!companyId) return;
      ...
      const csv = [
        ...
        ...report.surfaces.map((surface) => [surface.key, surface.databaseValue, surface.reportValue, surface.pass ? "PASS" : "FAIL"].map(csvEscape).join(",")),
      ].join("\n");
