...
const phase6RouteSource = readFileSync(
  new URL("./spGoldenCoastPhase6PosSaleRoutes.ts", import.meta.url),
  "utf8",
);
const phase6AutoHadiSource = readFileSync(
  new URL("./goldenCoastPhase6AutoHadi.ts", import.meta.url),
  "utf8",
);
const phase7RouteSource = readFileSync(
  new URL("./spGoldenCoastPhase7HadiTransferRoutes.ts", import.meta.url),
  "utf8",
);
const phase7ServiceSource = readFileSync(
  new URL(
    "../../services/accounting/goldenCoastPhase7HadiTransfer.ts",
    import.meta.url,
  ),
  "utf8",
);
const phase10RouteSource = readFileSync(
  new URL(
    "./spGoldenCoastPhase10SalesCashSettlementRoutes.ts",
    import.meta.url,
  ),
  "utf8",
);
...
const legacySalesGuardSource = readFileSync(
  new URL("./spGoldenCoastLegacySalesGuard.ts", import.meta.url),
  "utf8",
);
const posMutationsSource = readFileSync(
  new URL(
    "../../../client/src/pages/pos/hooks/usePosMutations.ts",
    import.meta.url,
  ),
  "utf8",
);
const spIndexSource = readFileSync(
...
describe("Golden Coast Phase 12 final accounting-model hardening", () => {
  ...
  it("atomically routes Phase 6 sale cash into the configured HADI company", () => {
    ...
    expect(phase6AutoHadiSource).toContain(
  ...
  it("routes the live Supplier Partner POS to Phase 6 when Golden Coast setup is present", () => {
    ...
    expect(posMutationsSource).toContain(
  ...
  it("fails closed against stale clients that try the generic SP sale route for Golden Coast", () => {
    expect(legacySalesGuardSource).toContain(
      "GC_PHASE6_CANONICAL_POS_REQUIRED",
    );
    expect(
      spIndexSource.indexOf("registerSpGoldenCoastLegacySalesGuard(app)"),
    ).toBeLessThan(spIndexSource.indexOf("registerSpSalesRoutes(app)"));
  });
  ...
  it("keeps retired Phase 1 and Phase 5 mutation registrars off the production composition root", () => {
    expect(spIndexSource).not.toContain(
      "registerSpGoldenCoastPhase1PostingRoutes(app)",
    );
    expect(spIndexSource).not.toContain(
      "registerSpGoldenCoastPhase5PosSaleRoutes(app)",
    );
    expect(spIndexSource).toContain(
