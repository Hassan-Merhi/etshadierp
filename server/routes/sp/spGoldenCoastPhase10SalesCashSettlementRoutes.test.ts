...
const accessControlSource = readFileSync(
  new URL("./spAccessControl.ts", import.meta.url),
  "utf8",
);
const serviceSource = readFileSync(
  new URL(
    "../../services/accounting/goldenCoastPhase10SalesCashSettlement.ts",
    import.meta.url,
  ),
  "utf8",
);
...
describe("Golden Coast Phase 10 GC Sales Cash settlement route surface", () => {
  it("registers after the existing Golden Coast production accounting routes and before legacy SP sales", () => {
    const phase8Index = spIndexSource.indexOf(
      "registerSpGoldenCoastPhase8ContainerOffloadRoutes(app);",
    );
    const phase10Index = spIndexSource.indexOf(
      "registerSpGoldenCoastPhase10SalesCashSettlementRoutes(app);",
    );
    const legacySalesIndex = spIndexSource.indexOf(
  ...
  it("inherits sp_sales_create permission and audit logging from the shared SP boundary", () => {
    expect(accessControlSource).toContain(
      'if ((path.includes("sales") || path.includes("sale")) && method !== "GET")',
    );
  ...
  it("resolves only one active canonical GC Sales Cash account in the selected company", () => {
    ...
    expect(routeSource).toContain(
      "getGoldenCoastAccountDefinition(PHASE10_ROLE)",
    );
    ...
    expect(routeSource).toContain(
      "definition.acceptedAccountTypes.includes(account.accountType)",
    );
  });
  ...
  it("computes the payable balance from opening semantics plus posted vouchers at the settlement date", () => {
    ...
    expect(routeSource).toContain(
      "SUM(CAST(ve.debit_amount AS numeric) - CAST(ve.credit_amount AS numeric))",
    );
    ...
    expect(routeSource).toContain(
      "COALESCE(v.effective_date, v.voucher_date) <=",
    );
  ...
  it("detects an exact replay before the mutable balance cap", () => {
    const digestIndex = routeSource.indexOf(
      "const settlementDigest = goldenCoastPhase10SettlementDigest",
    );
    const replayIndex = routeSource.indexOf(
      "const replayed = await findReplayedSettlement",
    );
    const lockIndex = routeSource.indexOf(
      "LOCK TABLE voucher_entries IN SHARE ROW EXCLUSIVE MODE",
    );
    const datedBalanceIndex = routeSource.indexOf(
      "const datedBalanceUsd = await gcSalesCashDebitBalance",
    );
    const allPostedBalanceIndex = routeSource.indexOf(
