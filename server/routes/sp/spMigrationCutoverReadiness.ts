...
async function getCompletedActions(sourceId: number, targetId: number): Promise<Set<string>> {
  ...
  `);
  return new Set(resultRows(result).map((row: unknown) => String(row.action)));
}
...
export async function buildCutoverReadiness(sourceId: number, targetId: number): Promise<unknown> {
  ...
  if (missingAccounts.length) {
    blockers.push({
      ...
      message: `Target is missing required accounts: ${missingAccounts.map((row: unknown) => row.sub_type).join(", ")}.`,
