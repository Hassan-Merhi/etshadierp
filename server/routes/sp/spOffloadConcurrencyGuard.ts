...
async function guardSpOffload(req: Request, res: Response, next: NextFunction): Promise<void> {
  ...
  try {
    ...
    const requestedLandedTotal = requestedCharges.reduce(
      (sum: number, charge: unknown) => sum + (Number.isFinite(Number(charge?.amountUsd)) ? Number(charge.amountUsd) : 0),
      0
    );
    ...
    if (requestedCharges.some((charge: unknown) => charge?.chargeType === "prepaid_used")) {
      requiredSubTypes.push("sp_prepaid");
    }
    if (requestedCharges.some((charge: unknown) => charge?.chargeType === "parent_agent")) {
    ...
    const bankIds = uniquePositiveIds(
      requestedCharges
        .filter((charge: unknown) => charge?.chargeType === "paid_now")
        .map((charge: unknown) => charge?.creditBankAccountId)
    );
    if (bankIds.length > 0) {
    ...
    const ledgerIds = uniquePositiveIds(
      requestedCharges
        .filter((charge: unknown) => charge?.chargeType === "unpaid_payable" || charge?.chargeType === "other")
        .map((charge: unknown) => charge?.creditLedgerAccountId)
    );
    ...
    const parentAgentIds = uniquePositiveIds(
      requestedCharges
        .filter((charge: unknown) => charge?.chargeType === "parent_agent")
        .map((charge: unknown) => charge?.parentAgentAccountId)
    );
