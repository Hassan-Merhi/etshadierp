...
function selectedCompanyId(req: Request): number {
  ...
  if (!Number.isInteger(companyId) || companyId <= 0) throw new GoldenCoastPhase1InputError("No company selected");
  return companyId;
}
...
async function validatePhase1LedgerRolesTx(
  ...
  for (const requirement of requirements) {
    ...
    if (!account) throw new GoldenCoastPhase1InputError(`${requirement.label} is not an active ledger account in this company`);
...
function accountOverrides(value: unknown): Map<string, { code?: string; name?: string }> {
  ...
  for (const raw of value) {
    if (!raw || typeof raw !== "object" || Array.isArray(raw)) throw new GoldenCoastPhase1InputError("Each account override must be an object");
    const input = raw as Record<string, unknown>;
    const subType = typeof input.subType === "string" ? input.subType.trim() : "";
    if (!subType || !allowed.has(subType)) throw new GoldenCoastPhase1InputError(`Unknown Phase 1 account subType: ${subType || "(missing)"}`);
    ...
    const result = await db.transaction(async (tx) => {
      await tx
        ...
        .where(
          and(
            ...
            eq(ledgerAccounts.active, false),
            isNull(ledgerAccounts.deletedAt)
          )
        );

      const existingRows = await tx
        .select({ id: ledgerAccounts.id, subType: ledgerAccounts.subType, code: ledgerAccounts.code, name: ledgerAccounts.name })
        .from(ledgerAccounts)
        .where(
          and(
            eq(ledgerAccounts.companyId, companyId),
      ...
      for (const definition of GOLDEN_COAST_PHASE1_ACCOUNT_DEFS) {
        ...
        const [account] = await tx
          ...
          .values({ companyId, code: override?.code ?? definition.code, name: override?.name ?? definition.name, accountType: definition.accountType, subType: definition.subType, active: true })
          .returning({ id: ledgerAccounts.id, subType: ledgerAccounts.subType, code: ledgerAccounts.code, name: ledgerAccounts.name });
        created.push({ id: Number(account.id), subType: String(account.subType), code: account.code, name: account.name });
      }
  ...
  try {
    ...
    const result = await db.transaction(async (tx) => {
      const built = buildGoldenCoastPhase1PostingBatch({
        ...
        actor: { userId: userId ?? null, username: req.session.username || "unknown", reason: "Golden Coast Phase 1 accounting" },
      });
      ...
      return { postings, clientRequestId: built.clientRequestId, eventType: built.eventType };
    });
    ...
    for (const item of result.postings) {
      ...
      try {
        ...
        logger.error("Golden Coast Phase 1 compatibility audit failed (non-fatal)", { companyId: selectedCompany, voucherId: item.posted.voucher.id, role: item.role, error: auditError });
      }
    }
    ...
    res.json({
      ...
      postings: result.postings.map((item) => ({ role: item.role, voucher: item.posted.voucher, entries: item.posted.entries, replayed: item.posted.replayed })),
    });
  } catch (error: unknown) {
    logger.error("Golden Coast Phase 1 posting failed", { module: "golden-coast-accounting", companyId, userId, durationMs: Date.now() - startedAt, error });
    if (error instanceof GoldenCoastPhase1InputError) {
      res.status(400).json({ message: error.message, code: "GC_PHASE1_INPUT_INVALID" });
      return;
    }
...
async function handlePhase1Accounts(req: Request, res: Response): Promise<void> {
  try {
    ...
    const [phase1Ledgers, cashLedgers, banks, companyLocations] = await Promise.all([
      db.select({ id: ledgerAccounts.id, code: ledgerAccounts.code, name: ledgerAccounts.name, accountType: ledgerAccounts.accountType, subType: ledgerAccounts.subType })
        .from(ledgerAccounts)
        .where(and(eq(ledgerAccounts.companyId, companyId), eq(ledgerAccounts.active, true), isNull(ledgerAccounts.deletedAt), inArray(ledgerAccounts.subType, GOLDEN_COAST_PHASE1_LEDGER_SUBTYPES))),
      db.select({ id: ledgerAccounts.id, code: ledgerAccounts.code, name: ledgerAccounts.name, accountType: ledgerAccounts.accountType, subType: ledgerAccounts.subType })
        .from(ledgerAccounts)
        .where(and(eq(ledgerAccounts.companyId, companyId), eq(ledgerAccounts.active, true), isNull(ledgerAccounts.deletedAt), inArray(ledgerAccounts.accountType, ["Cash", "Bank"]))),
      db.select({ id: bankAccounts.id, code: bankAccounts.code, name: bankAccounts.name, bankName: bankAccounts.bankName })
        .from(bankAccounts)
        .where(and(eq(bankAccounts.companyId, companyId), eq(bankAccounts.active, true), isNull(bankAccounts.deletedAt))),
      db.select({ id: locations.id, code: locations.code, name: locations.name })
        .from(locations)
    ...
    const missingRequiredSubTypes = GOLDEN_COAST_PHASE1_LEDGER_SUBTYPES.filter((subType) => !foundSubTypes.has(subType));
    res.json({ ledgerAccounts: phase1Ledgers, cashLedgerAccounts: cashLedgers, bankAccounts: banks, locations: companyLocations, missingRequiredSubTypes });
  } catch (error: unknown) {
    if (error instanceof GoldenCoastPhase1InputError) {
      res.status(400).json({ message: error.message, code: "GC_PHASE1_INPUT_INVALID" });
      return;
...
export function registerGoldenCoastAccountingRoutes(app: Express): void {
  app.post("/api/golden-coast/accounting/phase1/setup-accounts", requireAuth, requireRole("Developer"), (req, res) => void handlePhase1SetupAccounts(req, res));
  app.get("/api/golden-coast/accounting/phase1/accounts", requireAuth, requireNonPOS, (req, res) => void handlePhase1Accounts(req, res));
  app.post("/api/golden-coast/accounting/phase1/preview", requireAuth, requireNonPOS, (req, res) => void handlePhase1Preview(req, res));
  app.post("/api/golden-coast/accounting/phase1/post", requireAuth, requireNonPOS, (req, res) => void handlePhase1Post(req, res));
}
