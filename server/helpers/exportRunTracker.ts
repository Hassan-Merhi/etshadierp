...
export async function createExportRun(
  ...
  try {
    const r = await pool.query(
      `INSERT INTO daily_export_runs (run_type, status) VALUES ($1, 'running') RETURNING id`,
      [runType],
    );
 ...
 */
export async function updateExportRun(
  id: number,
  ...
  try {
    await pool.query(
      `UPDATE daily_export_runs SET ${setClauses.join(", ")} WHERE id = $${values.length}`,
      values,
    );
  } catch (err: any) {
 ...
 */
export async function finishExportRun(
  id: number,
  data: Record<string, any>,
): Promise<void> {
