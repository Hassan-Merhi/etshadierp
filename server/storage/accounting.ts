...
import type {
  LedgerAccount, InsertLedgerAccount, BankAccount, InsertBankAccount,
  FixedAsset, InsertFixedAsset, Voucher, InsertVoucher, VoucherEntry, InsertVoucherEntry,
} from "@shared/schema";
...
export async function getAllLedgerAccounts(companyId: number, includeHidden: boolean = false): Promise<LedgerAccount[]> {
  const conditions = [
...
export async function updateBankAccount(id: number, updates: Partial<InsertBankAccount>, companyId: number): Promise<BankAccount> {
...
export async function getStockItemTransactions(stockItemId: number, companyId: number, startDate?: string, endDate?: string): Promise<any[]> {
  ...
  await db.transaction(async (tx) => {
    ...
    if (voucher.voucherType === "Sales" && voucher.locationId) {
      ...
      for (const saleItem of salesItemsList) {
        ...
        await adjustInventory(tx, voucher.locationId, saleItem.stockItemId, quantity, voucher.companyId, costPrice, "Sales-Reversal", id);
      }
    ...
    if (voucher.voucherType === "Stock Transfer") {
      ...
      if (transferVoucher) {
        ...
        for (const item of transferItems) {
          ...
          if (!sourceLocationId) throw new Error(`Cannot reverse stock transfer: source location ID is missing for transfer voucher ${id}`);
          if (!destinationLocationId) throw new Error(`Cannot reverse stock transfer: destination location ID is missing for transfer voucher ${id}`);

          await adjustInventory(tx, sourceLocationId, item.stockItemId, quantity, voucher.companyId, rate, "StockTransfer-Reversal", id);
          await adjustInventory(tx, destinationLocationId, item.stockItemId, -quantity, voucher.companyId, rate, "StockTransfer-Reversal", id);
        }
    ...
    if (voucher.voucherType === "Production" || voucher.voucherType === "Consumption" || voucher.voucherType === "Mixed" || voucher.voucherType === "Stock Adjustment") {
      ...
      if (adjustmentVoucher) {
        ...
        for (const item of adjustmentItems) {
          ...
          await adjustInventory(tx, adjustmentVoucher.locationId, item.stockItemId, reversedQuantity, voucher.companyId, rate, `${adjustmentType}-Reversal`, id);
        }
    ...
    if (linkedPOs.length > 0) {
      ...
      for (const [containerId, totals] of Array.from(containerUpdates.entries())) {
        ...
        if (container) {
          ...
          const remainingPOs = await tx.select().from(schema.purchaseOrders).where(eq(schema.purchaseOrders.containerId, containerId)).limit(1);
          if (remainingPOs.length === 0) {
            ...
            await tx.update(schema.containers).set({
...
export async function closeFiscalPeriod(
  ...
  return await db.transaction(async (tx) => {
    ...
    interface AccountBalance { accountId: number; accountCode: string; accountName: string; accountType: string; balance: number; }
    ...
    for (const account of accounts) {
      ...
      if (account.accountType === "Income") {
        totalIncome += -balance;
        accountBalances.push({ accountId: account.id, accountCode: account.code, accountName: account.name, accountType: account.accountType, balance: -balance });
      } else {
        totalExpense += balance;
        accountBalances.push({ accountId: account.id, accountCode: account.code, accountName: account.name, accountType: account.accountType, balance });
      }
    }
    ...
    const [closingVoucher] = await tx.insert(schema.vouchers).values({
      companyId, voucherNumber, voucherType: "Journal", voucherDate: periodEndDate,
      description: `Fiscal Period Close: ${periodStartDate} to ${periodEndDate}${notes ? ` - ${notes}` : ''}`,
      totalAmount: Math.abs(netIncome).toFixed(2), optional: false,
    }).returning();

    for (const account of accountBalances) {
      if (account.balance === 0) continue;
      if (account.accountType === "Income") {
        await tx.insert(schema.voucherEntries).values({ voucherId: closingVoucher.id, ledgerAccountId: account.accountId, debitAmount: account.balance.toFixed(2), creditAmount: "0", narration: `Close ${account.accountName} for period ending ${periodEndDate}` });
      } else {
        await tx.insert(schema.voucherEntries).values({ voucherId: closingVoucher.id, ledgerAccountId: account.accountId, debitAmount: "0", creditAmount: account.balance.toFixed(2), narration: `Close ${account.accountName} for period ending ${periodEndDate}` });
      }
    }

    if (netIncome !== 0) {
      if (netIncome > 0) {
        await tx.insert(schema.voucherEntries).values({ voucherId: closingVoucher.id, ledgerAccountId: retainedEarningsAccountId, debitAmount: "0", creditAmount: netIncome.toFixed(2), narration: `Net Income for period ending ${periodEndDate}` });
      } else {
        await tx.insert(schema.voucherEntries).values({ voucherId: closingVoucher.id, ledgerAccountId: retainedEarningsAccountId, debitAmount: Math.abs(netIncome).toFixed(2), creditAmount: "0", narration: `Net Loss for period ending ${periodEndDate}` });
      }
    }

    const [closure] = await tx.insert(schema.fiscalPeriodClosures).values({
      companyId, periodStartDate, periodEndDate, closedByUserId, closingVoucherId: closingVoucher.id,
      retainedEarningsAccountId, totalIncome: totalIncome.toFixed(2), totalExpense: totalExpense.toFixed(2),
      netIncome: netIncome.toFixed(2), status: "CLOSED", notes: notes || null,
    }).returning();

    for (const account of accountBalances) {
      await tx.update(schema.ledgerAccounts).set({ openingBalance: "0", openingBalanceSide: "Dr" }).where(eq(schema.ledgerAccounts.id, account.accountId));
    }
...
export async function getExchangeRates(companyId: number): Promise<schema.ExchangeRate[]> {
  return await db.select().from(schema.exchangeRates).where(eq(schema.exchangeRates.companyId, companyId)).orderBy(sql`${schema.exchangeRates.effectiveDate} DESC`);
}

export async function getLatestExchangeRate(companyId: number, fromCurrency: string, toCurrency: string): Promise<schema.ExchangeRate | undefined> {
  const results = await db.select().from(schema.exchangeRates).where(and(
  ...
  )).orderBy(sql`${schema.exchangeRates.effectiveDate} DESC`).limit(1);
  return results[0];
}

export async function getExchangeRateForDate(companyId: number, fromCurrency: string, toCurrency: string, date: string): Promise<schema.ExchangeRate | undefined> {
  const results = await db.select().from(schema.exchangeRates).where(and(
    eq(schema.exchangeRates.companyId, companyId),
    eq(schema.exchangeRates.fromCurrency, fromCurrency),
    eq(schema.exchangeRates.toCurrency, toCurrency),
...
export async function getAllCustomers(companyId: number, search?: string, limit?: number): Promise<schema.Customer[]> {
  ...
  let query = db.select().from(schema.customers).where(and(...conditions)).orderBy(schema.customers.legalName) as any;
...
export async function getCustomerByCode(code: string, companyId: number): Promise<schema.Customer | undefined> {
  const [customer] = await db.select().from(schema.customers).where(and(eq(schema.customers.code, code), eq(schema.customers.companyId, companyId)));
  return customer;
}
...
export async function getAllInterCompanyTransfers(companyId?: number): Promise<schema.InterCompanyTransfer[]> {
  if (companyId) {
    return await db.select().from(schema.interCompanyTransfers)
      .where(or(eq(schema.interCompanyTransfers.fromCompanyId, companyId), eq(schema.interCompanyTransfers.toCompanyId, companyId)))
      .orderBy(sql`${schema.interCompanyTransfers.transferDate} DESC`);
  }
  return await db.select().from(schema.interCompanyTransfers).orderBy(sql`${schema.interCompanyTransfers.transferDate} DESC`);
}

export async function getInterCompanyTransferById(id: number): Promise<schema.InterCompanyTransfer | undefined> {
  const [transfer] = await db.select().from(schema.interCompanyTransfers).where(eq(schema.interCompanyTransfers.id, id));
  return transfer;
}

export async function createInterCompanyTransfer(transfer: schema.InsertInterCompanyTransfer): Promise<schema.InterCompanyTransfer> {
...
export async function getCompanySettings(companyId: number): Promise<schema.CompanySettings | undefined> {
  const [settings] = await db.select().from(schema.companySettings).where(eq(schema.companySettings.companyId, companyId));
  return settings;
}
...
export async function addCustomerBalanceEntry(entry: schema.InsertCustomerBalance): Promise<schema.CustomerBalance> {
  ...
  const [latestBalance] = await db
    ...
    .where(and(eq(schema.customerBalances.customerId, entry.customerId), eq(schema.customerBalances.companyId, entry.companyId)))
...
export async function getCustomerStatement(
  ...
  const conditions = [eq(schema.customerBalances.customerId, customerId), eq(schema.customerBalances.companyId, companyId)];
  ...
  return await db.select().from(schema.customerBalances).where(and(...conditions)).orderBy(schema.customerBalances.transactionDate);
}
...
export async function getRoleFeaturePermissions(companyId: number): Promise<schema.RoleFeaturePermission[]> {
  return await db.select().from(schema.roleFeaturePermissions).where(eq(schema.roleFeaturePermissions.companyId, companyId));
}
...
export async function getRoleFeaturePermission(companyId: number, role: string, featureKey: string): Promise<schema.RoleFeaturePermission | undefined> {
  const [permission] = await db.select().from(schema.roleFeaturePermissions).where(and(
...
export async function upsertRoleFeaturePermission(permission: schema.InsertRoleFeaturePermission): Promise<schema.RoleFeaturePermission> {
  const [result] = await db
    ...
    .values(permission)
    .onConflictDoUpdate({
      target: [schema.roleFeaturePermissions.companyId, schema.roleFeaturePermissions.role, schema.roleFeaturePermissions.featureKey],
...
export async function getErpUserPageAccess(companyId: number, userId: string): Promise<string[]> {
  const rows = await db.select({ pageKey: schema.erpUserPageAccess.pageKey })
...
export async function setErpUserPageAccess(companyId: number, userId: string, pageKeys: string[]): Promise<void> {
  await db.delete(schema.erpUserPageAccess).where(and(eq(schema.erpUserPageAccess.companyId, companyId), eq(schema.erpUserPageAccess.userId, userId)));
...
export async function getErpUserHiddenCostFields(userId: string): Promise<string[]> {
  const [user] = await db.select({ hiddenErpCostFields: schema.users.hiddenErpCostFields }).from(schema.users).where(eq(schema.users.id, userId));
...
export async function setSystemSetting(key: string, value: string | null): Promise<schema.SystemSetting> {
  ...
  if (existing) {
    const [updated] = await db.update(schema.systemSettings).set({ value, updatedAt: sql`now()` }).where(eq(schema.systemSettings.key, key)).returning();
    return updated;
  } else {
...
export async function listSpreadsheets(companyId: number): Promise<Pick<schema.Spreadsheet, "id" | "name" | "createdBy" | "updatedAt">[]> {
  return db.select({ id: schema.spreadsheets.id, name: schema.spreadsheets.name, createdBy: schema.spreadsheets.createdBy, updatedAt: schema.spreadsheets.updatedAt })
...
export async function getSpreadsheet(id: number, companyId: number): Promise<schema.Spreadsheet | undefined> {
  const [row] = await db.select().from(schema.spreadsheets).where(and(eq(schema.spreadsheets.id, id), eq(schema.spreadsheets.companyId, companyId)));
  return row;
}

export async function createSpreadsheet(companyId: number, name: string, data: any, createdBy?: string): Promise<schema.Spreadsheet> {
  const [row] = await db.insert(schema.spreadsheets).values({ companyId, name, data, createdBy: createdBy ?? null }).returning();
  return row;
}

export async function updateSpreadsheet(id: number, companyId: number, fields: { name?: string; data?: any }): Promise<schema.Spreadsheet | undefined> {
  const [row] = await db.update(schema.spreadsheets).set({ ...fields, updatedAt: sql`now()` }).where(and(eq(schema.spreadsheets.id, id), eq(schema.spreadsheets.companyId, companyId))).returning();
  return row;
}

export async function deleteSpreadsheet(id: number, companyId: number): Promise<void> {
  await db.delete(schema.spreadsheets).where(and(eq(schema.spreadsheets.id, id), eq(schema.spreadsheets.companyId, companyId)));
}
...
export async function getLiveSpreadsheetById(id: number, companyId: number): Promise<schema.LiveSpreadsheet | undefined> {
  const [row] = await db.select().from(schema.liveSpreadsheets).where(and(eq(schema.liveSpreadsheets.id, id), eq(schema.liveSpreadsheets.companyId, companyId)));
  return row;
}
...
export async function updateLiveSpreadsheet(id: number, companyId: number, fields: Partial<schema.InsertLiveSpreadsheet>): Promise<schema.LiveSpreadsheet | undefined> {
  const [row] = await db.update(schema.liveSpreadsheets).set({ ...fields, updatedAt: sql`now()` }).where(and(eq(schema.liveSpreadsheets.id, id), eq(schema.liveSpreadsheets.companyId, companyId))).returning();
  return row;
}

export async function deleteLiveSpreadsheet(id: number, companyId: number): Promise<void> {
  await db.delete(schema.liveSpreadsheets).where(and(eq(schema.liveSpreadsheets.id, id), eq(schema.liveSpreadsheets.companyId, companyId)));
}
