...
function collectCandidates(file, text) {
  ...
  for (let match = suppression.exec(text); match; match = suppression.exec(text)) candidates.push({ start: match.index, end: match.index + match[0].length, repl: "@ts-note", label: "remove-suppression" });
...
function tryGroup(file, current, group, stats) {
  if (!group.length) return true;
  const trial = applyEdits(current.text, group);
  setText(file, trial);
  stats.checks += 1;
  if (diagnostics(file).length === 0) {
    current.text = trial;
    stats.accepted += group.length;
    for (const edit of group) stats.labels[edit.label] = (stats.labels[edit.label] || 0) + 1;
    return true;
  }
  setText(file, current.text);
  return false;
}
function solveShallow(file, current, group, stats, depth, maxDepth) {
  if (!group.length) return false;
  if (tryGroup(file, current, group, stats)) return true;
  if (depth >= maxDepth || group.length === 1) {
    stats.rejected += group.length;
    return false;
  }
  const mid = Math.ceil(group.length / 2);
  const right = solveShallow(file, current, group.slice(0, mid), stats, depth + 1, maxDepth);
  const left = solveShallow(file, current, group.slice(mid), stats, depth + 1, maxDepth);
  return right || left;
}
const priority = ["infer-variable", "infer-property", "infer-return", "infer-parameter", "remove-cast", "infer-call-typeargs", "remove-suppression", "unknown"];
const stats = { files: 0, accepted: 0, rejected: 0, checks: 0, labels: {} };
for (const file of files) {
  const original = fs.readFileSync(file, "utf8");
  const current = { text: original };
  const acceptedAtStart = stats.accepted;
  for (let round = 0; round < 6; round += 1) {
    let candidates = collectCandidates(file, current.text);
    if (!candidates.length) break;
    const acceptedBefore = stats.accepted;
    if (tryGroup(file, current, candidates, stats)) continue;
    let progressed = false;
    for (const label of priority) {
      candidates = collectCandidates(file, current.text).filter((candidate) => candidate.label === label);
      if (!candidates.length) continue;
      const chunkSize = label === "unknown" ? 24 : 32;
      const maxDepth = label === "unknown" ? 1 : 4;
      for (let i = 0; i < candidates.length; i += chunkSize) {
        if (solveShallow(file, current, candidates.slice(i, i + chunkSize), stats, 0, maxDepth)) progressed = true;
      }
      if (progressed) break;
    }
    if (!progressed || stats.accepted === acceptedBefore) break;
  }
  if (stats.accepted > acceptedAtStart) {
    setText(file, current.text);
    if (diagnostics(file).length === 0) {
      fs.writeFileSync(file, current.text);
      stats.files += 1;
      console.log(`SOLVED ${path.relative(root, file).replaceAll("\\", "/")} ${stats.accepted - acceptedAtStart}`);
