...
requireText(companyContext, "queryKey: companyDataKey(url, companyId)", "reference prefetch is not company scoped");
requireText(companyContext, "...stableReferenceQueryPolicy", "reference prefetch does not use the stable policy");
...
if (failures.length) {
  console.error("Group A Phase 2 contract failed:");
  ...
  JSON.stringify(
    {
      ...
      phase: 2,
      status: "complete",
      idlePolling: "visible-tab-only essential counts and presence",
      sharedReferenceCache: true,
