...
for (const token of [
  'currentLocation === "/factory/pos"',
  'aria-label={isFactoryPosRoute ? "Factory point of sale workspace"',
  'data-pos-workspace={isFactoryPosRoute ? "true" : undefined}',
  "factoryPosWorkspaceClasses",
  "touch-manipulation",
  "[&_input]:min-h-10",
  "[&_table]:min-w-max",
  "Loading factory point of sale",
]) {
  if (!factoryShell.includes(token)) failures.push(`Factory POS adoption contract missing: ${token}`);
}
...
console.log(JSON.stringify({ phase: "7E", status: "ok", protectedContracts: 38 }, null, 2));
