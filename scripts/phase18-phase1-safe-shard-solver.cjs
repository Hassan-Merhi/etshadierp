...
const shardIndex = Number(process.env.PHASE1_SHARD_INDEX ?? 0);
const shardTotal = Number(process.env.PHASE1_SHARD_TOTAL ?? 1);
...
const targetFiles = allTargetFiles.filter((_, index) => index % shardTotal === shardIndex);
...
for (const file of targetFiles) {
  ...
  if (stats.accepted>before && current.text!==original) {
    ...
    if (diagnostics(file).length===0) {
      ...
      stats.files += 1;
      console.log(`PHASE1_SAFE_FILE shard=${shardIndex} ${path.relative(root,file).replaceAll('\\','/')} accepted=${stats.accepted-before}`);
