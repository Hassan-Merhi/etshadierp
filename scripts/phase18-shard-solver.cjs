...
const shardIndex = Number(process.env.PHASE18_SHARD_INDEX ?? 0);
const shardTotal = Number(process.env.PHASE18_SHARD_TOTAL ?? 1);
const configPath = ts.findConfigFile(root, ts.sys.fileExists, "tsconfig.json");
if (!configPath) throw new Error("tsconfig.json not found");
const configFile = ts.readConfigFile(configPath, ts.sys.readFile);
const parsed = ts.parseJsonConfigFileContent(configFile.config, ts.sys, path.dirname(configPath));
const allTargetFiles = parsed.fileNames.filter((file) => {
  const rel = path.relative(root, file).replaceAll("\\", "/");
  return /^(client\/src|server|shared)\//.test(rel) && /\.(ts|tsx)$/.test(rel) && !rel.endsWith(".d.ts") && fs.existsSync(file);
}).sort();
const files = allTargetFiles.filter((_, index) => index % shardTotal === shardIndex);
...
function setText(file, text) {
  ...
  entry.text = text; entry.version += 1; state.set(key, entry);
}
...
function annotationStart(source, typeNode) {
  let i = typeNode.getStart() - 1; while (i >= 0 && /\s/.test(source[i])) i -= 1; return source[i] === ":" ? i : null;
}
function ownerCandidate(node, sourceFile) {
  let typeNode = node;
  for (let depth = 0; depth < 6 && typeNode.parent; depth += 1) {
    const owner = typeNode.parent;
    if (owner.type === typeNode) {
      const start = annotationStart(sourceFile.text, typeNode); if (start === null) return null;
...
function callCandidate(node, sourceFile) {
  let current = node;
  for (let depth = 0; depth < 5 && current.parent; depth += 1) {
    ...
    if ((ts.isCallExpression(parent) || ts.isNewExpression(parent)) && parent.typeArguments?.length === 1 && contains(parent.typeArguments[0], node)) {
      const start = parent.typeArguments.pos - 1, end = parent.typeArguments.end + 1;
...
function candidate(node, sourceFile) {
  ...
  return ownerCandidate(node, sourceFile) || callCandidate(node, sourceFile) || { start: node.getStart(sourceFile), end: node.end, repl: "unknown", label: "unknown" };
}
function collect(file, text) {
  const sf = ts.createSourceFile(file, text, ts.ScriptTarget.Latest, true, file.endsWith("x") ? ts.ScriptKind.TSX : ts.ScriptKind.TS);
  const edits = [];
  const visit = (node) => { if (node.kind === ts.SyntaxKind.AnyKeyword) edits.push(candidate(node, sf)); ts.forEachChild(node, visit); };
  visit(sf);
  const suppression = /@ts-(?:ignore|expect-error)\b/g;
  for (let m = suppression.exec(text); m; m = suppression.exec(text)) edits.push({ start: m.index, end: m.index + m[0].length, repl: "@ts-note", label: "remove-suppression" });
  const unique = new Map();
  for (const edit of edits) { const key = `${edit.start}:${edit.end}:${edit.repl}`; if (!unique.has(key)) unique.set(key, edit); }
...
function tryGroup(file, current, edits, stats) {
...
function splitSolve(file, current, edits, stats, depth=0) {
  if (!edits.length) return false;
  if (tryGroup(file,current,edits,stats)) return true;
  if (edits.length === 1 || depth >= 5) { stats.rejected += edits.length; return false; }
...
const priorities=["infer-variable","infer-property","infer-return","infer-parameter","remove-cast","infer-call-typeargs","remove-suppression","unknown"];
const stats={shard:shardIndex,total:shardTotal,files:0,accepted:0,rejected:0,checks:0,labels:{}};
for (const file of files) {
  const original=fs.readFileSync(file,"utf8");
  if (!/\bany\b|@ts-(?:ignore|expect-error)\b/.test(original)) continue;
  const current={text:original}; const startAccepted=stats.accepted;
  for (let round=0;round<5;round+=1) {
    let edits=collect(file,current.text); if (!edits.length) break;
    const before=stats.accepted;
    if (tryGroup(file,current,edits,stats)) continue;
    let progressed=false;
    for (const label of priorities) {
      edits=collect(file,current.text).filter(e=>e.label===label); if (!edits.length) continue;
      const max = label === "unknown" ? 16 : 32;
      for (let i=0;i<edits.length;i+=max) if (splitSolve(file,current,edits.slice(i,i+max),stats)) progressed=true;
      if (progressed) break;
    }
    if (!progressed || stats.accepted===before) break;
  }
  if (stats.accepted>startAccepted) {
    setText(file,current.text);
    if (diagnostics(file).length===0) { fs.writeFileSync(file,current.text); stats.files+=1; console.log(`SHARD_SOLVED ${shardIndex} ${path.relative(root,file).replaceAll("\\","/")} ${stats.accepted-startAccepted}`); }
    else setText(file,original);
  }
}
console.log(`SHARD_RESULT ${JSON.stringify(stats)}`);
