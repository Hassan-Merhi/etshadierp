...
const responsivePath = "client/src/components/ui/responsive-accessibility.tsx";
const primitive = await read(primitivePath);
const pageState = await read(pageStatePath);
const responsive = await read(responsivePath);

requireMatch(primitive, /export function FinancialScreen\(/, "shared financial page container exists");
requireMatch(primitive, /id=\{Comp === "main" \? "main-content"/, "financial page container provides the skip-link target");
...
requireMatch(primitive, /sm:flex-row/, "financial headers and status rows retain responsive stacking");
...
requireMatch(primitive, /aria-labelledby/, "financial headings label their regions");
