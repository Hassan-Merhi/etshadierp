...
-   **Factory Data Import**: Bulk import tool for opening balances via CSV/Excel upload or manual entry. Supports three import types: Factory Supplier Balances (creates/updates suppliers with opening balance), Raw Stock Inventory (creates containers and raw stock records), and Bales Inventory (creates bales with auto-generated reference numbers). Available under Factory Production > Data > Import Data sidebar section. Factory suppliers have an `openingBalance` field.
-   **Factory Mode Expansion**: Factory Mode now includes the same major modules as ERP Mode: Dashboard, Daybook, Accounts, Vouchers, Create, Payroll, Analytics, Location Inventory, Stock OTW, Stock Query, Import Data, and Settings. Accounting data is SHARED between ERP and Factory modes (same tables, same endpoints). Factory Mode reuses ERP page components at `/factory/*` routes. A `sourceModule` field on vouchers (`ERP`|`FACTORY`, default `ERP`) allows tagging factory-created vouchers while still showing all transactions. Factory operational data (suppliers, containers, bales) remains isolated. The `FactorySidebar` component manages navigation with admin-only filtering for Settings.
-   **Factory Domain Isolation**: The Factory production system is completely isolated from ERP with its own database tables (`factory_*`), API namespace (`/api/factory/*`), and UI pages. Factory has independent master data: `factory_suppliers`, `factory_categories`, `factory_bale_products`, `factory_containers`, `factory_raw_stock`, `factory_mix_batches`, `factory_mix_batch_sources`, `factory_pressing_batches`, `factory_bales`, `factory_bale_sequences`. Integration with ERP occurs only at the finalize step where users select an ERP warehouse location (`erpLocationId`). Multi-product pressing: `POST /api/factory/pressing/create-multi` accepts an array of `{productId, qty, weightPerBale}` and creates ONE pressing batch with all bales. Partial finalization: the finalize endpoint accepts fewer scanned bales than expected, sets batch status to `PARTIALLY_FINALIZED`, and returns `missingBales` array. Unscanned bales remain `PENDING_PRESSING` and can be finalized later. The finalize/counting page uses expandable batch cards with a scan-to-verify flow showing all bales with green checkmarks for scanned ones, and a validation dialog that warns about missing bales but allows proceeding. All factory routes are in `server/factoryRoutes.ts` and registered via `registerFactoryRoutes()`. Factory UI is under the FactoryProduction page with sidebar navigation including Master Data (Suppliers, Containers, Bale Products), Raw Materials, Production, Logistics, Analytics, and Traceability sections.
