...
| ESLint warnings | 9,453 total | `npm run lint` |
...
- **`totals.warningCeiling`** is the repository total, currently 9,453, and may
...
- **`perRule`** freezes each rule at its own count, checked by
  `npm run audit:lint-ratchet`. This is the part that matters: 8,827 of the
  9,453 warnings are `no-explicit-any`, so a total-only gate is a count of
