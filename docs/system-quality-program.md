...
| Signal | Now | Command |
|---|---|---|
| Type escapes (AST) | 8,832 total | `npm run audit:type-escapes` |
| ESLint warnings | 9,461 total | `npm run lint` |
...
- **`totals.warningCeiling`** is the repository total, currently 9,461, and may
...
- **`perRule`** freezes each rule at its own count, checked by
  `npm run audit:lint-ratchet`. This is the part that matters: 8,832 of the
  9,461 warnings are `no-explicit-any`, so a total-only gate is a count of
