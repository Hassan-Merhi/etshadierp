...
| Signal | Now | Command |
|---|---|---|
| Type escapes (AST) | 8,923 total | `npm run audit:type-escapes` |
| ESLint warnings | 9,569 total | `npm run lint` |
...
| God-file backlog | 52 files, 26,074 excess lines | `npm run audit:god-files` |
...
- **`totals.warningCeiling`** is the repository total, currently 9,569, and may
...
- **`perRule`** freezes each rule at its own count, checked by
  `npm run audit:lint-ratchet`. This is the part that matters: 8,921 of the
  9,569 warnings are `no-explicit-any`, so a total-only gate is a count of
