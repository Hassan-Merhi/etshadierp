...
| Signal | Now | Command |
|---|---|---|
| Type escapes (AST) | 8,990 total | `npm run audit:type-escapes` |
| ESLint warnings | 9,645 total | `npm run lint` |
...
| God-file backlog | 55 files, 29,068 excess lines | `npm run audit:god-files` |
...
- **`totals.warningCeiling`** is the repository total, currently 9,645, and may
...
- **`perRule`** freezes each rule at its own count, checked by
  `npm run audit:lint-ratchet`. This is the part that matters: 8,988 of the
  9,645 warnings are `no-explicit-any`, so a total-only gate is a count of
