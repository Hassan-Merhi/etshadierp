...
| Signal | Now | Command |
|---|---|---|
| Type escapes (AST) | 8,996 total | `npm run audit:type-escapes` |
| ESLint warnings | 9,651 total | `npm run lint` |
...
| God-file backlog | 56 files, 29,231 excess lines | `npm run audit:god-files` |
...
- **`totals.warningCeiling`** is the repository total, currently 9,651, and may
...
- **`perRule`** freezes each rule at its own count, checked by
  `npm run audit:lint-ratchet`. This is the part that matters: 8,994 of the
  9,651 warnings are `no-explicit-any`, so a total-only gate is a count of
