...
| Signal | Now | Command |
|---|---|---|
| Type escapes (AST) | 10,088 total | `npm run audit:type-escapes` |
| ESLint warnings | 10,755 total | `npm run lint` |
...
- **`totals.warningCeiling`** is the repository total, currently 10,755, and may
...
- **`perRule`** freezes each rule at its own count, checked by
  `npm run audit:lint-ratchet`. This is the part that matters: 10,086 of the
  10,755 warnings are `no-explicit-any`, so a total-only gate is a count of
