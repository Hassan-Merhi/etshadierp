...
- Phase 2's coverage targets — 35% backend, 20% frontend — are not met. The floors were raised to hold the measured gains (backend statements 27→28 and functions 32→33, frontend lines 11→12 and functions 7→8) rather than left where they were.
  - Revisited after the merge. Frontend lines went 13.98% → 17.57% (8,946 → 11,550 covered) by testing the surfaces that had never been imported at all: 44 pages that were at flat zero, the two fetch-patching pagination clients, and the raw-stock recalculation model. Floors moved with it — lines 12→16, statements 11→15, functions 8→9, branches 8→10. The remaining 2.4 points to 20% are roughly 1,600 lines and sit in page bodies whose behaviour needs fixtures rather than a mount, so they are worth writing as behavioural tests rather than as more shells.
  - Backend held at 31.4% lines. The work in this round was gate and evidence coverage rather than volume, so its floors are unchanged: raising them without a measured gain buys brittleness and no signal.
