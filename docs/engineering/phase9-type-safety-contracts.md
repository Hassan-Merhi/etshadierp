...
The initial session-company response is parsed from `unknown`. It accepts a positive integer ID or `null`, preserving the existing synchronization behavior.

## Selector cleanup

`CompanySelector` uses shared `Company` and `CompanyType` contracts. It no longer contains `company: any`, `as any`, or `error: any` in the company-switching path.
...
- Company switching and navigation behavior are unchanged.
...
Phase 9 remains a draft. It must not be merged until earlier roadmap phases are integrated and the owner explicitly authorizes the merge. CI, TypeScript compilation, formatting, lint, tests, build, browser verification, and deployment verification were not run.
