...
import { AlertTriangle, Check, Pencil, Plus, RotateCcw, StickyNote, Trash2, XIcon, MessageCircle, CheckCircle2, ArrowUp } from "lucide-react";
...
export function AgentCard({ agent, companyId, waGroupChatId }: { agent: AgentDutySummary; companyId: number; waGroupChatId?: string }) {
  ...
  const { data: prepaidData } = useQuery<{ designations: PrepaidDesignation[] }>({ queryKey: prepaidQKey, initialData: { designations: [] }, staleTime: 120_000 });
  ...
  const setAllPrepaidMutation = useMutation({
    ...
    onSuccess: (_data, containerIds, context) => {
      ...
      if (containerIds.length > prev) toast({ title: "Container designated as prepaid", description: "It now appears at the top of the list with a Prepaid badge." });
    },
  ...
  const replacePrepaidMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: prepaidQKey }); setReplaceTarget(null); setReplaceAmountWarning(null); },
  ...
  const noteMutation = useMutation({
    mutationFn: (newNote: string) => apiRequest("PUT", `/api/git/agent-note/${companyId}/${encodeURIComponent(agent.agentName)}`, { note: newNote }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: noteQKey }),
  });
  const saveNote   = () => { noteMutation.mutate(draftNote.trim()); setEditingNote(false); };
  ...
  const createAdjMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: adjQKey }); setNewDesc(""); setNewAmount(""); setNewType("debit"); setShowAdjForm(false); },
  ...
  const deleteAdjMutation = useMutation({
    mutationFn: (id: number) => apiRequest("DELETE", `/api/git/agent-adjustments/${companyId}/${encodeURIComponent(agent.agentName)}/${id}`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: adjQKey }),
    onError: (e: any) => toast({ title: "Failed to delete", description: e.message, variant: "destructive" }),
  });
  const saveAdj = () => {
    const amt = parseFloat(newAmount);
    if (!newDesc.trim() || isNaN(amt) || amt <= 0) { toast({ title: "Invalid entry", description: "Please enter a description and a positive amount.", variant: "destructive" }); return; }
  ...
  const sendToWhatsApp = useCallback(async () => {
    await sendAgentCardToWhatsApp({ agent, customOrder, dbPrepaidIds, adjustments, transitTransporterFilter, toast, setWaSending });
  ...
  // ── Computed state ────────────────────────────────────────────────────────
  const { agentName, matchConfidence, ledgerBalance, offloadedDutyTotal, clearedByPayments, openBalance, warnings, clearedRows, partialRows, openRows } = agent;
  const activePreviewRows = useMemo(() => agent.activePreviewRows.filter(r => !!(r.numberPlate ?? "").trim()), [agent.activePreviewRows]);
  ...
  const prepaidTransitRows  = useMemo(() => activePreviewRows.filter(r => prepaidTransitSet.has(r.id)), [activePreviewRows, prepaidTransitSet]);
  const remainingTransitRows = useMemo(() => activePreviewRows.filter(r => !prepaidTransitSet.has(r.id)), [activePreviewRows, prepaidTransitSet]);
  const designatedPrepaidSum = useMemo(() => prepaidTransitRows.reduce((s, r) => s + Number(r.dutyFee ?? 0), 0), [prepaidTransitRows]);
  const minOpenRemaining = allOpenPartial.length > 0 ? Math.min(...allOpenPartial.map(r => r.remainingAmount).filter(a => a > 0.01)) : Infinity;
  const allBudgetDesignated = designatedPrepaidSum > 0 && prepaidBudget > 0 && (
  ...
  const enhancedRemainder = allBudgetDesignated ? (offloadedDutyTotal ?? 0) * 2 + 1 : remainderForOP + designatedPrepaidSum;
  const enhancedAllocated = useMemo(() => {
    ...
    return openAndPartial.map(row => {
      ...
      if (rem >= needed) { rem -= needed; return { ...row, clearedAmount: row.dutyFee, remainingAmount: 0, allocationStatus: "Cleared" as ApiAllocStatus }; }
      else if (rem > 0) { const extra = rem; rem = 0; return { ...row, clearedAmount: row.clearedAmount + extra, remainingAmount: row.remainingAmount - extra, allocationStatus: "Partially Cleared" as ApiAllocStatus }; }
      return row;
    });
  }, [openAndPartial, enhancedRemainder]);
  const enhancedCoveredIds = useMemo(() => new Set(enhancedAllocated.filter(r => r.clearedAmount >= r.dutyFee).map(r => r.id)), [enhancedAllocated]);
  const visibleOpenPartial = useMemo(() => openAndPartial.filter(r => !enhancedCoveredIds.has(r.id)), [openAndPartial, enhancedCoveredIds]);
  ...
  const confidenceBadge = { exact: { label: "Exact match", cls: "bg-green-700 text-white" }, fuzzy: { label: "Fuzzy match", cls: "bg-amber-600 text-white" }, unmapped: { label: "No mapping", cls: "bg-muted text-muted-foreground border" } }[matchConfidence];
  ...
  return (
    ...
    <div className="space-y-0" data-testid={`agent-card-${agentName}`}>
      {waGroupChatId && (
        <div className="flex justify-end mb-1">
          <button type="button" onClick={sendToWhatsApp} disabled={waSending} title={`Send ${agentName} balance to WhatsApp`} data-testid={`button-wa-send-${agentName}`} className="flex items-center gap-1.5 px-3 py-1 rounded-md bg-green-600 text-white text-xs font-semibold disabled:opacity-60 shadow-sm">
            {waSending ? <span className="animate-spin inline-block w-3 h-3 border-2 border-white border-t-transparent rounded-full" /> : <MessageCircle className="h-3.5 w-3.5" />}
      ...
      <div className="rounded-md border border-border overflow-hidden shadow-sm">
        ...
        {warnings.map(code => {
          const meta = WARNING_META[code]; const Icon = meta.icon;
          return (
            <div key={code} className={cn("px-3 py-1.5 border-b text-xs flex gap-2 items-start", meta.className)}>
              <Icon className="h-3.5 w-3.5 shrink-0 mt-0.5" /><span>{meta.message}</span>
            </div>
          );
        })}
          ...
          <div className="px-3 py-2 border-b bg-amber-50/60 dark:bg-amber-950/15 flex items-start gap-2 min-h-[2rem]">
            ...
            {editingNote ? (
              <div className="flex-1 flex items-start gap-1.5">
                <textarea autoFocus value={draftNote} onChange={e => setDraftNote(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); saveNote(); } if (e.key === "Escape") cancelNote(); }} placeholder="e.g. Peage $400 · Road fees $530" rows={2} className="flex-1 text-xs rounded border border-amber-300 dark:border-amber-700 bg-white dark:bg-amber-950/30 px-2 py-1 text-amber-900 dark:text-amber-200 resize-none focus:outline-none focus:ring-1 focus:ring-amber-400" data-testid={`input-note-${agentName}`} />
                <button onClick={saveNote} title="Save" className="mt-0.5 text-green-700 dark:text-green-400 hover:text-green-900" data-testid={`button-save-note-${agentName}`}><Check className="h-4 w-4" /></button>
                <button onClick={cancelNote} title="Cancel" className="mt-0.5 text-muted-foreground hover:text-foreground"><XIcon className="h-4 w-4" /></button>
              </div>
            ) : (
              <div className="flex-1 flex items-start justify-between gap-2">
                <p className="text-xs text-amber-800 dark:text-amber-300 whitespace-pre-wrap leading-relaxed">{note}</p>
                <button onClick={() => { setDraftNote(note); setEditingNote(true); }} title="Edit note" className="shrink-0 text-amber-500 hover:text-amber-700 dark:hover:text-amber-300" data-testid={`button-edit-note-${agentName}`}><Pencil className="h-3 w-3" /></button>
              </div>
            )}
          </div>
        )}
        ...
        <div className="border-b">
            ...
            <div className="px-3 pt-2 space-y-1">
              {adjustments.map(a => (
                <div key={a.id} className="flex items-center gap-2 text-xs">
                  ...
                  <span className={cn("font-semibold tabular-nums shrink-0", a.type === "debit" ? "text-green-700 dark:text-green-400" : "text-red-600 dark:text-red-400")}>{a.type === "debit" ? "+" : "−"}${fmt(a.amount, 0)}</span>
                  <Badge variant="outline" className={cn("text-[10px] shrink-0 no-default-active-elevate", a.type === "debit" ? "text-green-700 border-green-400" : "text-red-600 border-red-400")}>{a.type === "debit" ? "Dr" : "Cr"}</Badge>
                  <button onClick={() => deleteAdjMutation.mutate(a.id)} disabled={deleteAdjMutation.isPending} className="shrink-0 text-muted-foreground hover:text-red-500 transition-colors disabled:opacity-40" title="Remove" data-testid={`button-delete-adj-${a.id}`}><Trash2 className="h-3 w-3" /></button>
                </div>
              ))}
            </div>
          )}
          {showAdjForm ? (
            <div className="px-3 py-2 flex items-center gap-1.5 flex-wrap">
              <input autoFocus value={newDesc} onChange={e => setNewDesc(e.target.value)} onKeyDown={e => e.key === "Enter" && saveAdj()} placeholder="Description (e.g. Peage, Road fees…)" className="flex-1 min-w-[140px] text-xs rounded border border-input bg-background px-2 py-1 focus:outline-none focus:ring-1 focus:ring-ring" data-testid={`input-adj-desc-${agentName}`} />
              <input type="number" min="0" step="any" value={newAmount} onChange={e => setNewAmount(e.target.value)} onKeyDown={e => e.key === "Enter" && saveAdj()} placeholder="Amount" className="w-24 text-xs rounded border border-input bg-background px-2 py-1 focus:outline-none focus:ring-1 focus:ring-ring text-right" data-testid={`input-adj-amount-${agentName}`} />
              ...
              <button type="button" onClick={saveAdj} disabled={createAdjMutation.isPending} className="px-2.5 py-1 rounded bg-primary text-primary-foreground text-[10px] font-semibold disabled:opacity-50 shrink-0" data-testid={`button-adj-save-${agentName}`}>{createAdjMutation.isPending ? "…" : "Save"}</button>
              <button type="button" onClick={() => { setShowAdjForm(false); setNewDesc(""); setNewAmount(""); setNewType("debit"); }} className="p-1 text-muted-foreground hover:text-foreground" title="Cancel"><XIcon className="h-3 w-3" /></button>
            </div>
          ) : (
            <div className="px-3 py-1.5 flex justify-end">
              <button type="button" onClick={() => setShowAdjForm(true)} className="flex items-center justify-center h-5 w-5 rounded-full border border-dashed border-muted-foreground/40 text-muted-foreground hover:border-primary hover:text-primary transition-colors" title="Add manual entry" data-testid={`button-adj-add-${agentName}`}><Plus className="h-3 w-3" /></button>
            </div>
          )}
        </div>
        ...
        {isCustomOrder && (
          <div className="flex items-center gap-2 px-3 py-1 bg-blue-50 dark:bg-blue-950/20 border-b border-blue-200 dark:border-blue-800 text-xs text-blue-800 dark:text-blue-300">
            ...
            <button onClick={resetOrder} className="ml-auto flex items-center gap-1 px-2 py-0.5 rounded border border-blue-300 dark:border-blue-700 hover:bg-blue-100 dark:hover:bg-blue-900/40 text-blue-700 dark:text-blue-300 font-medium" data-testid={`button-reset-order-${agentName}`}><RotateCcw className="h-3 w-3" />Reset to auto (FIFO)</button>
          </div>
        )}
        {isDbOverride && prepaidTransitRows.length > 0 && (
          <div className="flex justify-end px-3 py-1 border-b">
            <button onClick={() => setAllPrepaidMutation.mutate([])} className="flex items-center gap-1 px-2 py-0.5 rounded border border-muted text-xs text-muted-foreground hover:border-primary hover:text-primary transition-colors" data-testid={`button-reset-prepaid-transit-${agentName}`}><RotateCcw className="h-3 w-3" />Reset to auto</button>
          </div>
        )}
    ...
    <Dialog open={replaceTarget !== null} onOpenChange={open => { if (!open) { setReplaceTarget(null); setReplaceAmountWarning(null); } }}>
      <DialogContent className="max-w-lg">
        ...
        {replaceAmountWarning && (
          <div className="rounded-md border border-amber-300 bg-amber-50 dark:bg-amber-950/20 dark:border-amber-700 px-3 py-2 text-xs text-amber-800 dark:text-amber-300 flex items-start gap-2">
            ...
            <div className="flex-1">
              ...
              <p>Old: <span className="font-mono font-semibold">${fmt(replaceAmountWarning.oldAmount, 0)}</span> → New: <span className="font-mono font-semibold">${fmt(replaceAmountWarning.newAmount, 0)}</span></p>
            </div>
          </div>
        )}
        <div className="max-h-64 overflow-y-auto rounded-md border divide-y text-xs">
          {remainingTransitRows.length === 0 ? (
            ...
            remainingTransitRows.map(r => {
              ...
              return (
                <button key={r.id} data-testid={`button-replace-pick-${r.id}`} className={`w-full flex items-center gap-3 px-3 py-2 text-left hover-elevate ${isWarned ? "bg-amber-50/60 dark:bg-amber-950/20" : ""}`}
                  onClick={() => {
                    if (!replaceTarget) return;
                    if (amountsDiffer && !isWarned) { setReplaceAmountWarning({ oldAmount: replaceTarget.dutyFee, newAmount: r.dutyFee, newContainerId: r.id }); return; }
                    replacePrepaidMutation.mutate({ oldContainerId: replaceTarget.id, newContainerId: r.id, confirmDifferentAmount: amountsDiffer ? true : undefined });
                  }}>
        ...
        <DialogFooter>
          <Button variant="outline" size="sm" onClick={() => { setReplaceTarget(null); setReplaceAmountWarning(null); }}>Cancel</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
    </>
