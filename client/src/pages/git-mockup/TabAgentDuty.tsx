...
import type {
  CompanyViewMode, AgentDutyResponse, AgentDutyCompanySection,
  AgentDutySummary, AgentDutyWaSettings,
} from "./types";
...
export function TabAgentDuty() {
  ...
  const { data, isLoading, isError, error } = useQuery<AgentDutyResponse>({
    queryKey: [queryUrl], staleTime: 60_000, retry: 1,
  });

  const { data: waSettings } = useQuery<AgentDutyWaSettings>({
    queryKey: ["/api/git/agent-duty-wa-settings"], staleTime: 120_000,
  });
  ...
  const modeSelector = (
    <div className="flex items-center gap-2 flex-wrap" data-testid="agent-duty-mode-selector">
      ...
      <Button size="sm" variant={companyMode === "session" ? "default" : "outline"} className="text-xs gap-1.5" onClick={() => setCompanyMode("session")} data-testid="button-agent-duty-my-company">My Company</Button>
      <Button size="sm" variant={companyMode === "all" ? "default" : "outline"} className="text-xs gap-1.5" onClick={() => setCompanyMode("all")} data-testid="button-agent-duty-all-companies">All Accessible Companies</Button>
      {companyMode === "all" && (
        <Button size="sm" variant="outline" className={cn("text-xs gap-1.5 toggle-elevate", mergeAgents && "toggle-elevated")} onClick={() => setMergeAgents(v => !v)} data-testid="button-agent-duty-merge">
          <Layers className="h-3 w-3" />Merge same agents
        </Button>
      )}
    </div>
  );
          ...
          <div className="grid grid-cols-4 divide-x border-b">
            {[1,2,3,4].map(j => (
              <div key={j} className="px-2 py-3 space-y-1.5">
                <Skeleton className="h-2.5 w-20 mx-auto" /><Skeleton className="h-4 w-16 mx-auto" /><Skeleton className="h-2 w-12 mx-auto" />
              </div>
            ))}
          </div>
          <div className="p-3 space-y-1.5">{[1,2,3].map(j => <Skeleton key={j} className="h-5 w-full" />)}</div>
        </div>
      ))}
    </div>
  );
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-md border border-red-200 bg-red-50 dark:bg-red-950/20 px-4 py-3 flex gap-3 items-start text-sm text-red-800 dark:text-red-300">
        ...
        <div>
          ...
          <div className="text-xs mt-0.5 text-red-700 dark:text-red-400">{(error as Error)?.message ?? "Network or server error."}</div>
        </div>
      </div>
    </div>
  );
  ...
  if (totalAgents === 0) return (
    <div className="space-y-4">
      ...
      <div className="rounded-md border border-dashed px-6 py-10 text-center text-muted-foreground text-sm">
        ...
        <div className="text-xs mt-1">{companyMode === "all" ? "No containers with a non-zero duty fee and agent name exist across accessible companies." : "No containers with a non-zero duty fee and agent name exist for this company."}</div>
      </div>
    </div>
  );
        ...
        <div key={section.companyId} className="space-y-4" data-testid={`company-section-${section.companyId}`}>
          {companyMode === "all" && !(mergeAgents && displaySections.length === 1) && (
            <div className="flex items-center gap-2 pt-1">
              ...
              <Badge variant="outline" className="text-xs no-default-active-elevate">{section.agents.length} {section.agents.length === 1 ? "agent" : "agents"}</Badge>
