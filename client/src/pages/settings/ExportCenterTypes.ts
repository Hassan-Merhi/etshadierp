export interface Recipient { id: number; email: string; active: boolean; created_at: string; }
export interface ExportSettings { gmailUser: string; scheduleEnabled: boolean; lastRunAt: string | null; scheduleHour: number; scheduleTimezone: string; }
export interface Company { id: number; name: string; code: string; }
export interface JobStep { time: string; message: string; type: "info" | "success" | "error" | "warning"; }
export interface JobStatus { status: "running" | "done" | "error"; steps: JobStep[]; error?: string; hasZip: boolean; }
...
export interface WaRecipient { id: number; chatId: string; name: string; isGroup: boolean; active: boolean; }
