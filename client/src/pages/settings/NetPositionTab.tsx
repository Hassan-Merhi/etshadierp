    ...
    <div className="space-y-4 mt-4">
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-48">
                ...
                <DropdownMenuItem
                  onClick={() => qc.invalidateQueries({ queryKey: ["/api/whatsapp/np-settings"] })}
                  data-testid="menu-np-refresh"
                >
      ...
      <div className="flex items-center gap-2 rounded-md bg-muted/40 border px-3 py-2">
        ...
        <p className="text-xs text-muted-foreground">
          ...
          Jan 1, {new Date().getFullYear()} → today ({npDefaultEnd}). Every send covers from the start of the current
          year up to the run date.
        </p>
      </div>
      ...
      <Card data-testid="card-np-settings">
        ...
        <CardContent className="space-y-5">
          <div className="space-y-1.5">
            ...
            <p className="text-xs text-muted-foreground">
              A ZIP with one net position Excel per company will be sent to this group.
            </p>
            {waGroups.length === 0 ? (
              <p className="text-xs text-muted-foreground italic">
                No active group recipients — add one in the Recipients section below.
              </p>
            ) : (
              <Select
          ...
          <div className="flex gap-2 flex-wrap">
            <Button
              onClick={() => npSaveSettings.mutate()}
              disabled={npSaveSettings.isPending}
              data-testid="button-np-save"
            >
              {npSaveSettings.isPending ? (
                <>
          ...
          <p className="text-xs text-muted-foreground">
            "Send Now" (via the Actions menu) immediately sends the ZIP ({npDefaultStart} → {npDefaultEnd}) to the
            selected WhatsApp group. The scheduler checks every hour and sends automatically when the time matches.
          </p>
        </CardContent>
      </Card>
    </div>
