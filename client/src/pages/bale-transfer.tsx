...
export default function BaleTransferPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <Card className="p-6">
        <h2 className="text-2xl font-semibold mb-6" data-testid="heading-new-transfer">
          New Transfer
        </h2>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Select
                ...
                onValueChange={(v) => setSelectedSourceLocation(parseInt(v))}
              >
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto border rounded p-3">
                {bales.map((bale: any) => (
                  <div
                    ...
                    data-testid={`bale-item-${bale.id}`}
                  >
                    <span data-testid={`text-bale-code-${bale.id}`}>
                      {bale.baleCode} - {bale.category} ({bale.grade})
                    </span>
                    <Button
                      size="sm"
                      variant="outline"
              ...
              <div className="space-y-2 border rounded p-3">
                {selectedBales.map((bale) => (
                  <div
                    ...
                    data-testid={`selected-bale-${bale.id}`}
                  >
                    <div className="flex-1">
                      <p className="font-semibold" data-testid={`text-selected-code-${bale.id}`}>
                        {bale.baleCode}
                      </p>
      ...
      <Card className="p-6">
        <h2 className="text-2xl font-semibold mb-6" data-testid="heading-recent-transfers">
          Recent Transfers
        </h2>
        <div className="space-y-4">
          {transfers.length === 0 ? (
            <p className="text-gray-500" data-testid="text-no-transfers">
              No transfers yet
            </p>
          ) : (
            transfers.map((transfer: any) => (
              <div key={transfer.id} className="border rounded p-4" data-testid={`transfer-card-${transfer.id}`}>
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <p className="font-semibold" data-testid={`text-transfer-id-${transfer.id}`}>
                      Transfer #{transfer.id}
                    </p>
                    <p className="text-sm text-gray-600" data-testid={`text-transfer-date-${transfer.id}`}>
