    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border bg-card overflow-hidden">
        <Table>
          ...
          <TableBody>
            {catsLoading ? (
              <TableRow><TableCell colSpan={3} className="h-32 text-center text-muted-foreground">Loading groups...</TableCell></TableRow>
            ) : workerCategories.length === 0 ? (
              <TableRow><TableCell colSpan={3} className="h-32 text-center text-muted-foreground">No worker groups created yet.</TableCell></TableRow>
            ) : (
                ...
                <TableRow key={cat.id}>
                  ...
                  <TableCell>
                    <div className="flex flex-wrap gap-1">
                      {catWorkers.filter(w => (cat.workerIds || []).includes(w.id)).map(w => (
                        <Badge key={w.id} variant="secondary" className="text-[10px]">{w.name}</Badge>
                      ))}
                      {(cat.workerIds || []).length === 0 && <span className="text-xs text-muted-foreground italic">No workers assigned</span>}
                    </div>
                  </TableCell>
                  <TableCell className="text-right">
                    <div className="flex items-center justify-end gap-1">
                      <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => openEditCat(cat)} data-testid={`button-edit-category-${cat.id}`}>
                        <Pencil className="h-3.5 w-3.5" />
                      </Button>
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive" onClick={() => { if (confirm("Delete this group?")) deleteCatMutation.mutate(cat.id); }} data-testid={`button-delete-category-${cat.id}`}>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              ...
              <Input value={catName} onChange={e => setCatName(e.target.value)} placeholder="e.g. Pressing Team" data-testid="input-category-name" />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium">Assign Workers</label>
              <div className="grid grid-cols-2 gap-2 max-h-60 overflow-y-auto pr-2">
                {catWorkers.filter(w => w.active !== false).map(w => (
                  <div
                    key={w.id}
                  ...
                  >
                    <div className={`h-4 w-4 rounded-sm border flex items-center justify-center ${catWorkerIds.includes(w.id) ? "bg-primary border-primary" : "bg-background border-input"}`}>
          ...
          <DialogFooter>
            ...
            <Button onClick={saveCat} disabled={createCatMutation.isPending || updateCatMutation.isPending} data-testid="button-save-category">
