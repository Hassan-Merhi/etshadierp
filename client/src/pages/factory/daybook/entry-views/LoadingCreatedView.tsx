...
export function LoadingCreatedView({
  ...
  const expectedBalesTotal = lines.reduce((s: number, l: unknown) => s + (parseInt(l.quantity || "0") || 0), 0);
  const scannedBales = balesList.length;
  const totalWeightKg = balesList.reduce((s: number, b: unknown) => s + n(b.weight), 0);
              ...
              <div>
                ...
                <div className="rounded-md border overflow-hidden">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {Object.entries(
                        balesList.reduce((acc: Record<string, { count: number; weight: number }>, b: unknown) => {
                    ...
                    <tfoot>
                      <tr className="bg-muted/50 border-t font-semibold">
                        ...
                        <td className="px-3 py-2 text-right font-mono text-muted-foreground">
                          {formatNumber(
                            balesList.reduce(
                              (sum: number, b: unknown) => sum + parseFloat(b.weight || b.weightKg || "0"),
                              0
                            )
                          )}
                        </td>
