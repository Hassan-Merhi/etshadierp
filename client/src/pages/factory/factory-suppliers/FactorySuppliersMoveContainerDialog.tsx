    ...
    >
      <DialogContent className="max-w-sm">
        ...
        <p className="text-sm text-muted-foreground mb-2">
          Moving <span className="font-semibold text-foreground">{model.moveContainerDialog.containerRef}</span> to a
          new supplier shifts the outstanding balance immediately. Payments already recorded under the current supplier
        ...
        <Select value={model.moveTargetSupplierId} onValueChange={model.setMoveTargetSupplierId}>
          ...
          <SelectContent>
            {model.activeSuppliers
              .filter((supplier) => supplier.id !== model.statementSupplierId)
              .map((supplier) => (
          ...
          <Button
            ...
            onClick={() => {
              if (model.moveContainerDialog.containerId && model.moveTargetSupplierId) {
                model.moveContainerMutation.mutate({
                  containerId: model.moveContainerDialog.containerId,
                  targetSupplierId: parseInt(model.moveTargetSupplierId),
                });
              }
            }}
          >
