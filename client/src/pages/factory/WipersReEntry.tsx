...
export default function WipersReEntry() {
  ...
  const cleanupMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/factory/bales"] });
      toast({ title: "Bales Removed", description: `${data.removedCount || cleanupSelectedIds.size} bales removed from stock` });
    },
    onError: (err: Error) => {
  ...
  const openBrowserPrint = (labels: LabelData[], format: "A4" | "A5" | "sticker", designColor?: A4DesignColor) => {
    if (format === "sticker") {
      ...
      if (w) { w.document.write(generateStickerLabelsHtml(labels)); w.document.close(); w.focus(); setTimeout(() => w.print(), 500); }
    } else if (format === "A5") {
      const w = window.open("", "_blank");
      if (w) { w.document.write(generateA5LabelsHtml(labels)); w.document.close(); w.focus(); setTimeout(() => w.print(), 500); }
    } else {
      if (!designColor) {
      ...
      const w = window.open("", "_blank");
      if (w) { w.document.write(generateCombinedLabelsHtml(labels, designColor)); w.document.close(); w.focus(); setTimeout(() => w.print(), 500); }
    }
  };
      ...
      <div className="p-6 space-y-6">
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="space-y-1">
              <Label htmlFor="location">Warehouse Location <span className="text-destructive">*</span></Label>
                  ...
                  <TableBody>
                    {cart.map(item => (
                      <TableRow key={item.productId} data-testid={`row-cart-${item.productId}`}>
                        ...
                        <TableCell className="text-muted-foreground text-sm">{item.product.articleCode || item.product.code}</TableCell>
                        <TableCell className="text-right">
                          <div className="flex items-center justify-end gap-1">
                            <Button size="icon" variant="ghost" onClick={() => updateQty(item.productId, -1)} data-testid={`button-qty-minus-${item.productId}`}>
                            ...
                            />
                            <Button size="icon" variant="ghost" onClick={() => updateQty(item.productId, 1)} data-testid={`button-qty-plus-${item.productId}`}>
                        ...
                        <TableCell>
                          <Button size="icon" variant="ghost" onClick={() => removeItem(item.productId)} data-testid={`button-remove-${item.productId}`}>
            ...
            {cart.length > 0 && (
              <div className="flex flex-wrap items-center justify-between gap-4 pt-2 border-t">
                <div className="flex gap-4 text-sm">
                  <span>Total bales: <span className="font-semibold font-mono" data-testid="text-total-qty">{totalQty}</span></span>
                  <span>Total weight: <span className="font-semibold font-mono" data-testid="text-total-kg">{formatLabelNum(totalKg)} kg</span></span>
                </div>
                <Button
                  onClick={() => {
                    if (!entryDate) { toast({ title: "Error", description: "Entry date is required", variant: "destructive" }); return; }
                    if (!selectedLocationId) { toast({ title: "Error", description: "Warehouse location is required", variant: "destructive" }); return; }
                    setConfirmOpen(true);
                  }}
            ...
            <CardTitle className="text-base flex items-center justify-between gap-2">
              <span className="flex items-center gap-2">
                ...
                {currentWipersBales.length > 0 && (
                  <Badge variant="secondary">{currentWipersBales.length} bales</Badge>
                )}
              </span>
                  ...
                  {cleanupSelectedIds.size > 0 && (
                    <div className="border rounded-md p-4 space-y-3 bg-destructive/5">
                      <p className="text-sm font-medium text-destructive flex items-center gap-2">
                        ...
                        Removing {cleanupSelectedIds.size} bale{cleanupSelectedIds.size !== 1 ? "s" : ""} — supervisor authorization required
                      </p>
                      ...
                      >
                        ...
                        {cleanupMutation.isPending ? "Removing..." : `Remove ${cleanupSelectedIds.size} Bale${cleanupSelectedIds.size !== 1 ? "s" : ""}`}
                      </Button>
                    </div>
                  )}
                </>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                <div className="rounded-md border px-4 py-3">
                  ...
                  <p className="text-2xl font-bold font-mono" data-testid="text-result-qty">{createdBales.length}</p>
                </div>
                <div className="rounded-md border px-4 py-3">
                  <p className="text-xs text-muted-foreground mb-1">Entry Date</p>
                  <p className="text-lg font-semibold" data-testid="text-result-date">{entryDate}</p>
                </div>
                <div className="rounded-md border px-4 py-3">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Button onClick={() => handlePrint("A5")} variant="outline" data-testid="button-print-a5">
                  <Printer className="h-4 w-4 mr-2" />Print A5
                </Button>
                <Button onClick={handlePrintAll} data-testid="button-print-all">
                  <Printer className="h-4 w-4 mr-2" />Print All Formats
                </Button>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1 max-h-40 overflow-y-auto">
                  {createdBales.map(b => (
                    <Badge key={b.id} variant="secondary" className="font-mono text-xs" data-testid={`badge-ref-${b.id}`}>
                      {b.referenceNumber}
                    </Badge>
                  ))}
                </div>
      ...
      <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Review details before creating bales
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm">
            <div className="flex items-center justify-between rounded-md bg-muted/50 px-3 py-2">
              <span className="text-muted-foreground">Entry Date</span>
              <span className="font-semibold text-primary" data-testid="text-confirm-date">{entryDate}</span>
            </div>
            <div className="flex items-center justify-between rounded-md bg-muted/50 px-3 py-2">
                ...
                <TableBody>
                  {cart.map(item => (
                    <TableRow key={item.productId}>
                      ...
                      <TableCell className="text-right font-mono text-xs">{formatLabelNum(item.qty * item.weightPerBaleKg)} kg</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
            ...
            <div className="rounded-md bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 px-3 py-2 text-xs text-amber-800 dark:text-amber-200 space-y-1">
              <p className="flex items-center gap-1 font-medium"><AlertTriangle className="h-3 w-3" /> Important</p>
              <p>Bales will be recorded under <strong>{entryDate}</strong>, not today's date.</p>
              <p>New reference numbers continue from the existing REF sequence.</p>
            </div>
          </div>
          <DialogFooter className="gap-2">
            <Button variant="outline" onClick={() => setConfirmOpen(false)} data-testid="button-confirm-cancel">Cancel</Button>
            <Button
