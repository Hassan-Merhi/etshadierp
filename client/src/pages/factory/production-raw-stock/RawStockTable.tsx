              ...
              return (
                <>
                  ...
                  >
                    ...
                    <TableCell className="text-right font-mono text-sm py-3 text-muted-foreground">{formatNumber(catReceived)}</TableCell>
                    <TableCell className="text-right font-mono text-sm py-3 text-muted-foreground">{formatNumber(catUsed)}</TableCell>
                    <TableCell className="text-right font-mono text-sm py-3 text-muted-foreground">{formatNumber(catRemaining - catFree)}</TableCell>
                    <TableCell className="text-right font-mono text-sm py-3 font-semibold text-foreground">{formatNumber(catFree)}</TableCell>
                    <TableCell className="text-right font-mono text-sm py-3 text-muted-foreground">${formatNumber(catValue)}</TableCell>
                    <TableCell className="py-3" />
                  </TableRow>

                  {isExpanded && rows.map((row, idx) => (
                    <TableRow key={`${cat}-${idx}`} className="group hover:bg-accent/5 transition-colors" data-testid={`row-raw-stock-${row.supplierId}`}>
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className="text-[10px] text-muted-foreground flex items-center gap-2 mt-0.5">
                            ...
                            {row.sourceType === "OPENING_BALANCE" && (
                              <span className="inline-flex items-center px-1.5 rounded bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 border border-blue-100 dark:border-blue-800 font-medium">OB</span>
                            )}
                          </span>
                        </div>
                      </TableCell>
                      <TableCell className="text-right font-mono text-xs text-muted-foreground py-3">{formatNumber(parseFloat(row.receivedKg))}</TableCell>
                      <TableCell className="text-right font-mono text-xs text-muted-foreground py-3">{formatNumber(parseFloat(row.usedKg))}</TableCell>
                      <TableCell className="text-right font-mono text-xs text-muted-foreground py-3">{formatNumber(parseFloat(row.reservedKg || "0"))}</TableCell>
                      <TableCell className="text-right font-mono text-sm py-3 font-medium text-foreground">{formatNumber(parseFloat(row.freeKg || "0"))}</TableCell>
                      <TableCell className="text-right font-mono text-xs text-muted-foreground py-3">
                        <div className="flex flex-col items-end">
                          ...
                          <span className="text-[10px] opacity-60">${parseFloat(row.costPerKgUsd || "0").toFixed(4)}/kg</span>
                        </div>
                      </TableCell>
                      <TableCell className="py-3 text-right pr-4">
                        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <Button size="icon" variant="ghost" className="h-8 w-8 text-primary hover:bg-primary/10" onClick={() => onAdjust(row)} title="Adjust stock cost/qty" data-testid={`button-adjust-${row.supplierId}`}>
                            <Plus className="h-3.5 w-3.5" />
                          </Button>
                          <Button size="icon" variant="ghost" className="h-8 w-8 text-destructive hover:bg-destructive/10" onClick={() => onDeduct(row)} title="Deduct damaged/wasted stock" data-testid={`button-deduct-${row.supplierId}`}>
                            <MinusCircle className="h-3.5 w-3.5" />
                          </Button>
                          <Button size="icon" variant="ghost" className="h-8 w-8 text-emerald-600 hover:bg-emerald-50" onClick={() => onAddToBatch(row)} title="Add to mix batch" data-testid={`button-batch-${row.supplierId}`}>
