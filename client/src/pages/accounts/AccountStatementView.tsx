                ...
                return (
                  <div key={section.currencyCode} className="space-y-2">
                    <div className="flex items-center gap-2">
                      <Badge variant="secondary" className="text-sm px-3 py-1 font-bold">{section.currencyCode}</Badge>
                      ...
                      <TableBody>
                        {section.rows.map((row: any, idx: number) => (
                          <TableRow key={idx} className="text-xs">
                            ...
                            <TableCell><Badge variant="outline">{typeLabel[row.type] || row.type}</Badge></TableCell>
                            <TableCell>{row.description}</TableCell>
                            <TableCell className={`text-right ${typeColor(row.type)}`}>{ccPfx(section.currencyCode)}{fmt(Math.abs(row.amount))}</TableCell>
                            <TableCell className="text-right font-medium">{ccPfx(section.currencyCode)}{fmt(Math.abs(row.runningBalance))}</TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                    </Table>
    ...
    <Card>
      <CardHeader className="pb-3 flex flex-row items-center justify-between gap-2 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Button variant="outline" size="sm" onClick={handlePrint}><FileDown className="h-4 w-4 mr-1" /> Print</Button>
          {(appMode === "factory" || appMode === "erp") && (
            <Button variant="outline" size="sm" onClick={openWaRuleDialog}><MessageCircle className="h-4 w-4 mr-1" /> WhatsApp</Button>
          )}
        </div>
      </CardHeader>
      <CardContent>
        {transactionsLoading ? <Skeleton className="h-40 w-full" /> : (
          <>
            {!hideBalances && vouchersWithBalance.length > 0 && (
               <div className="flex flex-wrap gap-3 mb-4">
                  <div className="rounded-lg border bg-muted/40 px-4 py-2 flex items-center gap-3">
                     ...
                     <div><p className="text-xs text-muted-foreground leading-none mb-0.5">Closing Balance</p>
                     <p className="text-base font-semibold leading-none tabular-nums">{formatAmount(Math.abs(closingBalance))}</p></div>
                  </div>
               </div>
            )}
            <div ref={printRef}>
