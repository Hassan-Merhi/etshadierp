...
export function StockMovementDialog({
  ...
  const { data: stockMovementData, isLoading: stockMovementLoading } = useQuery<any>({
    queryKey: stockMovementItem ? ["/api/inventory/movement", stockMovementItem.stockItemId, stockMovementItem.locationId, stockMovementPeriod] : [],
    enabled: stockMovementOpen && !!stockMovementItem,
  });

  const { data: smDrillData, isLoading: smDrillLoading } = useQuery<any>({
    queryKey: stockMovementItem && drillMonth ? ["/api/inventory/movement/drill", stockMovementItem.stockItemId, stockMovementItem.locationId, drillMonth.year, drillMonth.month] : [],
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <DialogDescription className="text-xs">
                  {stockMovementItem?.stockItemName} {stockMovementItem?.locationName ? `at ${stockMovementItem.locationName}` : "(All Locations)"}
                </DialogDescription>
              </div>
            </div>
            {!drillMonth && (
              <div className="flex items-center gap-2">
                <PeriodFilter
                  value={stockMovementPeriod}
                  onValueChange={setStockMovementPeriod}
                  compact
                />
              </div>
            )}
              ...
              <table className="w-full text-sm border-collapse" style={{ minWidth: 860 }}>
                <thead className="sticky top-0 z-20">
                  <tr className="bg-muted border-b">
                    <th rowSpan={2} className="text-left align-bottom px-4 py-2 border-r font-semibold w-24">Date</th>
                    <th rowSpan={2} className="text-left align-bottom px-4 py-2 border-r font-semibold">Particulars</th>
                    <th rowSpan={2} className="text-left align-bottom px-4 py-2 border-r font-semibold w-32">Vch Type</th>
                    <th colSpan={3} className="text-center px-2 py-1.5 border-r font-semibold text-green-700 dark:text-green-400 text-xs">Inward</th>
                    <th colSpan={3} className="text-center px-2 py-1.5 border-r font-semibold text-red-700 dark:text-red-400 text-xs">Outward</th>
