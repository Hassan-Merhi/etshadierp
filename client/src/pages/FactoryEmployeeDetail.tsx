          ...
          <Tabs defaultValue="profile">
            ...
            <TabsContent value="profile" className="mt-0">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <Card>
                  ...
                  <CardContent className="px-5 pb-4">
                    {infoRow("Code", employee.code)}
                    {infoRow("Department", employee.department)}
                    {infoRow("Join Date", formatDate(employee.joinDate))}
                    {infoRow("Status", employee.active ? "Active" : "Inactive")}
                  </CardContent>
                </Card>
                <Card>
                  <CardHeader className="pb-2 pt-4 px-5">
                    <CardTitle className="text-sm text-muted-foreground">Compensation</CardTitle>
                  </CardHeader>
                  <CardContent className="px-5 pb-4">
                    {infoRow("Monthly Salary", fmt(employee.monthlySalary))}
                    {infoRow("Current Balance", fmt(employee.currentBalance))}
                    {infoRow("Total Deposits", fmt(employee.totalDeposits))}
                    {infoRow("Total Withdrawals", fmt(employee.totalWithdrawals))}
                  </CardContent>
                </Card>
              </div>
            </TabsContent>
            ...
            <TabsContent value="deposits" className="mt-0">
              <Card>
                <CardHeader className="pb-2 pt-4 px-5 flex flex-row items-center justify-between gap-2">
                  <CardTitle className="text-sm text-muted-foreground">Deposits</CardTitle>
                  <Button size="sm" onClick={() => setDepositOpen(true)} data-testid="button-new-deposit">
                ...
                <CardContent className="px-5 pb-4 overflow-x-auto">
                  {stmtLoading ? (
                    ...
                    <p className="text-sm text-muted-foreground py-8 text-center">No deposits recorded yet</p>
                  ) : (
                    <Table>
                      <TableHeader>
                        <TableRow>
                      ...
                      <TableBody>
                        {depositRows.map((row) => (
                          <TableRow key={row.id} data-testid={`row-deposit-${row.id}`}>
                            <TableCell className="text-sm">{formatDate(row.voucherDate)}</TableCell>
                            <TableCell className="font-mono text-xs text-muted-foreground">{row.voucherNumber}</TableCell>
                            <TableCell className="text-sm text-muted-foreground">{row.narration || row.description || "—"}</TableCell>
                            <TableCell className="text-right font-mono text-sm text-green-600 dark:text-green-400">{fmt(row.credit)}</TableCell>
                            <TableCell className={`text-right font-mono text-sm font-semibold ${row.balance < 0 ? "text-red-600 dark:text-red-400" : ""}`}>{fmt(row.balance)}</TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                    </Table>
            ...
            <TabsContent value="withdrawals" className="mt-0">
              <Card>
                <CardHeader className="pb-2 pt-4 px-5 flex flex-row items-center justify-between gap-2">
                  ...
                  <Button size="sm" onClick={() => setWithdrawOpen(true)} data-testid="button-new-withdrawal">
                ...
                <CardContent className="px-5 pb-4 overflow-x-auto">
                  {stmtLoading ? (
                    ...
                    <p className="text-sm text-muted-foreground py-8 text-center">No withdrawals recorded yet</p>
                  ) : (
                    <Table>
                      <TableHeader>
                        <TableRow>
                      ...
                      <TableBody>
                        {withdrawRows.map((row) => (
                          <TableRow key={row.id} data-testid={`row-withdrawal-${row.id}`}>
                            <TableCell className="text-sm">{formatDate(row.voucherDate)}</TableCell>
                            <TableCell className="font-mono text-xs text-muted-foreground">{row.voucherNumber}</TableCell>
                            <TableCell className="text-sm text-muted-foreground">{row.narration || row.description || "—"}</TableCell>
                            <TableCell className="text-right font-mono text-sm text-red-600 dark:text-red-400">{fmt(row.debit)}</TableCell>
                            <TableCell className={`text-right font-mono text-sm font-semibold ${row.balance < 0 ? "text-red-600 dark:text-red-400" : ""}`}>{fmt(row.balance)}</TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
            ...
            <TabsContent value="payroll" className="mt-0">
              <div className="space-y-4">
                <Card>
                  <CardHeader className="pb-2 pt-4 px-5">
                    <CardTitle className="text-sm text-muted-foreground">Bulk Payroll Deposit</CardTitle>
                  </CardHeader>
                      ...
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <div className="flex gap-2">
                          <Button
                            ...
                            variant="outline"
                            onClick={() => {
                              const activeIds = allEmployees.filter((e) => e.active).map((e) => e.id);
                              setSelectedEmployees(new Set(activeIds));
                              const amounts: Record<number, string> = {};
                              allEmployees.filter((e) => e.active).forEach((e) => {
                                amounts[e.id] = e.monthlySalary || "0";
                              });
                              setPayrollAmounts(amounts);
                            }}
                          ...
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => { setSelectedEmployees(new Set()); setPayrollAmounts({}); }}
                        ...
                        <Table>
                          <TableHeader>
                            <TableRow>
                              ...
                              <TableHead className="text-right">Amount to Deposit</TableHead>
                            </TableRow>
                          </TableHeader>
                          <TableBody>
                            {allEmployees.filter((e) => e.active).map((emp) => {
                              ...
                              return (
                                <TableRow
                                  ...
                                  data-testid={`row-payroll-emp-${emp.id}`}
                                >
                                  <TableCell>
                                    <Checkbox
                                      checked={isSelected}
                                      onCheckedChange={(checked) => {
                                        ...
                                        if (checked) {
                                          ...
                                          setPayrollAmounts((a) => ({
                                            ...a,
                                            [emp.id]: a[emp.id] ?? (emp.monthlySalary || "0"),
                                          }));
                                        } else {
                                          next.delete(emp.id);
                                        }
                                  ...
                                  <TableCell className="text-right font-mono text-sm">{fmt(emp.monthlySalary)}</TableCell>
                                  <TableCell className="text-right">
                                    <Input
                                      ...
                                      value={payrollAmounts[emp.id] ?? (isSelected ? emp.monthlySalary || "" : "")}
                                      onChange={(e) => setPayrollAmounts((a) => ({ ...a, [emp.id]: e.target.value }))}
                                      disabled={!isSelected}
                    ...
                    {selectedEmployees.size > 0 && (
                      <div className="flex items-center justify-between pt-2 border-t">
                        <div className="text-sm text-muted-foreground">
                          {selectedEmployees.size} employee{selectedEmployees.size !== 1 ? "s" : ""} selected · Total:{" "}
                          <span className="font-mono font-semibold text-foreground">
                            {fmt(
                              Array.from(selectedEmployees).reduce((sum, empId) => {
                                const amt = payrollAmounts[empId] ?? allEmployees.find((e) => e.id === empId)?.monthlySalary ?? "0";
                        ...
                        >
                          {bulkPayrollMutation.isPending ? (
                            <><Loader2 className="h-4 w-4 mr-2 animate-spin" />Processing...</>
                          ) : (
                            <><CheckCircle2 className="h-4 w-4 mr-2" />Run Payroll</>
                          )}
                        </Button>
                      </div>
                    )}
