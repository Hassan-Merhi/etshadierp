...
export function EmployeeStatementDialog({
  statementEmployee, setStatementEmployee, transactionsLoading,
  employeeTransactions, statementExpanded, setStatementExpanded, cleanTxnDesc,
      ...
      <DialogContent className="max-w-4xl w-[95vw] max-h-[85vh] overflow-y-auto">
        ...
        {!transactionsLoading && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-2">
            <div className="rounded-md border bg-muted/30 p-3">
              ...
              <p className="font-mono font-semibold text-sm mt-1">{formatAmount(parseFloat(statementEmployee?.calculatedBalance || "0"))}</p>
              <p className="text-xs text-muted-foreground mt-0.5">{parseFloat(statementEmployee?.calculatedBalance || "0") >= 0 ? "Owed to employee" : "Advance taken"}</p>
            </div>
            <div className="rounded-md border bg-muted/30 p-3">
              <p className="text-xs text-muted-foreground">Total Deposited</p>
              <p className="font-mono font-semibold text-sm mt-1">{formatAmount(employeeTransactions.filter((t: any) => !t.isDebit).reduce((s: number, t: any) => s + parseFloat(t.amount || "0"), 0))}</p>
        ...
        <div className="mt-2">
          {transactionsLoading ? (
            ...
            <div className="text-center py-6 text-muted-foreground text-sm">No transactions found for this employee</div>
          ) : (() => {
                  ...
                  <div className="overflow-y-auto max-h-[50vh] space-y-0">
                      ...
                      <Table>
                        ...
                        <TableBody>
                          <TableRow className="bg-muted/20 font-medium">
                            ...
                            <TableCell className="text-right font-mono text-sm">{openingBalance < 0 ? formatAmount(Math.abs(openingBalance)) : <span className="text-muted-foreground">—</span>}</TableCell>
                            <TableCell className="text-right font-mono text-sm">{openingBalance >= 0 ? formatAmount(openingBalance) : <span className="text-muted-foreground">—</span>}</TableCell>
                          </TableRow>
                          {sorted.map((txn: any) => (
                            <TableRow key={txn.id || `${txn.voucherId}-${txn.date}`}>
                              <TableCell className="font-mono text-sm whitespace-nowrap">{txn.date ? formatDisplayDate(txn.date) : "-"}</TableCell>
                              <TableCell className="text-sm text-muted-foreground">{cleanTxnDesc(txn.narration || txn.voucherDescription || txn.description || txn.voucherType || "-")}</TableCell>
                              <TableCell className="text-right font-mono text-sm">{txn.isDebit ? formatAmount(parseFloat(txn.amount || "0")) : <span className="text-muted-foreground">—</span>}</TableCell>
                              <TableCell className="text-right font-mono text-sm">{!txn.isDebit ? formatAmount(parseFloat(txn.amount || "0")) : <span className="text-muted-foreground">—</span>}</TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
                    ...
                    <div className="md:hidden space-y-0 border rounded-md overflow-hidden">
                      ...
                      <div className="px-3 pt-2 pb-3 space-y-1 border-t-2">
                        <div className="flex justify-between text-sm"><span className="text-muted-foreground">Opening Balance</span><span className="font-mono font-semibold">{formatAmount(Math.abs(openingBalance))}{openingBalance < 0 ? " (Dr)" : ""}</span></div>
                        <div className="flex justify-between text-sm"><span className="text-muted-foreground">Total Debit</span><span className="font-mono font-semibold">{formatAmount(totalDebit)}</span></div>
                        <div className="flex justify-between text-sm"><span className="text-muted-foreground">Total Credit</span><span className="font-mono font-semibold">{formatAmount(totalCredit)}</span></div>
                        <div className="flex justify-between text-sm font-semibold"><span>Current Balance</span><span className={`font-mono ${currentBalance >= 0 ? "" : "text-destructive"}`}>{formatAmount(Math.abs(currentBalance))}{currentBalance < 0 ? " (Dr)" : ""}</span></div>
                      </div>
                    </div>
                  </div>
                )}
