...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Import Cycle Balance Breakdown */}
      <Card className="p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className={cn(
            ...
            Math.abs(importCycleData?.netImportCycleBalance ?? 0) < 0.01 ? "text-green-600" : "text-red-600"
          )}>
            {importCycleData ? formatCurrency(importCycleData.netImportCycleBalance) : "Loading..."}
          </div>
        </div>
        
        {importCycleData && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Assets & Expenses Side (Debit) */}
            <div className="space-y-3">
              <h4 className="text-sm font-medium text-muted-foreground border-b pb-2">Assets & Expenses (Debit)</h4>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span>Stock On The Way</span>
                  <span className="font-medium">{formatCurrency(importCycleData.components.stockOtwValue)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Cash Balance</span>
                  <span className="font-medium">{formatCurrency(importCycleData.components.cashBalance)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Bank Balance</span>
                  <span className="font-medium">{formatCurrency(importCycleData.components.bankBalance)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Stock on Floor</span>
                  <span className="font-medium">{formatCurrency(importCycleData.components.stockOnFloorValue)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Direct Expenses</span>
                  <span className="font-medium">{formatCurrency(importCycleData.components.directExpenseBalance)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Indirect Expenses</span>
                  <span className="font-medium">{formatCurrency(importCycleData.components.indirectExpenseBalance)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Cost of Goods Sold</span>
                  <span className="font-medium">{formatCurrency(importCycleData.components.cogsBalance)}</span>
                </div>
                <div className="flex justify-between text-blue-600">
                  <span>Payroll Expense</span>
                  <span className="font-medium">{formatCurrency(importCycleData.components.payrollExpenseBalance)}</span>
                </div>
                <div className="flex justify-between text-blue-600">
                  <span>Salary Advances</span>
                  <span className="font-medium">{formatCurrency(importCycleData.components.salaryAdvancesBalance)}</span>
                </div>
                <div className="border-t pt-2 mt-2 flex justify-between font-semibold">
                  <span>Total Assets/Expenses</span>
                  <span>{formatCurrency(
                    importCycleData.components.stockOtwValue +
                    importCycleData.components.cashBalance +
                    importCycleData.components.bankBalance +
                    importCycleData.components.stockOnFloorValue +
                    importCycleData.components.directExpenseBalance +
                    importCycleData.components.indirectExpenseBalance +
                    importCycleData.components.cogsBalance +
                    importCycleData.components.payrollExpenseBalance +
                    importCycleData.components.salaryAdvancesBalance
                  )}</span>
                </div>
              </div>
            </div>

            {/* Liabilities & Income Side (Credit) */}
            <div className="space-y-3">
              <h4 className="text-sm font-medium text-muted-foreground border-b pb-2">Liabilities & Income (Credit)</h4>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span>Supplier Balance</span>
                  <span className="font-medium">{formatCurrency(importCycleData.components.supplierBalance)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Duty Agent</span>
                  <span className="font-medium">{formatCurrency(importCycleData.components.dutyAgentBalance)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Transporter Agent</span>
                  <span className="font-medium">{formatCurrency(importCycleData.components.transporterAgentBalance)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Loans</span>
                  <span className="font-medium">{formatCurrency(importCycleData.components.loansBalance)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Income</span>
                  <span className="font-medium">{formatCurrency(importCycleData.components.incomeBalance)}</span>
                </div>
                <div className="flex justify-between text-blue-600">
                  <span>Payroll Liabilities</span>
                  <span className="font-medium">{formatCurrency(importCycleData.components.payrollLiabilitiesBalance)}</span>
                </div>
                <div className="border-t pt-2 mt-2 flex justify-between font-semibold">
                  <span>Total Liabilities/Income</span>
                  <span>{formatCurrency(
                    importCycleData.components.supplierBalance +
                    importCycleData.components.dutyAgentBalance +
                    importCycleData.components.transporterAgentBalance +
                    importCycleData.components.loansBalance +
                    importCycleData.components.incomeBalance +
                    importCycleData.components.payrollLiabilitiesBalance
                  )}</span>
                </div>
              </div>
            </div>
