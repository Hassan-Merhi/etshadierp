...
interface BankAccount { id: number; accountNumber: string; bankName: string; accountName: string; balance: string; }
interface LedgerAccount { id: number; code: string; name: string; accountType: string; }
interface Supplier { id: number; code: string; legalName: string; }
interface Customer { id: number; code: string; legalName: string; openingBalance?: string; }
interface Employee { id: number; code: string; firstName: string; lastName: string; openingBalance?: string; }
interface FixedAsset { id: number; code: string; name: string; openingBalance?: string; }
interface FactorySupplierBasic { id: number; name: string; parentId: number | null; }
interface Account {
...
export function JournalForm({ voucherIdToEdit, isPOS }: JournalFormProps) {
  ...
  const { data: supplierSearchResults = [] } = useQuery<Supplier[]>({
    ...
    staleTime: 30 * 1000,
    queryFn: async () => {
      const res = await fetch(`/api/suppliers?search=${encodeURIComponent(debouncedAccountSearch)}&limit=50`, { credentials: "include" });
  ...
  const { data: customerSearchResults = [] } = useQuery<Customer[]>({
    ...
    staleTime: 30 * 1000,
    queryFn: async () => {
      const res = await fetch(`/api/customers?search=${encodeURIComponent(debouncedAccountSearch)}&limit=50`, { credentials: "include" });
  ...
  const allAccounts = useMemo<CombinedAccount[]>(() => {
    const accounts: CombinedAccount[] = [
      ...
      ...supplierSearchResults.filter(s => !suppliers.find(p => p.id === s.id)).map((s) => ({ type: "supplier" as const, id: s.id, name: s.legalName, code: s.code })),
      ...employees.map((e) => ({ type: "employee" as const, id: e.id, name: `${e.firstName} ${e.lastName}`, code: e.code, openingBalance: e.openingBalance })),
      ...fixedAssets.map((f) => ({ type: "fixedAsset" as const, id: f.id, name: f.name, code: f.code, openingBalance: f.openingBalance })),
      ...customers.map((c: any) => ({ type: "customer" as const, id: c.id, name: c.legalName, code: c.code, openingBalance: c.openingBalance })),
      ...customerSearchResults.filter((c: any) => !customers.find((p: any) => p.id === c.id)).map((c: any) => ({ type: "customer" as const, id: c.id, name: c.legalName, code: c.code })),
      ...factorySuppliersList.map((s) => ({ type: "factorySupplier" as const, id: s.id, name: s.name, code: String(s.id) })),
    ];
  ...
  const filteredJournalAccounts = useMemo(() => {
    ...
    return allAccounts.filter((acc) => (acc.name || '').toLowerCase().includes(term) || (acc.code || '').toLowerCase().includes(term));
  ...
  const handleJournalTypeChange = (index: number, newType: "DR" | "CR") => {
    ...
    if (newType === "CR") {
      ...
      const totalDebits = updatedEntries.reduce((sum, entry) => sum + (entry.type === "DR" ? (parseFloat(entry.amount) || 0) : 0), 0);
      const otherCredits = updatedEntries.reduce((sum, entry, i) => i !== index && entry.type === "CR" ? sum + (parseFloat(entry.amount) || 0) : sum, 0);
  ...
  useEffect(() => {
    if (!voucherToEdit || voucherToEdit.voucherType !== "Journal" || !Array.isArray(voucherToEdit.entries) || voucherToEdit.entries.length === 0 || allAccounts.length === 0) return;
    ...
    journalForm.reset({
      ...
      entries: formEntries.length > 0 ? formEntries : [{ type: "DR", accountType: "ledger", accountId: 0, accountName: "", amount: "" }],
    ...
    onError: (error: any, formData: JournalFormData) => {
      if (error.name === "OfflineQueued") {
        const syntheticVoucher: any = {
          id: -Date.now(), voucherNumber: "PENDING", voucherType: "Journal",
          voucherDate: format(formData.voucherDate, "yyyy-MM-dd"),
          description: formData.notes || "Journal (pending sync)",
          totalAmount: formData.entries.filter((e: any) => e.type === "DR" && parseFloat(e.amount || "0") > 0).reduce((sum: number, e: any) => sum + parseFloat(e.amount || "0"), 0).toFixed(2),
          optional: formData.optional || false, createdAt: new Date().toISOString(),
        };
        queryClient.setQueriesData({ queryKey: ["/api/vouchers"] }, (old: any) => Array.isArray(old) ? [syntheticVoucher, ...old] : old);
        discardJournalDraft();
        journalForm.reset({ voucherDate: new Date(), entries: [{ type: "DR", accountType: "ledger", accountId: 0, accountName: "", amount: "" }], notes: "", optional: false });
        return;
      }
      if ((error as any)._handledGlobally) return;
      toast({ title: "Error", description: error.message || `Failed to ${voucherIdToEdit ? "update" : "create"} journal voucher`, variant: "destructive" });
    },
  });
  ...
  const handleExportJournalVoucher = async (detailed: boolean) => {
    ...
    if (detailed) {
      const exportData = validEntries.map((entry: any) => ({
        "Voucher Type": "Journal", "Date": voucherDate, "DR/CR": entry.type,
        "Account": entry.accountName || "", "Account Type": entry.accountType || "",
        "Amount": parseFloat(entry.amount).toFixed(2), "Notes": formData.notes || "",
      ...
      const exportData = [{
        "Voucher Type": "Journal", "Date": voucherDate, "Total Debit": totalDr.toFixed(2),
        "Total Credit": totalCr.toFixed(2), "Number of Entries": validEntries.length,
  ...
  const handleJournalKeyDown = (e: React.KeyboardEvent, rowIndex: number, fieldName: "type" | "account" | "amount") => {
    ...
    if (fieldName === "amount") {
      if (e.key === "ArrowUp") {
        e.preventDefault();
        if (rowIndex > 0) setTimeout(() => { const el = document.querySelector(`[data-testid="input-journal-amount-${rowIndex - 1}"]`) as HTMLInputElement; if (el) { el.focus(); el.select(); } }, 50);
        return;
      } else if (e.key === "ArrowDown") {
        e.preventDefault();
        if (rowIndex < journalFields.length - 1) setTimeout(() => { const el = document.querySelector(`[data-testid="input-journal-amount-${rowIndex + 1}"]`) as HTMLInputElement; if (el) { el.focus(); el.select(); } }, 50);
        return;
      } else if (e.key === "ArrowLeft") {
        e.preventDefault();
        setTimeout(() => { const el = document.querySelector(`[data-testid="input-journal-account-${rowIndex}"]`) as HTMLInputElement; if (el) el.focus(); }, 50);
        return;
      } else if (e.key === "ArrowRight") {
        e.preventDefault();
        if (rowIndex < journalFields.length - 1) setTimeout(() => { const el = document.querySelector(`[data-testid="input-journal-type-${rowIndex + 1}"]`) as HTMLElement; if (el) el.focus(); }, 50);
        return;
      }
    }
    if (fieldName === "amount" && e.key === "Tab" && !e.shiftKey) {
      ...
      setTimeout(() => { const el = document.querySelector(`[data-testid="input-journal-type-${rowIndex + 1}"]`) as HTMLElement; if (el) el.focus(); }, 100);
    }
    if (fieldName === "amount" && e.key === "Enter") {
      ...
      if (isLastRow) {
        ...
        setTimeout(() => { const el = document.querySelector(`[data-testid="input-journal-type-${rowIndex + 1}"]`) as HTMLElement; if (el) el.focus(); }, 50);
      }
    }
  };
  ...
  const handleAutoCreateAccount = async (name: string): Promise<Account | null> => {
    ...
    try {
      ...
      const response = await fetch("/api/ledger-accounts", {
        ...
        credentials: "include", body: JSON.stringify({ name: name.trim(), accountType: "Indirect Expense", companyId: selectedCompany.id }),
      });
      if (!response.ok) { const err = await response.json(); throw new Error(err.message || "Failed to create account"); }
    ...
    <div className="space-y-4">
            ...
            <Form {...journalForm}>
              <form noValidate onSubmit={journalForm.handleSubmit(onJournalSubmit)} className="space-y-5">
                <div className="flex items-center justify-between gap-4 flex-wrap">
                  <div>
                    <p className="text-sm font-semibold">{voucherToEdit?.voucherNumber ? `#${voucherToEdit.voucherNumber}` : "New Journal Entry"}</p>
                        ...
                        <div className="flex items-start gap-2">
                          ...
                          <div className="flex-1 min-w-0">
                            <Input
                              ...
                              onChange={(e) => { setJournalAccountSearchTerm(e.target.value); setJournalAccountHighlightedIndex(0); }}
                              onFocus={() => { setAccountPickersActivated(true); setAccountPickersNeeded(true); setActiveJournalRow(index); setShowAccountSidebar(true); setJournalAccountSearchTerm(""); }}
                              onBlur={() => { setTimeout(() => { if (activeJournalRow === index) { setJournalAccountSearchTerm(""); setActiveJournalRow(null); } }, 200); }}
                            ...
                            {activeJournalRow === index && filteredJournalAccounts.length > 0 && (
                              <div className="mt-1 border rounded-md bg-popover shadow-md max-h-44 overflow-y-auto z-20 relative">
                                {filteredJournalAccounts.slice(0, 10).map((account: any) => (
                                  <button key={`${account.type}-${account.id}`} type="button"
                                    className="w-full text-left px-3 py-2.5 text-sm hover-elevate border-b last:border-b-0"
                                    onMouseDown={(e) => { e.preventDefault(); handleJournalAccountSelect(account); setShowAccountSidebar(false); }}
                                  >
                          ...
                          render={({ field }) => (
                            <FormItem>
                              <div className="flex items-center gap-3">
                                ...
                                <FormControl>
                                  <Input {...field} type="number" step="0.01" min="0" placeholder="0.00" className="font-mono text-right"
                                    data-testid={`input-journal-amount-mobile-${index}`}
                                    onBlur={(e) => { const v = Number(e.target.value); if (!isNaN(v) && v > 0 && selectedCurrency !== "USD") journalForm.setValue(`entries.${index}.amount`, convertToUSD(v).toFixed(2)); }}
                                  />
                                </FormControl>
                              </div>
                              <FormMessage />
                          ...
                          render={({ field }) => (
                            <FormItem>
                              <div className="flex items-center gap-3">
                                ...
                                <FormControl>
                                  <Input {...field} value={field.value ?? ""} placeholder="Optional note for this entry" className="text-sm" data-testid={`input-journal-narration-mobile-${index}`} />
                                </FormControl>
                              </div>
                            </FormItem>
                          )}
                      ...
                      {journalFields.map((field, index) => {
                        ...
                        return (
                          <tr key={field.id} className="border-t hover:bg-muted/20 transition-colors">
                                ...
                                render={({ field }) => (
                                  <FormItem>
                                    <Select value={field.value} onValueChange={(v: "DR" | "CR") => handleJournalTypeChange(index, v)}>
                                      <FormControl>
                                        <SelectTrigger
                                          className={cn("w-20 text-center font-semibold border",
                                            field.value === "DR"
                                          ...
                                          onKeyDown={(e) => {
                                            if (e.key === "Tab" && !e.shiftKey) { e.preventDefault(); setTimeout(() => { const el = document.querySelector(`[data-testid="input-journal-account-${index}"]`) as HTMLInputElement; if (el) el.focus(); }, 50); }
                                            else if (e.key === "ArrowRight") { e.preventDefault(); setTimeout(() => { const el = document.querySelector(`[data-testid="input-journal-account-${index}"]`) as HTMLInputElement; if (el) el.focus(); }, 50); }
                                            else if (e.key === "ArrowLeft") { e.preventDefault(); setTimeout(() => { const el = document.querySelector(`[data-testid="input-journal-amount-${index}"]`) as HTMLInputElement; if (el) { el.focus(); el.select(); } }, 50); }
                                            else if (e.key === "ArrowUp" && index > 0) { e.preventDefault(); setTimeout(() => { const el = document.querySelector(`[data-testid="input-journal-type-${index - 1}"]`) as HTMLElement; if (el) el.focus(); }, 50); }
                                            else if (e.key === "ArrowDown" && index < journalFields.length - 1) { e.preventDefault(); setTimeout(() => { const el = document.querySelector(`[data-testid="input-journal-type-${index + 1}"]`) as HTMLElement; if (el) el.focus(); }, 50); }
                                          }}
                                        >
                                          <SelectValue placeholder="DR" />
                                        </SelectTrigger>
                                        ...
                                        <Input
                                          ...
                                          onKeyDown={(e) => {
                                            if (showAccountSidebar) {
                                              if (e.key === "ArrowUp") { e.preventDefault(); setJournalAccountHighlightedIndex(p => p > 0 ? p - 1 : Math.max(0, filteredJournalAccounts.length - 1)); return; }
                                              if (e.key === "ArrowDown") { e.preventDefault(); setJournalAccountHighlightedIndex(p => p < filteredJournalAccounts.length - 1 ? p + 1 : 0); return; }
                                              if (e.key === "Enter") { e.preventDefault(); e.stopPropagation(); const sel = filteredJournalAccounts[journalAccountHighlightedIndex]; if (sel) { handleJournalAccountSelect(sel); setShowAccountSidebar(false); } return; }
                                            }
                                            if (e.key === "Tab" && !e.shiftKey) { e.preventDefault(); setTimeout(() => { const el = document.querySelector(`[data-testid="input-journal-amount-${index}"]`) as HTMLInputElement; if (el) { el.focus(); el.select(); } }, 50); }
                                            else if (e.key === "ArrowUp" && index > 0) { e.preventDefault(); setTimeout(() => { const el = document.querySelector(`[data-testid="input-journal-account-${index - 1}"]`) as HTMLInputElement; if (el) el.focus(); }, 50); }
                                            else if (e.key === "ArrowDown" && index < journalFields.length - 1) { e.preventDefault(); setTimeout(() => { const el = document.querySelector(`[data-testid="input-journal-account-${index + 1}"]`) as HTMLInputElement; if (el) el.focus(); }, 50); }
                                            else if (e.key === "ArrowRight") { e.preventDefault(); setTimeout(() => { const el = document.querySelector(`[data-testid="input-journal-amount-${index}"]`) as HTMLInputElement; if (el) { el.focus(); el.select(); } }, 50); }
                                            else if (e.key === "ArrowLeft") { e.preventDefault(); setTimeout(() => { const el = document.querySelector(`[data-testid="input-journal-type-${index}"]`) as HTMLElement; if (el) el.focus(); }, 50); }
                                          }}
                                        />
                                        {entry?.accountId > 0 && (
                            ...
                            <td className="p-2">
                              <FormField
                                ...
                                render={({ field }) => (
                                  <FormItem>
                                    <FormControl>
                                      <Input {...field} value={field.value ?? ""} placeholder="Optional note…" className="text-sm h-8" data-testid={`input-journal-narration-${index}`} />
                                    </FormControl>
                                  </FormItem>
                                )}
                              />
                            ...
                            <td className="p-2">
                              {journalFields.length > 1 && (
                                <Button type="button" variant="ghost" size="icon" onClick={() => removeJournal(index)} data-testid={`button-journal-remove-${index}`}>
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  <div className={cn("rounded-lg border px-4 py-2 flex items-center gap-2",
                    Math.abs(totalDebit - totalCredit) <= 0.01
                      ? "bg-emerald-50 border-emerald-200 dark:bg-emerald-900/20 dark:border-emerald-800"
                      : "bg-red-50 border-red-200 dark:bg-red-900/20 dark:border-red-800"
                  )}>
                    {Math.abs(totalDebit - totalCredit) <= 0.01 ? (
                      <><CheckCircle className="h-4 w-4 text-emerald-600 dark:text-emerald-400 shrink-0" /><span className="text-sm font-medium text-emerald-700 dark:text-emerald-300">Balanced</span></>
                    ) : (
                      <><AlertTriangle className="h-4 w-4 text-red-600 dark:text-red-400 shrink-0" /><span className="text-sm font-medium text-red-700 dark:text-red-300">Off by {formatAmount(Math.abs(totalDebit - totalCredit))}</span></>
                    )}
                  </div>
                </div>
                  ...
                  <div className="flex items-center gap-2">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem onClick={() => handleExportJournalVoucher(false)} data-testid="export-journal-summary">Summary Export</DropdownMenuItem>
                        <DropdownMenuItem onClick={() => handleExportJournalVoucher(true)} data-testid="export-journal-detailed">Detailed Export</DropdownMenuItem>
                      </DropdownMenuContent>
                    </DropdownMenu>
                    <Button type="submit" disabled={journalMutation.isPending || Math.abs(totalDebit - totalCredit) > 0.01} data-testid="button-save-journal-voucher">
            ...
            <div className="p-4 border-b">
              <div className="flex items-center justify-between gap-2 mb-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => setShowAccountSidebar(false)} className="text-xs text-muted-foreground hover:text-foreground" data-testid="button-close-account-sidebar">✕</button>
                </div>
              </div>
              <div className="relative">
                {isAutoCreating ? <Loader2 className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground animate-spin" /> : <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />}
                <Input
                  placeholder="Search by name or code..."
                  value={journalAccountSearchTerm}
                  onChange={(e) => { setJournalAccountSearchTerm(e.target.value); setJournalAccountHighlightedIndex(0); }}
                  onKeyDown={(e) => {
                    if (e.key === "ArrowDown") { e.preventDefault(); if (filteredJournalAccounts.length > 0) setJournalAccountHighlightedIndex(p => Math.min(p + 1, filteredJournalAccounts.length - 1)); }
                    else if (e.key === "ArrowUp") { e.preventDefault(); if (filteredJournalAccounts.length > 0) setJournalAccountHighlightedIndex(p => Math.max(p - 1, 0)); }
                    else if (e.key === "Enter") { if (filteredJournalAccounts.length > 0 && journalAccountHighlightedIndex >= 0 && journalAccountHighlightedIndex < filteredJournalAccounts.length) { e.preventDefault(); handleJournalAccountSelect(filteredJournalAccounts[journalAccountHighlightedIndex]); } }
                  }}
                  className="pl-9"
              ...
              <div className="space-y-1">
                {filteredJournalAccounts.length === 0 ? (
                  ...
                  filteredJournalAccounts.map((account, idx) => {
                    ...
                    const isSelected = journalEntries[activeJournalRow ?? 0]?.accountId === account.id && journalEntries[activeJournalRow ?? 0]?.accountType === account.type;
                    ...
                    return (
                      <button
                        ...
                        className={cn("w-full text-left px-3 py-2 rounded-md text-sm hover-elevate active-elevate-2 flex items-center justify-between gap-2", isHighlighted && "bg-accent", isSelected && "bg-primary/10")}
      ...
      <AlertDialog open={!!waPendingPrompt} onOpenChange={(open) => { if (!open) setWaPendingPrompt(null); }}>
        <AlertDialogContent data-testid="dialog-whatsapp-prompt">
          ...
          <AlertDialogFooter>
            <AlertDialogCancel data-testid="button-whatsapp-skip" onClick={() => setWaPendingPrompt(null)}>Skip for Now</AlertDialogCancel>
            <AlertDialogAction data-testid="button-whatsapp-send" disabled={sendWaStatementMutation.isPending}
