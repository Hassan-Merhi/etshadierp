            ...
            <>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription className="text-xs">50/50 split — supplier share is informational only, not posted to Supplier Cash Payable</CardDescription>
                </CardHeader>
        ...
        <TabsContent value="sales-form" className="mt-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription className="text-xs">Downloads the same workbook format as the supplier sales form — Costing, Sales, ENTRY, Summary, and Summary-Itemwise sheets filled with current data.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex items-end gap-3 flex-wrap">
                <div>
                  <label className="text-xs text-muted-foreground">From</label>
                  <Input type="date" value={exportFrom} onChange={(e) => setExportFrom(e.target.value)} className="mt-1 w-36" data-testid="input-sp-export-from" />
                </div>
                <div>
                  <label className="text-xs text-muted-foreground">To</label>
                  <Input type="date" value={exportTo} onChange={(e) => setExportTo(e.target.value)} className="mt-1 w-36" data-testid="input-sp-export-to" />
                </div>
                <div className="min-w-40">
                  ...
                  <Select value={exportLocationId} onValueChange={setExportLocationId} data-testid="select-sp-export-location">
                    <SelectTrigger className="mt-1" data-testid="select-sp-export-location-trigger"><SelectValue placeholder="All locations" /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="all">All locations</SelectItem>
                      {locations.map((l: any) => <SelectItem key={l.id} value={String(l.id)} data-testid={`option-location-${l.id}`}>{l.name}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
                <div className="min-w-48">
                  <label className="text-xs text-muted-foreground">Opening Cash Account (optional)</label>
                  <Select value={exportCashAccountId} onValueChange={setExportCashAccountId} data-testid="select-sp-export-cash-account">
                    <SelectTrigger className="mt-1" data-testid="select-sp-export-cash-account-trigger"><SelectValue placeholder="No account (manual entry)" /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="none">No account (manual entry)</SelectItem>
                      {accounts.map((a: any) => <SelectItem key={a.id} value={String(a.id)} data-testid={`option-cash-account-${a.id}`}>{a.name}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-3 pt-1">
                <Button onClick={handleExportSalesForm} disabled={exporting || exportingV2 || !exportFrom || !exportTo} variant="outline" data-testid="button-sp-export-sales-form">
                  {exporting ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" /> Generating…</> : <><FileSpreadsheet className="h-4 w-4 mr-2" /> Export Template Form</>}
                </Button>
                <Button onClick={handleExportSalesFormV2} disabled={exporting || exportingV2 || !exportFrom || !exportTo} data-testid="button-sp-export-sales-form-v2">
                  {exportingV2 ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" /> Generating…</> : <><FileSpreadsheet className="h-4 w-4 mr-2" /> Export System Sales Form</>}
                </Button>
              </div>

              <div className="space-y-1">
                <p className="text-xs text-muted-foreground"><span className="font-medium text-foreground">Export Template Form</span> — fills the original supplier template with your data (18-day max, formula-based).</p>
                <p className="text-xs text-muted-foreground"><span className="font-medium text-foreground">Export System Sales Form</span> — clean from-scratch workbook built from your live system data. Opening &amp; closing stock match the Location Inventory page. Supports any date range. 5 sheets, no Ageing: ENTRY, Summary (visible) + Costing, Sales, Summary-Itemwise (hidden). Optionally select an Opening Cash Account to auto-fill day-1 cash from the ledger balance.</p>
              </div>
            </CardContent>
          </Card>
        </TabsContent>
