...
function StockInSalesReportSummary() {
  ...
  const exportExcel = async () => {
    ...
    try {
      ...
      sheet.columns = [
        ...
        { header: "Adjustments", key: "stockAdjustmentQty", width: 16 },
  ...
  return (
    <div className="container mx-auto space-y-6 p-6">
      ...
      {reconciliation && (
        <div className={`rounded-xl border p-4 ${reconciliation.matches ? "bg-emerald-500/5" : "bg-red-500/5"}`} data-testid="stock-in-sales-reconciliation">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <div className="flex items-center gap-2">
                {reconciliation.matches ? <TrendingUp className="h-4 w-4 text-emerald-600" /> : <TrendingDown className="h-4 w-4 text-red-600" />}
                <p className="text-sm font-semibold">Inventory Reconciliation</p>
              </div>
              <p className="mt-1 text-xs text-muted-foreground">
                Expected closing stock is compared with Location Inventory as of {reconciliation.asOfDate}.
              </p>
            </div>
            <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm">
              <div><span className="text-muted-foreground">Expected</span> <span className="ml-1 font-mono font-semibold">{formatNumber(reconciliation.expectedClosingQty, 3)}</span></div>
              <div><span className="text-muted-foreground">Actual</span> <span className="ml-1 font-mono font-semibold">{formatNumber(reconciliation.actualStockQty, 3)}</span></div>
              <div><span className="text-muted-foreground">Difference</span> <span className={`ml-1 font-mono font-semibold ${reconciliation.matches ? "text-emerald-600" : "text-red-600"}`}>{formatSignedQty(reconciliation.differenceQty)}</span></div>
              <div><span className="text-muted-foreground">Actual Value</span> <span className="ml-1 font-mono font-semibold">{formatAmount(reconciliation.actualStockValue)}</span></div>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center gap-2 print:hidden">
        ...
        <Input placeholder="Search..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} className="h-9 w-44" />
            ...
            <Table className="min-w-[1900px]">
              ...
              <TableBody>
                {isLoading ? (
                  Array.from({ length: 6 }).map((_, index) => (
                    <TableRow key={index}>
                      {Array.from({ length: 15 }).map((__, cellIndex) => (
                        <TableCell key={cellIndex}><Skeleton className={`h-4 ${cellIndex === 0 ? "w-24" : "ml-auto w-20"}`} /></TableCell>
                      ))}
                    </TableRow>
                  ))
                ) : isError ? (
                  ...
                  <TableRow><TableCell colSpan={15} className="py-10 text-center text-sm text-muted-foreground">No stock or sales activity found for these filters.</TableCell></TableRow>
                ) : (
                  <>
                    {rows.map((row) => {
                      ...
                      return (
                        <TableRow key={row.periodKey} className="cursor-pointer hover:bg-muted/40" onClick={() => openDetail(row)}>
                          ...
                          <TableCell className={`text-right font-mono ${row.stockAdjustmentQty < 0 ? "text-red-600" : row.stockAdjustmentQty > 0 ? "text-emerald-600" : ""}`}>{formatSignedQty(row.stockAdjustmentQty)}</TableCell>
                          <TableCell className="text-right font-mono">{formatNumber(row.totalAvailableQty, 3)}</TableCell>
                          <TableCell className="text-right font-mono">{formatNumber(row.stockOutQty, 3)}</TableCell>
                          <TableCell className="text-right font-mono font-semibold">{formatNumber(row.closingStockQty, 3)}</TableCell>
                          <TableCell className="text-right font-mono font-semibold">{formatAmount(row.closingStockValue)}</TableCell>
                          <TableCell className="text-right font-mono">{formatAmount(row.totalSales)}</TableCell>
                          <TableCell className="text-right font-mono text-muted-foreground">{formatAmount(row.costOfSales)}</TableCell>
                          <TableCell className={`text-right font-mono font-semibold ${profitClass}`}>{formatSignedAmount(row.costProfit)}</TableCell>
                          <TableCell className={`text-right font-mono font-semibold ${profitClass}`}>{formatSignedAmount(row.avgProfitPerBale)}</TableCell>
                    ...
                    <TableRow className="bg-muted/40 font-semibold hover:bg-muted/40">
                      ...
                      <TableCell className="text-right font-mono">{formatSignedQty(summary.stockAdjustmentQty)}</TableCell>
                      <TableCell className="text-right font-mono">{formatNumber(summary.totalAvailableQty, 3)}</TableCell>
                      ...
                      <TableCell className="text-right font-mono">{formatSignedAmount(summary.avgProfitPerBale)}</TableCell>
                      <TableCell />
                    </TableRow>
                  </>
                )}
