    ...
    <Popover>
      ...
      <PopoverContent className="w-56 p-2" align="start">
        <div className="max-h-72 space-y-1 overflow-y-auto">
          ...
          {items.length === 0 && (
            <p className="px-2 py-3 text-xs text-muted-foreground">{tUi("no.options.available")}</p>
          )}
        </div>
      </PopoverContent>
    </Popover>
    ...
    <div className="container mx-auto space-y-6 p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex items-start gap-2">
          ...
          <div>
            <PageHeader title={tUi("stock.in.sales.report")} />
      ...
      <div className="flex flex-wrap items-center gap-2 print:hidden">
        ...
        {isFetching && !isLoading && (
          <RefreshCw className="h-4 w-4 animate-spin text-muted-foreground" aria-label={tUi("refreshing")} />
        )}
      </div>
            ...
            <Table className="min-w-[1180px]">
              <TableHeader>
                <TableRow className="bg-muted/40 hover:bg-muted/40">
                  ...
                  <TableHead className="text-right">{tUi("stock.in.qty")}</TableHead>
                  <TableHead className="text-right">{tUi("stock.in.value")}</TableHead>
                  <TableHead className="text-right">{tUi("avg.in.rate")}</TableHead>
                  <TableHead className="text-right">{tUi("stock.out.qty")}</TableHead>
                  <TableHead className="text-right">{tUi("total.sales")}</TableHead>
                  <TableHead className="text-right">{tUi("cost.2")}</TableHead>
                  <TableHead className="text-right">{tUi("cost.profit")}</TableHead>
                  <TableHead className="text-right">{tUi("avg.profit.bale")}</TableHead>
                ...
                {isLoading ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={10}>
                      <div className="flex flex-col items-center gap-3 py-10 text-center">
                        ...
                        <p className="text-sm font-medium">{tUi("unable.to.load.the.report")}</p>
                  ...
                  <TableRow>
                    <TableCell colSpan={10} className="py-10 text-center text-sm text-muted-foreground">
                      {tUi("no.stock.in.or.sales.activity.found.try.adjustin")}
                    </TableCell>
                  </TableRow>
                ) : (
                  <>
