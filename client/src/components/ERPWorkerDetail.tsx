...
export function ERPWorkerDetail({ worker, onBack, onEdit }: Props) {
  ...
  const createAdvanceMutation = useMutation({
    mutationFn: async (data: { employeeId: number; companyId: number; advanceDate: string; amount: string; notes?: string }) => {
  ...
  const uploadDocMutation = useMutation({
    mutationFn: async (data: { fileName: string; fileType: string; fileSize: number; fileData: string; description?: string }) => {
      return apiRequest("POST", `/api/employees/${worker.id}/docs`, { ...data, companyId: selectedCompany?.id, employeeId: worker.id });
    },
    onSuccess: () => {
  ...
  const infoRow = (label: string, value: string | number | boolean | undefined | null, testId?: string) => (
    <div className="flex justify-between py-1.5 border-b last:border-0">
      ...
      <span className="text-xs font-medium text-right max-w-[55%] truncate" data-testid={testId}>{value === undefined || value === null || value === "" ? "—" : String(value)}</span>
    </div>
  );
  ...
  const statementRows: LedgerRow[] = (() => {
    ...
    // Opening balance row
    if (openingBalance !== 0) {
      rows.push({ date: worker.joinDate?.slice(0, 10) || "—", type: "Opening Balance", ref: "—", description: "Opening balance", debit: openingBalance > 0 ? openingBalance : 0, credit: openingBalance < 0 ? Math.abs(openingBalance) : 0 });
    }
  ...
  const advancesOutstanding = advances.filter(a => !a.fullyPaid).reduce((s, a) => s + parseFloat(a.remainingBalance || "0"), 0);
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!ALLOWED_TYPES.includes(file.type)) {
      toast({ title: "Invalid file type", description: "Allowed: PDF, JPG, PNG, WEBP, DOC, DOCX, XLS, XLSX, TXT", variant: "destructive" });
      return;
    }
    setPendingFile(file);
  ...
  const handleUploadConfirm = () => {
    ...
    reader.onload = (e) => {
      ...
      uploadDocMutation.mutate({ fileName: pendingFile.name, fileType: pendingFile.type, fileSize: pendingFile.size, fileData, description: uploadDesc });
    };
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-lg font-semibold" data-testid="text-worker-detail-name">{fullName}</h2>
          <p className="text-sm text-muted-foreground">{worker.code} · {worker.department || "—"}</p>
        </div>
      </div>
          ...
          <Card>
            <CardContent className="p-5 flex flex-col items-center gap-3">
              ...
              <div className="text-center">
                <p className="font-semibold text-base" data-testid="text-card-worker-name">{fullName}</p>
                <p className="text-xs text-muted-foreground mt-0.5" data-testid="text-card-worker-code">{worker.code}</p>
              </div>
        ...
        <div className="flex-1 min-w-0">
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="mb-4 flex-wrap">
              <TabsTrigger value="profile" data-testid="tab-erp-profile">Profile</TabsTrigger>
              <TabsTrigger value="statement" data-testid="tab-erp-statement">Statement</TabsTrigger>
              <TabsTrigger value="advances" data-testid="tab-erp-advances">Advances</TabsTrigger>
              <TabsTrigger value="docs" data-testid="tab-erp-docs">Docs</TabsTrigger>
            </TabsList>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Card>
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm flex items-center gap-2"><User className="h-3.5 w-3.5" /> Personal</CardTitle>
                  </CardHeader>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <Card>
                  <CardContent className="p-4 text-center">
                    ...
                    <p className={`text-xl font-bold ${netBalance >= 0 ? "text-green-700 dark:text-green-400" : "text-red-700 dark:text-red-400"}`} data-testid="stat-erp-net-balance">{fmt(netBalance)}</p>
                ...
                <Card>
                  <CardContent className="p-4 text-center">
                    ...
                    <p className="text-xl font-bold text-green-700 dark:text-green-400" data-testid="stat-erp-total-earned">{fmt(totalDeposits)}</p>
                ...
                <Card>
                  <CardContent className="p-4 text-center">
                    ...
                    <p className="text-xl font-bold text-blue-700 dark:text-blue-400" data-testid="stat-erp-total-paid">{fmt(totalWithdrawals)}</p>
                ...
                <Card>
                  <CardContent className="p-4 text-center">
                    ...
                    <p className={`text-xl font-bold ${advancesOutstanding > 0 ? "text-red-700 dark:text-red-400" : "text-muted-foreground"}`} data-testid="stat-erp-advances-left">{fmt(advancesOutstanding)}</p>
                        ...
                        <TableBody>
                          {statementRows.map((row, i) => (
                            <TableRow key={i} data-testid={`row-statement-${i}`}>
                              ...
                              <TableCell className={`text-right font-mono text-xs font-semibold ${row.balance >= 0 ? "text-green-700 dark:text-green-400" : "text-red-700 dark:text-red-400"}`}>
                                {fmt(row.balance)}
                              </TableCell>
                              <TableCell>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                <Card>
                  <CardContent className="p-4 text-center">
                    ...
                    <p className="text-lg font-bold" data-testid="stat-adv-total">{fmt(totalAdvancesGiven)}</p>
                  </CardContent>
                </Card>
                <Card>
                  <CardContent className="p-4 text-center">
                    <p className="text-xs text-muted-foreground mb-1">Outstanding</p>
                    <p className={`text-lg font-bold ${advancesOutstanding > 0 ? "text-red-700 dark:text-red-400" : "text-muted-foreground"}`} data-testid="stat-adv-outstanding">{fmt(advancesOutstanding)}</p>
                  </CardContent>
                </Card>
                <Card>
                  <CardContent className="p-4 text-center">
                    <p className="text-xs text-muted-foreground mb-1">Unpaid Count</p>
                    <p className="text-lg font-bold" data-testid="stat-adv-count">{advances.filter(a => !a.fullyPaid).length}</p>
                  </CardContent>
                </Card>
              </div>
                        ...
                        <TableBody>
                          {advances.map((adv) => (
                            <TableRow key={adv.id} data-testid={`row-advance-${adv.id}`}>
                              <TableCell className="text-xs whitespace-nowrap">{adv.advanceDate?.slice(0, 10)}</TableCell>
                              <TableCell className="text-right font-mono text-xs">{fmt(adv.amount)}</TableCell>
                              <TableCell className="text-right font-mono text-xs">{fmt(adv.remainingBalance)}</TableCell>
                              <TableCell className="text-xs max-w-[120px] truncate">{adv.notes || "—"}</TableCell>
                              <TableCell>
                                <Badge variant="outline" className={`text-xs ${adv.fullyPaid ? "text-green-700 dark:text-green-400" : "text-amber-700 dark:text-amber-400"}`}>
                                  {adv.fullyPaid ? "Repaid" : "Outstanding"}
                                </Badge>
                              </TableCell>
                              <TableCell className="text-right">
                                <div className="flex justify-end gap-1">
                                  <Button size="icon" variant="ghost" onClick={() => { setEditAdvance(adv); setAdvanceAmount(adv.amount); setAdvanceDate(adv.advanceDate?.slice(0, 10) || ""); setAdvanceNotes(adv.notes || ""); setAdvanceDialogOpen(true); }} data-testid={`button-edit-advance-${adv.id}`}>
                                    <Pencil className="h-3.5 w-3.5" />
                                  </Button>
                                  <Button size="icon" variant="ghost" onClick={() => deleteAdvanceMutation.mutate(adv.id)} data-testid={`button-delete-advance-${adv.id}`}>
                      ...
                      <CardContent className="p-3 flex items-center justify-between gap-3">
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-xs text-muted-foreground">{formatBytes(doc.fileSize)} · {doc.uploadedAt?.slice(0, 10)}</p>
                            {doc.description && <p className="text-xs text-muted-foreground truncate">{doc.description}</p>}
                          </div>
                        </div>
                        <div className="flex items-center gap-1 flex-shrink-0">
                          <Button size="icon" variant="ghost" onClick={() => handleView(doc)} data-testid={`button-view-doc-${doc.id}`}>
                            <Eye className="h-3.5 w-3.5" />
                          </Button>
                          <Button size="icon" variant="ghost" onClick={() => handleDownload(doc)} data-testid={`button-download-doc-${doc.id}`}>
                            <Download className="h-3.5 w-3.5" />
                          </Button>
                          <Button size="icon" variant="ghost" onClick={() => { setEditDocDialog(doc); setEditDocName(doc.fileName); setEditDocDesc(doc.description || ""); }} data-testid={`button-edit-doc-${doc.id}`}>
                            <Pencil className="h-3.5 w-3.5" />
                          </Button>
                          <Button size="icon" variant="ghost" onClick={() => deleteDocMutation.mutate(doc.id)} data-testid={`button-delete-doc-${doc.id}`}>
                            <Trash2 className="h-3.5 w-3.5 text-destructive" />
                          </Button>
                        </div>
                      </CardContent>
      ...
      {/* ── Upload dialog ── */}
      <Dialog open={uploadDialogOpen} onOpenChange={(open) => { if (!open) { setUploadDialogOpen(false); setPendingFile(null); if (fileInputRef.current) fileInputRef.current.value = ""; } }}>
        <DialogContent>
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <Textarea className="mt-1" placeholder="Describe the document..." value={uploadDesc} onChange={(e) => setUploadDesc(e.target.value)} data-testid="input-upload-desc" />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => { setUploadDialogOpen(false); setPendingFile(null); if (fileInputRef.current) fileInputRef.current.value = ""; }}>Cancel</Button>
            <Button onClick={handleUploadConfirm} disabled={!pendingFile || uploadDocMutation.isPending} data-testid="button-confirm-upload">
      ...
      {/* ── Edit doc dialog ── */}
      <Dialog open={!!editDocDialog} onOpenChange={(open) => { if (!open) setEditDocDialog(null); }}>
        <DialogContent>
          <DialogHeader><DialogTitle>Edit Document</DialogTitle></DialogHeader>
          <div className="space-y-3">
            <div>
              <Label>File Name</Label>
              <Input className="mt-1" value={editDocName} onChange={(e) => setEditDocName(e.target.value)} data-testid="input-edit-doc-name" />
            </div>
            <div>
              <Label>Description</Label>
              <Textarea className="mt-1" value={editDocDesc} onChange={(e) => setEditDocDesc(e.target.value)} data-testid="input-edit-doc-desc" />
            </div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={advanceDialogOpen} onOpenChange={setAdvanceDialogOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>{editAdvance ? "Edit Advance" : "Add Advance"}</DialogTitle></DialogHeader>
          <div className="space-y-3">
            <div>
              <Label>Date</Label>
              <Input type="date" className="mt-1" value={advanceDate} onChange={(e) => setAdvanceDate(e.target.value)} data-testid="input-advance-date" />
            </div>
            <div>
              <Label>Amount</Label>
              <Input type="number" className="mt-1" placeholder="0.00" value={advanceAmount} onChange={(e) => setAdvanceAmount(e.target.value)} data-testid="input-advance-amount" />
            </div>
            <div>
              <Label>Notes (optional)</Label>
              <Textarea className="mt-1" placeholder="Reason for advance..." value={advanceNotes} onChange={(e) => setAdvanceNotes(e.target.value)} data-testid="input-advance-notes" />
            </div>
          </div>
          <DialogFooter>
