...
describe("stock transfer revision write routes", () => {
  ...
  it("adds a newly revised item at source average cost and applies inventory exactly once", async () => {
    ...
    expect(harness.adjustInventory).toHaveBeenNthCalledWith(1, harness.db, 22, 17, -4, 6);
    expect(harness.adjustInventory).toHaveBeenNthCalledWith(2, harness.db, 18, 17, 4, 6, 12.5);
    expect(harness.updates).toContainEqual({
      table: harness.tables.vouchers,
      values: { totalAmount: "50.00" },
    });
  });
