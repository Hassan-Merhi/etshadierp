...
vi.mock("@tanstack/react-query", () => ({
  useQuery: ({ queryKey }: any) => {
    ...
    if (root === "/api/locations") return { data: [{ id: 2, name: "Source" }, { id: 8, name: "Dest" }] };
    if (root === "/api/my-locations") return { data: [{ id: 2, name: "Source" }] };
    if (root === "/api/vouchers" && queryKey?.[1] === 90) return { data: { id: 90, voucherDate: "2026-08-01", optional: false } };
    if (root === "/api/stock-transfers" && queryKey?.[1] === 90) {
      return { data: { id: 9, voucherId: 90, destinationLocationId: 8, notes: "original", items: [{ stockItemId: 7, sourceLocationId: 2, quantity: "5", rate: "10" }] } };
    }
    if (root === "/api/stock-transfers" && queryKey?.[1] === 9 && queryKey?.[2] === "revisions") {
      return { data: [{ id: 41, revisionNumber: 1, optional: true, items: [{ stockItemId: 7, stockItemName: "Item 7", sourceLocationName: "Source", originalQuantity: "5", delta: "1", newQuantity: "6" }] }] };
    }
    if (typeof root === "string" && root === "/api/locations/2/inventory") return { data: [{ stockItemId: 7, stockItemName: "Item 7", stockItemCode: "A7", averageRate: "10", quantity: "20" }] };
    return { data: [] };
  },
...
vi.mock("@/contexts/CurrencyContext", () => ({ useCurrencyContext: () => ({ formatAmount: (v: number) => `$${Number(v).toFixed(2)}` }) }));
vi.mock("@/contexts/AppModeContext", () => ({ useAppMode: () => "erp", useModePrefix: () => "" }));
vi.mock("@/lib/factoryApi", () => ({ getApiRequest: () => harness.apiRequest }));
vi.mock("@/lib/queryClient", () => ({
  queryClient: { invalidateQueries: harness.invalidateQueries, refetchQueries: harness.refetchQueries, getQueryData: harness.getQueryData },
}));
...
vi.mock("@/lib/excelHelper", () => ({ utils: { json_to_sheet: vi.fn(), book_new: vi.fn(), book_append_sheet: vi.fn() }, writeFile: vi.fn() }));
...
vi.mock("@/pages/vouchers/stock-transfer-form/dialogs/ApproveRevisionDialog", () => ({
  ApproveRevisionDialog: ({ approveRevisionTarget, approveRevisionMutation }: any) => approveRevisionTarget ? <button data-testid="confirm-approve-revision" onClick={() => approveRevisionMutation.mutate(approveRevisionTarget.id)}>Confirm approve</button> : null,
}));
vi.mock("@/pages/vouchers/stock-transfer-form/dialogs/SaveAsRevisionDialog", () => ({
  SaveAsRevisionDialog: ({ transferRevisionDialogOpen, transferRevisionNote, setTransferRevisionNote, computeTransferRevisionItems, confirmTransferSaveAsRevision }: any) =>
    transferRevisionDialogOpen ? <div data-testid="revision-dialog"><div data-testid="revision-items">{JSON.stringify(computeTransferRevisionItems())}</div><input data-testid="input-transfer-revision-note" value={transferRevisionNote} onChange={(e) => setTransferRevisionNote(e.target.value)} /><button data-testid="button-confirm-transfer-revision" onClick={confirmTransferSaveAsRevision}>Save Revision</button></div> : null,
}));
vi.mock("@/pages/vouchers/stock-transfer-form/dialogs/ImportTransferExcelDialog", () => ({ ImportTransferExcelDialog: () => null }));

vi.mock("@/components/ui/button", () => ({ Button: ({ children, variant: _v, size: _s, asChild: _a, ...props }: any) => <button {...props}>{children}</button> }));
vi.mock("@/components/ui/input", () => ({ Input: React.forwardRef<HTMLInputElement, any>((props, ref) => <input ref={ref} {...props} />) }));
vi.mock("@/components/ui/textarea", () => ({ Textarea: React.forwardRef<HTMLTextAreaElement, any>((props, ref) => <textarea ref={ref} {...props} />) }));
...
vi.mock("@/components/ui/switch", () => ({ Switch: ({ checked, onCheckedChange, ...props }: any) => <input type="checkbox" checked={!!checked} onChange={(e) => onCheckedChange?.(e.target.checked)} {...props} /> }));
...
vi.mock("@/components/ui/dropdown-menu", () => ({
  ...
  DropdownMenuContent: ({ children }: any) => <div>{children}</div>, DropdownMenuItem: ({ children, onClick, ...props }: any) => <button onClick={onClick} {...props}>{children}</button>,
}));
vi.mock("@/components/ui/alert-dialog", () => ({
  AlertDialog: ({ children, open }: any) => open ? <div>{children}</div> : null, AlertDialogAction: ({ children }: any) => <button>{children}</button>,
  AlertDialogCancel: ({ children }: any) => <button>{children}</button>, AlertDialogContent: ({ children }: any) => <div>{children}</div>, AlertDialogDescription: ({ children }: any) => <div>{children}</div>,
  AlertDialogFooter: ({ children }: any) => <div>{children}</div>, AlertDialogHeader: ({ children }: any) => <div>{children}</div>, AlertDialogTitle: ({ children }: any) => <div>{children}</div>,
}));
vi.mock("@/components/ui/form", async () => {
  const rhf = await import("react-hook-form");
  return {
    Form: ({ children }: any) => <>{children}</>, FormControl: ({ children }: any) => <>{children}</>, FormItem: ({ children }: any) => <div>{children}</div>, FormLabel: ({ children }: any) => <label>{children}</label>,
...
describe("stock transfer form revision behavior", () => {
  beforeEach(() => {
    ...
    harness.apiRequest.mockImplementation(async (_method: string, url: string) => ({ ok: true, json: async () => url.includes("/revisions") ? { id: 42 } : { id: 90 } }));
  ...
  it("updates the transfer before recording the revision and then refetches both revision histories", async () => {
    ...
    await waitFor(() => expect(harness.apiRequest).toHaveBeenCalledWith("PATCH", "/api/vouchers/90", expect.objectContaining({ totalAmount: "70" })));
    expect(harness.apiRequest).toHaveBeenCalledWith("PUT", "/api/stock-transfers/9", expect.objectContaining({
    ...
    expect(harness.toast).toHaveBeenCalledWith({ title: "Revision Saved", description: "Rev 2 recorded and transfer updated" });
  });
  ...
  it("approves a pending revision through the registered approval endpoint and invalidates transfer state", async () => {
    ...
    await waitFor(() => expect(harness.apiRequest).toHaveBeenCalledWith("POST", "/api/stock-transfer-revisions/41/approve", {}));
