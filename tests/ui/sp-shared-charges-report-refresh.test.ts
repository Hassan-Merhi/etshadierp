...
describe("Supplier Partner profit report refresh", () => {
  it.each([
    ...
    "refreshes every cached profit-report date range after %s %s",
    async (method, pathname) => {
      ...
      expect(
        options.predicate({
          queryKey: [
            "/api/sp/report/profit?startDate=2026-09-01&endDate=2026-09-30",
          ],
        }),
      ).toBe(true);
      expect(options.predicate({ queryKey: ["/api/sp/report/stock"] })).toBe(
        false,
      );
    },
  );
