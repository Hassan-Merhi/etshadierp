...
describe("repository-wide god-file architecture boundaries", () => {
  ...
  it("reports the outstanding split backlog as a falling ceiling", () => {
    ...
    //
    expect(report.summary.grandfatheredFiles).toBeLessThanOrEqual(0);
    expect(report.summary.grandfatheredExcessLines).toBeLessThanOrEqual(0);
  });
});
