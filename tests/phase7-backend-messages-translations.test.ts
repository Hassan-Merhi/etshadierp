...
describe("Phase 7 backend-message translations", () => {
  it("covers every reviewed backend phrase exactly once", () => {
    expect(backendMessagesPhase7Translations).toHaveLength(626);
    expect(
      new Set(backendMessagesPhase7Translations.map((entry) => entry.en)).size,
    ).toBe(626);
  ...
  it("translates authentication, import, approval and operations messages", () => {
    expect(translatePhase7BackendMessageText("Search Stock Items", "ar")).toBe(
      "البحث في عناصر المخزون",
    );
    expect(
      translatePhase7BackendMessageText(
    ...
    ).toBe("Trop de tentatives de connexion. Veuillez réessayer plus tard.");
    expect(
      translatePhase7BackendMessageText(
        "No active WhatsApp recipients configured",
        "ar",
      ),
    ).toBe("لا يوجد مستلمون نشطون مهيؤون في واتساب");
    expect(
      translatePhase7BackendMessageText("Security audit unavailable", "fr"),
    ).toBe("Audit de sécurité indisponible");
  });

  it("translates factory production bonus labels and templates", () => {
    expect(translatePhase7BackendMessageText("Production Bonus", "ar")).toBe(
      "مكافأة الإنتاج",
    );
    expect(translatePhase7BackendMessageText("Other Bonus", "fr")).toBe(
      "Autre prime",
    );
    expect(
      translatePhase7BackendMessageText(
        "Copied settings from 2026-08-06",
        "fr",
      ),
    ).toBe("Paramètres copiés depuis le 2026-08-06");
    expect(
      translatePhase7BackendMessageText(
      ...
      "العامل Nadia ينتمي إلى عدة مناصب إنتاج بتاريخ 2026-08-07. اختر منصب الإنتاج قبل حفظ إدخال المخزون.",
    );
  });

  it("preserves business values while translating nested operational fragments", () => {
    expect(
      translatePhase7BackendMessageText("Import job JOB-42 not found", "ar"),
    ).toBe("لم يتم العثور على مهمة الاستيراد JOB-42");
    expect(
      translatePhase7BackendMessageText(
        "Quantity 12 for stock item 44 exceeds available stock (8)",
        "fr",
      ),
    ).toBe(
    ...
    expect(
      translatePhase7BackendMessageText(
        'Voucher JV-88: "Office rent" — 1,500 from Main Bank',
        "ar",
      ),
  ...
  it("switches directly between Arabic and French", () => {
    expect(translatePhase7BackendMessageText("تم رفض الوصول", "fr")).toBe(
      "Accès refusé",
    );
    expect(translatePhase7BackendMessageText("Compte introuvable", "ar")).toBe(
  ...
  it("recognizes reviewed messages without translating stored business values", () => {
    ...
    expect(
      translatePhase7BackendMessageText("HMD INTERNATIONAL GROUP", "ar"),
    ).toBeNull();
