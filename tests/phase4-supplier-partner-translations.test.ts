...
describe("Phase 4 Supplier Partner translations", () => {
  it("covers every reviewed Supplier Partner phrase exactly once", () => {
    expect(supplierPartnerPhase4Translations).toHaveLength(230);
    expect(new Set(supplierPartnerPhase4Translations.map((entry) => entry.en)).size).toBe(230);
  ...
  it("translates Supplier Partner migration, reporting and setup copy", () => {
    ...
    expect(translatePhase4SupplierPartnerText("Supplier Payable", "fr")).toBe(
      "Montant dû au fournisseur",
    );
    expect(translatePhase4SupplierPartnerText("Supplier Partner setup complete", "ar")).toBe(
      "اكتمل إعداد شريك المورد",
    );
  });
  ...
  it("preserves dynamic company, voucher and migration identifiers", () => {
    ...
    expect(translatePhase4SupplierPartnerText("Voucher PV-440 for $1500", "fr")).toBe(
      "Pièce PV-440 de $1500",
    );
    expect(
      translatePhase4SupplierPartnerText(
        "4 charge mapping(s) need approval and 2 remain unmapped.",
        "ar",
      ),
    ).toBe("يحتاج 4 ربط رسوم إلى الموافقة ولا يزال 2 دون ربط.");
  });
